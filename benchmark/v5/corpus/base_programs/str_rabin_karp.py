...
def rabin_karp_search(text: str, pattern: str) -> List[int]:
    ...
    # High-order coefficient: base^(m-1) mod MOD
    ...
    for i in range(n - m + 1):
        if h_text == h_pattern:
            # Verify to avoid false positives from hash collision
...
def test_rabin_karp():
    ...
    # Test 9: matches reference brute-force on random input
    import random
    ...
    # Test 11: compute_hash consistency
    assert compute_hash("abc") == compute_hash("abc")
    assert compute_hash("abc") != compute_hash("abd")  # very likely

    # Test 12: pattern with repeated chars
...
if __name__ == "__main__":
    ...
    text = "abcabcabc"
    pattern = "abc"
    print(f"rabin_karp_search({text!r}, {pattern!r}) = "
