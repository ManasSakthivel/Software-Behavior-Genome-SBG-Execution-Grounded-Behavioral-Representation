# program_id: algo_rec_divide_conquer
# category: recursive_programs
# spec_version: 1.0
...
def test_divide_conquer():
    ...
    # Test 2: empty and single element
    ...
    # Test 9: random points — distance is non-negative and <= brute-force
    import random
    rng = random.Random(7)
    pts2 = [(rng.uniform(-100, 100), rng.uniform(-100, 100)) for _ in range(30)]
    _, d_dc = closest_pair(pts2)
    # brute force
    d_brute = min(_dist(pts2[i], pts2[j])
                  for i in range(len(pts2))
                  for j in range(i + 1, len(pts2)))
    assert abs(d_dc - d_brute) < 1e-9, f"dc={d_dc}, brute={d_brute}"
    ...
    # Test 13: all subsets of {1,2,3}
    ps3 = set(power_set([1, 2, 3]))
    expected = {frozenset(s) for s in [[], [1], [2], [3],
                                        [1,2], [1,3], [2,3], [1,2,3]]}
    assert ps3 == expected
