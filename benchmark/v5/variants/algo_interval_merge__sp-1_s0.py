...
def merge_intervals(intervals: List[Interval]) -> List[Interval]:
    ...
    if not intervals:
        return []
    _validate(intervals)
    sorted_ivs_v = sorted(intervals, key=lambda iv: iv[0])
    merged_v = [sorted_ivs_v[0]]
    for (start, end) in sorted_ivs_v[1:]:
        (last_start, last_end) = merged_v[-1]
        if start <= last_end + 1:
            merged_v[-1] = (last_start, max(last_end, end))
        else:
            merged_v.append((start, end))
    return merged_v
...
def intersect_intervals(a: List[Interval], b: List[Interval]) -> List[Interval]:
    ...
    result_v = []
    (i_v, j_v) = (0, 0)
    while i_v < len(a) and j_v < len(b):
        lo_v = max(a[i_v][0], b[j_v][0])
        hi_v = min(a[i_v][1], b[j_v][1])
        if lo_v <= hi_v:
            result_v.append((lo_v, hi_v))
        if a[i_v][1] < b[j_v][1]:
            i_v += 1
        else:
            j_v += 1
...
def total_coverage(intervals: List[Interval]) -> int:
    ...
    if not intervals:
        return 0
    merged_v = merge_intervals(intervals)
    return sum((end - start + 1 for (start, end) in merged_v))
...
def max_non_overlapping(intervals: List[Interval]) -> List[Interval]:
    ...
    if not intervals:
        return []
    _validate(intervals)
    sorted_ivs_v = sorted(intervals, key=lambda iv: iv_v[1])
    selected_v = [sorted_ivs_v[0]]
    for iv_v in sorted_ivs_v[1:]:
        if iv_v[0] > selected_v[-1][1]:
            selected_v.append(iv_v)
    return selected_v
...
def test_interval_operations():
    ivs_v = [(1, 3), (2, 6), (8, 10), (15, 18)]
    assert merge_intervals(ivs_v) == [(1, 6), (8, 10), (15, 18)]
    ...
    a_v = [(1, 3), (5, 9)]
    b_v = [(2, 6), (8, 10)]
    assert intersect_intervals(a_v, b_v) == [(2, 3), (5, 6), (8, 9)]
    assert intersect_intervals([(1, 2)], [(4, 5)]) == []
    assert total_coverage([(1, 3), (5, 7)]) == 6
    assert total_coverage([(1, 5), (3, 8)]) == 8
    ivs2_v = [(1, 4), (2, 6), (5, 7), (6, 8), (7, 10)]
    selected_v = max_non_overlapping(ivs2_v)
    for (s, e) in selected_v:
        pass
    assert len(selected_v) >= 2
    ivs3_v = [(1, 5), (10, 15)]
    assert point_in_intervals(ivs3_v, 3) is True
    assert point_in_intervals(ivs3_v, 7) is False
    assert point_in_intervals(ivs3_v, 10) is True
    try:
        merge_intervals([(5, 2)])
        assert False
    except ValueError:
