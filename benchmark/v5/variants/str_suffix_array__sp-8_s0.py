...
def search_pattern(s: str, sa: List[int], pattern: str) -> List[int]:
    ...
    # Find rightmost suffix with prefix == pattern
    lo, hi = left, n
    while lo < hi:
        mid = (lo + hi) // 2
        if s[sa[mid]:sa[mid] + m] == pattern:
...
def test_suffix_array():
    ...
    assert sa2 == [3, 2, 1, 0]  # shortest suffix first

    # Test 6: search_pattern — existing pattern
    s = "abcabcabc"
    sa3 = build_suffix_array(s)
    positions = search_pattern(s, sa3, "abc")
    assert sorted(positions) == [0, 3, 6], f"got {positions}"

    # Test 7: search_pattern — absent pattern
    assert search_pattern(s, sa3, "xyz") == []

    # Test 8: search_pattern — single char
    assert sorted(search_pattern("banana", sa, "a")) == [1, 3, 5]

    # Test 9: suffix array matches brute-force for random string
    import random
    ...
    # Test 10: LCP values are non-negative and <= string length
    s2 = "mississippi"
