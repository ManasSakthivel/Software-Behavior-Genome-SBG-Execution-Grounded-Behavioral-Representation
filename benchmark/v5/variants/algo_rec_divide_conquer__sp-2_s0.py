...
def fn_merge_sort_rec(arr: list) -> list:
    """Recursive merge sort. Returns new sorted list; input unchanged."""
    if len(arr) <= 1:
        return list(arr)
    mid = len(arr) // 2
    left = fn_merge_sort_rec(arr[:mid])
    right = fn_merge_sort_rec(arr[mid:])
...
def fn_closest_pair(points: List[Tuple]) -> Tuple:
...
def fn_power_set(items: List[Any]) -> List[FrozenSet]:
    """Return all 2^n subsets of items as frozensets."""
    if not items:
        return [frozenset()]
    first = items[0]
    rest_subsets = fn_power_set(items[1:])
    return rest_subsets + [s | {first} for s in rest_subsets]

def test_divide_conquer():
    assert fn_merge_sort_rec([3, 1, 4, 1, 5, 9, 2, 6]) == [1, 1, 2, 3, 4, 5, 6, 9]
    assert fn_merge_sort_rec([]) == []
    assert fn_merge_sort_rec([42]) == [42]
    assert fn_merge_sort_rec([1, 2, 3, 4]) == [1, 2, 3, 4]
    assert fn_merge_sort_rec([5, 4, 3, 2, 1]) == [1, 2, 3, 4, 5]
    original = [5, 3, 1]
    _ = fn_merge_sort_rec(original)
    assert original == [5, 3, 1]
    pts = [(0, 0), (3, 4), (1, 1), (10, 10), (1.1, 1.1)]
    (pair, d) = fn_closest_pair(pts)
    assert abs(d - _dist((1, 1), (1.1, 1.1))) < 1e-09
    (pair2, d2) = fn_closest_pair([(0, 0), (3, 4)])
