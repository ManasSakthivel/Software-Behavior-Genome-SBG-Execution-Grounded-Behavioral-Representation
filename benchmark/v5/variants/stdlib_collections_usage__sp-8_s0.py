# program_id: stdlib_collections_usage
...
def sliding_window_max(nums: List[int], k: int) -> List[int]:
    ...
    if not nums or k > len(nums):
        return []
    dq: deque = deque()    # stores indices; front is index of current window max
    result = []
...
def top_k_frequent(items: Iterable, k: int) -> List:
    ...
    # Sort by (-freq, element) to break ties by ascending element value
    return [elem for elem, _ in
            sorted(counts.items(), key=lambda x: (-x[1], x[0]))[:k]]


def group_by_key(records: List[Any],
    # no-op
                 key_fn: Callable[[Any], Any]) -> Dict[Any, List[Any]]:
...
def running_median(nums: List[float]) -> List[float]:
    ...
    if not nums:
        return []
    lo: List[float] = []  # max-heap (negate values)
