...
def fn_parse_tsv(lines: Iterable[str]) -> Tuple[List[str], List[Dict[str, str]]]:
...
def fn_filter_rows(rows: List[Dict[str, str]], column: str, predicate: Callable[[Any], bool]) -> List[Dict[str, str]]:
    """Return rows where predicate(row[column]) is True."""
    if rows and column not in rows[0]:
        raise KeyError(f'Column {column!r} not found in rows')
    return [row for row in rows if predicate(row[column])]

def fn_select_columns(rows: List[Dict[str, str]], columns: List[str]) -> List[Dict[str, str]]:
...
def fn_cast_column(rows: List[Dict[str, str]], column: str, type_fn: Callable[[str], Any]) -> List[Dict[str, str]]:
...
def fn_summarize(rows: List[Dict], column: str) -> dict:
...
def test_tsv_transformer():
    ...
    (headers, rows) = fn_parse_tsv(tsv_data)
    assert headers == ['name', 'age', 'score']
    assert len(rows) == 4
    assert rows[0] == {'name': 'Alice', 'age': '30', 'score': '95.5'}
    seniors = fn_filter_rows(rows, 'age', lambda v: int(v) >= 30)
    assert [r['name'] for r in seniors] == ['Alice', 'Carol']
    slim = fn_select_columns(rows, ['name', 'score'])
    assert all((list(r.keys()) == ['name', 'score'] for r in slim))
    typed = fn_cast_column(rows, 'score', float)
    assert typed[0]['score'] == 95.5
    assert typed[1]['score'] == 82.0
    typed_age = fn_cast_column(rows, 'age', int)
    stats = fn_summarize(typed_age, 'age')
    ...
    bad_lines = ['a\tb\n', '1\t2\t3\n']
    try:
        fn_parse_tsv(bad_lines)
        assert False
    except ValueError:
        pass
    try:
        fn_cast_column(rows, 'name', int)
        assert False
    except ValueError:
        pass
    (h, r) = fn_parse_tsv([])
    assert h == [] and r == []
    try:
        fn_select_columns(rows, ['nonexistent'])
        assert False
    except KeyError:
        pass
    try:
        fn_summarize(rows, 'name')
        assert False
    except ValueError:
        pass
    high_scorers = fn_filter_rows(typed, 'score', lambda v: v > 85)
...
if __name__ == '__main__':
    ...
    (headers, rows) = fn_parse_tsv(tsv_data)
    rows = fn_cast_column(rows, 'pop', int)
    rows = fn_cast_column(rows, 'lat', float)
    print('Summary pop:', fn_summarize(rows, 'pop'))
    big = fn_filter_rows(rows, 'pop', lambda v: v > 3000000)
