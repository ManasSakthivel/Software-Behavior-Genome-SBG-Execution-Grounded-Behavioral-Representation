...
def fn_word_frequencies(lines: List[str]) -> Counter:
...
def fn_top_k_words(counter: Counter, k: int) -> List[Tuple[str, int]]:
...
def fn_inverted_index(lines: List[str]) -> Dict[str, List[int]]:
...
def fn_ngrams(lines: List[str], n: int) -> Counter:
...
def fn_sliding_window_counts(words: List[str], window_size: int) -> List[Counter]:
...
def test_collections_pipeline():
    ...
    freq = fn_word_frequencies(corpus)
    assert freq['the'] == 5
    assert freq['fox'] == 3
    assert freq['quick'] == 3
    assert 'The' not in freq
    top3 = fn_top_k_words(freq, 3)
    assert top3[0][0] == 'the' and top3[0][1] == 5
    assert len(top3) == 3
    assert fn_top_k_words(freq, 0) == []
    idx = fn_inverted_index(corpus)
    ...
    try:
        ...
        pass
    words = ['a', 'b', 'a', 'c', 'a']
    windows = fn_sliding_window_counts(words, 3)
    assert len(windows) == 3
    assert windows[0]['a'] == 2
    assert windows[1]['a'] == 1
    assert windows[2]['a'] == 2
    try:
        fn_sliding_window_counts(words, 0)
        assert False
    except ValueError:
        pass
    assert fn_sliding_window_counts(['x', 'y'], 5) == []
...
if __name__ == '__main__':
    ...
    freq = fn_word_frequencies(corpus)
    print('Top 5:', fn_top_k_words(freq, 5))
    print('Bigrams:', fn_ngrams(corpus, 2).most_common(5))