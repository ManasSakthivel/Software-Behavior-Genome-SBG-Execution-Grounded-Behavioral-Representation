...
def fn_tokenize(source: str) -> List[Token]:
    """
...
def test_lexer():
    ...
    def fn_types(src):
        return [(t.type, t.value) for t in fn_tokenize(src) if t.type != 'EOF']
    assert fn_types('123 456') == [('INTEGER', '123'), ('INTEGER', '456')]
    assert fn_types('3.14') == [('FLOAT', '3.14')]
    assert fn_types('foo bar_2') == [('IDENT', 'foo'), ('IDENT', 'bar_2')]
    assert fn_types('+ - * /') == [('PLUS', '+'), ('MINUS', '-'), ('STAR', '*'), ('SLASH', '/')]
    assert fn_types('(x = 5)') == [('LPAREN', '('), ('IDENT', 'x'), ('ASSIGN', '='), ('INTEGER', '5'), ('RPAREN', ')')]
    toks = fn_types('result = 3.14 * (x + 2);')
    tok_types = [t for (t, _) in toks]
    assert tok_types == ['IDENT', 'ASSIGN', 'FLOAT', 'STAR', 'LPAREN', 'IDENT', 'PLUS', 'INTEGER', 'RPAREN', 'SEMICOL']
    all_toks = fn_tokenize('1 + 2')
    ...
    assert fn_types('1.5 2.7') == [('FLOAT', '1.5'), ('FLOAT', '2.7')]
    print('All lexer tokenizer tests passed.')
if __name__ == '__main__':
    test_lexer()
    src = 'area = 3.14 * radius * radius;'
    for tok in fn_tokenize(src):
        print(tok)