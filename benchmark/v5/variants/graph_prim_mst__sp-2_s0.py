...
def fn_prim_mst(graph: Dict[Any, List[Tuple[Any, float]]], start: Any) -> dict:
    """
...
def fn_mst_weight(graph: Dict[Any, List[Tuple[Any, float]]], start: Any) -> float:
    """Return total weight of MST component containing start."""
    return fn_prim_mst(graph, start)['total_weight']

def test_prim_mst():

    def fn_undirected(edges):
        g = {}
        for (u, v, w) in edges:
            g.setdefault(u, []).append((v, w))
            g.setdefault(v, []).append((u, w))
        return g
    g1 = fn_undirected([(0, 1, 2), (0, 3, 6), (1, 2, 3), (1, 3, 8), (1, 4, 5), (2, 4, 7), (3, 4, 9)])
    r1 = fn_prim_mst(g1, 0)
    assert abs(r1['total_weight'] - 16.0) < 1e-09, r1['total_weight']
    ...
    assert r2['edges'] == []
    g3 = fn_undirected([(0, 1, 5)])
    r3 = fn_prim_mst(g3, 0)
    assert r3['total_weight'] == 5.0
    assert len(r3['edges']) == 1
    g4 = fn_undirected([(0, 1, 1), (2, 3, 2)])
    r4 = fn_prim_mst(g4, 0)
    ...
    try:
        ...
        pass
    g5 = fn_undirected([(0, 1, -3)])
    try:
        fn_prim_mst(g5, 0)
        assert False
    except ValueError:
        pass
    g6 = fn_undirected([(0, 1, 4), (0, 2, 3), (1, 2, 1)])
    assert abs(fn_mst_weight(g6, 0) - 4.0) < 1e-09
    g7 = fn_undirected([(0, 1, 1), (1, 2, 1), (2, 3, 1), (3, 4, 1)])
    r7 = fn_prim_mst(g7, 0)
...
if __name__ == '__main__':
    ...
    def fn_undirected(edges):
        ...
        for (u, v, w) in edges:
            ...
            g.setdefault(v, []).append((u, w))
        return g
    g = fn_undirected([(0, 1, 2), (0, 3, 6), (1, 2, 3), (1, 3, 8), (1, 4, 5), (2, 4, 7), (3, 4, 9)])
    r = fn_prim_mst(g, 0)
