...
class Event:
    ...
    def fn_occurrences_from(self, ref: datetime.datetime) -> Iterator[datetime.datetime]:
...
class Scheduler:
    ...
    def fn_add_event(self, event: Event) -> None:
        if event.name in self._events:
            raise ValueError(f'Event {event.name!r} already registered')
        self._events[event.name] = event

    def fn_remove_event(self, name: str) -> None:
        if name not in self._events:
            raise KeyError(f'Event {name!r} not found')
        del self._events[name]

    def fn_next_occurrences(self, reference_time: datetime.datetime, limit: int=10) -> List[Tuple[datetime.datetime, str]]:
        ...
        for (name, ev) in self._events.items():
            gen = ev.fn_occurrences_from(reference_time)
    ...
    def fn_events_in_window(self, start: datetime.datetime, end: datetime.datetime) -> List[Tuple[datetime.datetime, str]]:
        """All occurrences in [start, end], sorted by (time, name)."""
        results: List[Tuple[datetime.datetime, str]] = []
        for (name, ev) in self._events.items():
            for occ in ev.fn_occurrences_from(start):
                if occ > end:
                    break
    ...
    def fn_event_count(self, reference_time: datetime.datetime, horizon_seconds: int=3600) -> Dict[str, int]:
        ...
        counts: Dict[str, int] = {}
        for (name, ev) in self._events.items():
            c = sum((1 for _ in self.fn_events_in_window(reference_time, horizon) if _[1] == name))
            counts[name] = c
        return counts
...
def test_scheduler():
    ...
    occs = list((next(ev.fn_occurrences_from(T0)) for _ in range(1)))
    first = next(ev.fn_occurrences_from(T0))
    assert first == T0
    gen = ev.fn_occurrences_from(T0)
    ...
    all_occs = list(ev2.fn_occurrences_from(T0))
    assert len(all_occs) == 3
    mid = T0 + datetime.timedelta(seconds=45)
    first_after_mid = next(ev.fn_occurrences_from(mid))
...
if __name__ == '__main__':
    ...
    s = Scheduler()
    s.fn_add_event(Event('heartbeat', T0, 30))
    s.fn_add_event(Event('report', T0, 300))
    for (dt, name) in s.fn_next_occurrences(T0, limit=8):
