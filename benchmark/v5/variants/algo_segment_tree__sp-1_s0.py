...
class SegmentTree:
    ...
    def _build(self, arr: List[int], node: int, start: int, end: int) -> None:
        if start == end:
            ...
            return
        mid_v = (start + end) // 2
        self._build(arr, 2 * node, start, mid_v)
        self._build(arr, 2 * node + 1, mid_v + 1, end)
    ...
    def _push_down(self, node: int, start: int, end: int) -> None:
        if self._lazy[node] != 0:
            ...
            self._tree[left] += self._lazy[node] * (mid_v - start + 1)
            self._lazy[left] += self._lazy[node]
            self._tree[right] += self._lazy[node] * (end - mid_v)
            self._lazy[right] += self._lazy[node]
            self._lazy[node] = 0

    def _update(self, node: int, start: int, end: int, l: int, r: int, delta: int) -> None:
        if r < start or end < l:
            return
        ...
        mid_v = (start + end) // 2
        self._update(2 * node, start, mid_v, l, r, delta)
        self._update(2 * node + 1, mid_v + 1, end, l, r, delta)
        self._tree[node] = self._tree[2 * node] + self._tree[2 * node + 1]
    ...
    def _query(self, node: int, start: int, end: int, l: int, r: int) -> int:
        ...
        self._push_down(node, start, end)
        mid_v = (start + end) // 2
        return self._query(2 * node, start, mid_v, l, r) + self._query(2 * node + 1, mid_v + 1, end, l, r)
    ...
    def to_list(self) -> List[int]:
        return [self.point_query(i_v) for i_v in range(self._n)]

def test_segment_tree():
    st_v = SegmentTree([1, 3, 5, 7, 9, 11])
    assert st_v.query(0, 5) == 36
    assert st_v.query(1, 3) == 15
    assert st_v.query(2, 2) == 5
    assert st_v.point_query(4) == 9
    st_v.update(1, 3, 2)
    assert st_v.query(1, 3) == 21
    assert st_v.query(0, 5) == 42
    assert st_v.point_query(0) == 1
    assert st_v.point_query(4) == 9
    st2_v = SegmentTree([0, 0, 0, 0])
    st2_v.update(0, 3, 5)
    assert st2_v.query(0, 3) == 20
    assert st2_v.to_list() == [5, 5, 5, 5]
    st3_v = SegmentTree([1, 2, 3, 4, 5])
    st3_v.update(0, 4, 1)
    st3_v.update(2, 4, 3)
    assert st3_v.to_list() == [2, 3, 7, 8, 9]
    assert st3_v.query(0, 4) == 29
    st_empty_v = SegmentTree([])
    st1_v = SegmentTree([42])
    assert st1_v.query(0, 0) == 42
    st1_v.update(0, 0, 8)
    assert st1_v.query(0, 0) == 50
    st4_v = SegmentTree([1, 2, 3])
    try:
        st4_v.query(0, 5)
        assert False
    except IndexError:
