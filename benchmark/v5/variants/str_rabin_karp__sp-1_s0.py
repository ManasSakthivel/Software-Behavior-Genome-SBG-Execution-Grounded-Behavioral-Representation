...
def rabin_karp_search(text: str, pattern: str) -> List[int]:
    ...
    if m > n:
        return []
    h_pattern_v = compute_hash(pattern)
    h_text_v = compute_hash(text[:m])
    high_v = pow(BASE, m - 1, MOD)
    matches_v = []
    for i_v in range(n - m + 1):
        if h_text_v == h_pattern_v:
            if text[i_v:i_v + m] == pattern:
                matches_v.append(i_v)
        if i_v + m < n:
            h_text_v = (h_text_v - ord(text[i_v]) * high_v) % MOD
            h_text_v = (h_text_v * BASE + ord(text[i_v + m])) % MOD
    return matches_v
...
def test_rabin_karp():
    ...
    result_v = rabin_karp_search('abc', '')
    assert result_v == [0, 1, 2, 3]
    assert rabin_karp_search('', 'a') == []
    assert rabin_karp_search('', '') == [0]
    import random
    rng_v = random.Random(42)
    text_v = ''.join((rng_v.choice('abcd') for __v in range(300)))
    pattern_v = ''.join((rng_v.choice('abcd') for __v in range(5)))
    rk_result_v = rabin_karp_search(text_v, pattern_v)
    brute_v = [i_v for i_v in range(len(text_v) - len(pattern_v) + 1) if text_v[i_v:i_v + len(pattern_v)] == pattern_v]
    assert rk_result_v == brute_v, f'RK={rk_result_v[:5]}..., brute={brute_v[:5]}...'
