...
class resource_guard:
    ...
    def __enter__(self) -> ManagedResource:
        self._resource.fn_acquire()
        return self._resource

    def __exit__(self, exc_type, exc_val, exc_tb) -> bool:
        self._resource.fn_release()
        return False
...
class CleanupRegistry:
    ...
    def fn_register(self, callback: Callable, *args: Any, **kwargs: Any) -> None:
        """Register a cleanup callback."""
        self._callbacks.append((callback, args, kwargs))

    def fn_run_all(self) -> List[Exception]:
        """
...
def test_resource_guard():
    ...
    order = []
    reg = CleanupRegistry()
    reg.fn_register(order.append, 'first')
    reg.fn_register(order.append, 'second')
    reg.fn_register(order.append, 'third')
    reg.fn_run_all()
    ...
    reg2.fn_register(failing, 1)
    reg2.fn_register(failing, 2)
    try:
        reg2.fn_run_all()
        assert False
