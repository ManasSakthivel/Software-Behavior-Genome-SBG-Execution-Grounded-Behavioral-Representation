...
def fn_build_huffman_tree(freq_map: Dict[str, int]) -> HuffNode:
...
def fn_build_codebook(root: HuffNode) -> Dict[str, str]:
...
def fn_encode(text: str, codebook: Dict[str, str]) -> str:
...
def fn_decode(bits: str, root: HuffNode) -> str:
...
def fn_huffman_encode_decode(text: str) -> Tuple[str, str, Dict[str, str]]:
    """Full pipeline: encode then decode, assert round-trip."""
    from collections import Counter
    freq = dict(Counter(text))
    root = fn_build_huffman_tree(freq)
    codebook = fn_build_codebook(root)
    bits = fn_encode(text, codebook)
    decoded = fn_decode(bits, root)
    assert decoded == text, f'Round-trip failed: {decoded!r} != {text!r}'
    return (bits, decoded, codebook)

def test_huffman():
    text = 'abracadabra'
    (bits, decoded, codebook) = fn_huffman_encode_decode(text)
    assert decoded == text
    ...
    text2 = 'aaaaabbbcc'
    (bits2, _, _) = fn_huffman_encode_decode(text2)
    assert len(bits2) < len(text2) * 8
    root_single = fn_build_huffman_tree({'a': 5})
    cb = fn_build_codebook(root_single)
    assert cb == {'a': '0'}
    assert fn_decode(fn_encode('aaa', cb), root_single) == 'aaa'
    root2 = fn_build_huffman_tree({'a': 3, 'b': 1})
    cb2 = fn_build_codebook(root2)
    (bits3, dec3, _) = fn_huffman_encode_decode('aaab')
    assert dec3 == 'aaab'
    try:
        fn_build_huffman_tree({})
        assert False
    except ValueError:
        pass
    try:
        fn_build_huffman_tree({'a': 0, 'b': 1})
        assert False
    except ValueError:
        pass
    root3 = fn_build_huffman_tree({'a': 1})
    cb3 = fn_build_codebook(root3)
    try:
        fn_encode('az', cb3)
        assert False
    except KeyError:
    ...
    (bits9, dec9, _) = fn_huffman_encode_decode(long_text)
    assert dec9 == long_text
    freq = {'a': 100, 'b': 5, 'c': 3, 'd': 1}
    root4 = fn_build_huffman_tree(freq)
    cb4 = fn_build_codebook(root4)
