...
def fn_parse_iso(s: str) -> datetime:
...
def fn_business_days_between(start: date, end: date) -> int:
...
def fn_add_business_days(d: date, n: int) -> date:
...
def fn_format_duration(seconds: int) -> str:
...
def fn_week_number(d: date) -> int:
    """Return ISO week number (1–53) for date d."""
    return d.isocalendar()[1]

def fn_dates_in_range(start: date, end: date) -> List[date]:
...
def test_datetime_processor():
    dt = fn_parse_iso('2024-03-15')
    assert dt.year == 2024 and dt.month == 3 and (dt.day == 15)
    dt2 = fn_parse_iso('2024-03-15T09:30:00')
    assert dt2.hour == 9 and dt2.minute == 30
    dt3 = fn_parse_iso('2024-06-01T12:00:00Z')
    assert dt3.hour == 12
    try:
        fn_parse_iso('15/03/2024')
        assert False
    except ValueError:
        pass
    d_mon = date(2024, 3, 4)
    d_sat = date(2024, 3, 9)
    assert fn_business_days_between(d_mon, d_sat) == 5
    d_fri = date(2024, 3, 8)
    d_mon2 = date(2024, 3, 11)
    assert fn_business_days_between(d_fri, d_mon2) == 1
    assert fn_business_days_between(date(2024, 3, 5), date(2024, 3, 4)) == 0
    assert fn_business_days_between(date(2024, 3, 5), date(2024, 3, 5)) == 0
    d = date(2024, 3, 8)
    result = fn_add_business_days(d, 1)
    assert result == date(2024, 3, 11)
    assert fn_add_business_days(date(2024, 3, 5), 0) == date(2024, 3, 5)
    assert fn_format_duration(0) == '0s'
    assert fn_format_duration(60) == '1m'
    assert fn_format_duration(3661) == '1h 1m 1s'
    assert fn_format_duration(86400) == '1d'
    assert fn_format_duration(90061) == '1d 1h 1m 1s'
    assert fn_week_number(date(2024, 1, 1)) == 1
    assert fn_week_number(date(2024, 3, 18)) == 12
    dr = fn_dates_in_range(date(2024, 3, 1), date(2024, 3, 5))
    assert len(dr) == 5
    assert dr[0] == date(2024, 3, 1) and dr[-1] == date(2024, 3, 5)
    assert fn_dates_in_range(date(2024, 3, 5), date(2024, 3, 1)) == []
    print('All datetime processor tests passed.')
if __name__ == '__main__':
    test_datetime_processor()
    print('business_days Mon-Fri:', fn_business_days_between(date(2024, 3, 4), date(2024, 3, 9)))
    print('add 5 business days from Fri:', fn_add_business_days(date(2024, 3, 8), 5))
    print('format_duration(3661):', fn_format_duration(3661))