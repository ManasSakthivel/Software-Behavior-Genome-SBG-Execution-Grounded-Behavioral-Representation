# program_id: err_exception_hierarchy
...
def run_pipeline(
...
) -> Tuple[List[Dict[str, Any]], List[AppError]]:
    """
    ...
    for raw in raw_records:
        try:
            typed = parse_record(raw)
            valid = validate_record(typed)
...
def test_exception_hierarchy():
    ...
    # no-op
    rec = validate_record({"name": "Alice", "age": 30, "score": 0.95})
    assert rec["name"] == "Alice"

    # Test 2: multiple field errors collected
    try:
    ...
    # Test 4: parse_record success
    parsed = parse_record({"name": "Carol", "age": "25", "score": "0.8"})
    assert parsed["age"] == 25
    assert abs(parsed["score"] - 0.8) < 1e-9

    # Test 5: parse_record raises ParseError
    try:
    ...
    # Test 6: ParseError chains original exception
    try:
        parse_record({"name": "X", "age": "bad", "score": "0.1"})
    except ParseError as e:
        assert e.__cause__ is not None

    # Test 7: safe_divide normal
    assert abs(safe_divide(10, 3) - 10 / 3) < 1e-9

    # Test 8: safe_divide by zero raises RangeError
    try:
        safe_divide(5.0, 0)
        assert False
    except RangeError:
    ...
    # Test 9: run_pipeline with mixed valid/invalid
    raw_data = [
        {"name": "Alice", "age": "30", "score": "0.9"},
        {"name": "", "age": "25", "score": "0.5"},    # validation fail
        {"name": "Bob", "age": "abc", "score": "0.7"}, # parse fail
        {"name": "Carol", "age": "28", "score": "0.8"},
    ]
    ok, errs = run_pipeline(raw_data)
    assert len(ok) == 2
    assert len(errs) == 2

    # Test 10: run_pipeline with all valid
    ok2, errs2 = run_pipeline([{"name": "X", "age": "20", "score": "0.5"}])
    assert len(ok2) == 1 and len(errs2) == 0

    # Test 11: run_pipeline with empty input
    ok3, errs3 = run_pipeline([])
    assert ok3 == [] and errs3 == []

    # Test 12: exception hierarchy
    try:
