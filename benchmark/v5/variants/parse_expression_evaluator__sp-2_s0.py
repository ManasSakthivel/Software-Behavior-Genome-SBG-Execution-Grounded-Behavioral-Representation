...
def fn_tokenize_expr(expr_str: str) -> List[_Token]:
...
def fn_evaluate(expr_str: str) -> float:
    """Parse and evaluate an arithmetic expression string."""
    tokens = fn_tokenize_expr(expr_str)
    return _Parser(tokens).fn_parse()

def test_expression_evaluator():

    def fn_approx(a, b, tol=1e-09):
        return abs(a - b) < tol
    assert fn_approx(fn_evaluate('1 + 2'), 3.0)
    assert fn_approx(fn_evaluate('2 + 3 * 4'), 14.0)
    assert fn_approx(fn_evaluate('(2 + 3) * 4'), 20.0)
    assert fn_approx(fn_evaluate('10 - 3 - 2'), 5.0)
    assert fn_approx(fn_evaluate('10 / 4'), 2.5)
    assert fn_approx(fn_evaluate('-5 + 3'), -2.0)
    assert fn_approx(fn_evaluate('-(2 + 3)'), -5.0)
    assert fn_approx(fn_evaluate('3.14 * 2'), 6.28)
    assert fn_approx(fn_evaluate('((2 + 3) * (4 - 1)) / 5'), 3.0)
    assert fn_approx(fn_evaluate('42'), 42.0)
    try:
        fn_evaluate('1 / 0')
        assert False
    ...
    try:
        ...
        pass
    assert fn_approx(fn_evaluate('2 * (3 + 4) - 6 / 2'), 11.0)
    toks = fn_tokenize_expr('1+2')
    assert toks[-1].type == 'EOF'
    assert fn_approx(fn_evaluate('-3.5 * -2'), 7.0)
    print('All expression evaluator tests passed.')
if __name__ == '__main__':
    test_expression_evaluator()
    expressions = ['2 + 3 * 4', '(2 + 3) * 4', '10 / (2 + 3)', '-(3 + 4) * 2', '3.14 * 2 * 2']
    for expr in expressions:
        print(f'  {expr!r:30s} = {fn_evaluate(expr)}')