...
def fn_compute_hash(s: str, base: int=BASE, mod: int=MOD) -> int:
...
def fn_rabin_karp_search(text: str, pattern: str) -> List[int]:
    """
    ...
    if m > n:
        return []
    h_pattern = fn_compute_hash(pattern)
    h_text = fn_compute_hash(text[:m])
...
def fn_rabin_karp_count(text: str, pattern: str) -> int:
    """Return the number of occurrences of pattern in text."""
    return len(fn_rabin_karp_search(text, pattern))

def test_rabin_karp():
    assert fn_rabin_karp_search('abcabcabc', 'abc') == [0, 3, 6]
    assert fn_rabin_karp_search('hello world', 'xyz') == []
    assert fn_rabin_karp_search('aaaa', 'aa') == [0, 1, 2]
    assert fn_rabin_karp_search('abc', 'abc') == [0]
    assert fn_rabin_karp_search('ab', 'abcd') == []
    assert fn_rabin_karp_search('banana', 'a') == [1, 3, 5]
    result = fn_rabin_karp_search('abc', '')
    assert result == [0, 1, 2, 3]
    assert fn_rabin_karp_search('', 'a') == []
    assert fn_rabin_karp_search('', '') == [0]
    import random
    rng = random.Random(42)
    text = ''.join((rng.choice('abcd') for _ in range(300)))
    pattern = ''.join((rng.choice('abcd') for _ in range(5)))
    rk_result = fn_rabin_karp_search(text, pattern)
    ...
    assert fn_rabin_karp_count('abababab', 'ab') == 4
    assert fn_rabin_karp_count('hello', 'xx') == 0
    assert fn_compute_hash('abc') == fn_compute_hash('abc')
    assert fn_compute_hash('abc') != fn_compute_hash('abd')
    assert fn_rabin_karp_search('aaaaaa', 'aaa') == [0, 1, 2, 3]
    print('All Rabin-Karp tests passed.')
if __name__ == '__main__':
    test_rabin_karp()
    text = 'abcabcabc'
    pattern = 'abc'
    print(f'rabin_karp_search({text!r}, {pattern!r}) = {fn_rabin_karp_search(text, pattern)}')