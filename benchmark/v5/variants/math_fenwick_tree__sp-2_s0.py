...
class FenwickTree:
    ...
    @classmethod
    def fn_from_list(cls, arr: List[int]) -> 'FenwickTree':
    ...
    def fn_update(self, i: int, delta: int) -> None:
    ...
    def fn_prefix_sum(self, i: int) -> int:
    ...
    def fn_range_sum(self, l: int, r: int) -> int:
        ...
        if l == 0:
            return self.fn_prefix_sum(r)
        return self.fn_prefix_sum(r) - self.fn_prefix_sum(l - 1)

    def fn_point_value(self, i: int) -> int:
        """Current value at index i."""
        return self.fn_range_sum(i, i)
...
def test_fenwick_tree():
    ...
    for (i, v) in enumerate([1, 3, 5, 7, 9, 11]):
        ft.fn_update(i, v)
    assert ft.fn_prefix_sum(5) == 36
    assert ft.fn_prefix_sum(2) == 9
    assert ft.fn_range_sum(1, 3) == 15
    assert ft.fn_range_sum(0, 5) == 36
    assert ft.fn_point_value(4) == 9
    ft.fn_update(2, 4)
    assert ft.fn_point_value(2) == 9
    assert ft.fn_prefix_sum(2) == 13
    ft2 = FenwickTree.fn_from_list([2, 4, 6, 8, 10])
    assert ft2.fn_prefix_sum(4) == 30
    assert ft2.fn_range_sum(1, 3) == 18
    arr = [3, 1, 4, 1, 5, 9, 2, 6]
    ft3 = FenwickTree.fn_from_list(arr)
    ft4 = FenwickTree(len(arr))
    for (i, v) in enumerate(arr):
        ft4.fn_update(i, v)
    for i in range(len(arr)):
        assert ft3.fn_prefix_sum(i) == ft4.fn_prefix_sum(i)
    try:
        ft.fn_prefix_sum(10)
        assert False
    except IndexError:
...
if __name__ == '__main__':
    test_fenwick_tree()
    arr = [1, 3, 5, 7, 9, 11]
    ft = FenwickTree.fn_from_list(arr)
    print('prefix_sum(5):', ft.fn_prefix_sum(5))
    print('range_sum(1,4):', ft.fn_range_sum(1, 4))
    ft.fn_update(2, 2)
    print('After update(2,+2), prefix_sum(5):', ft.fn_prefix_sum(5))