# program_id: file_tsv_transformer
...
def summarize(rows: List[Dict], column: str) -> dict:
    ...
    for i, row in enumerate(rows):
        try:
            ...
            raise ValueError(
                f"Row {i}: cannot convert column {column!r} to float: {exc}"
            ) from exc
    n = len(values)
    total = sum(values)
    min_val = min(values)
...
def test_tsv_transformer():
    ...
    assert stats["min"] == 25
    assert stats["max"] == 35
    assert abs(stats["mean"] - 29.5) < 1e-9

    # Test 6: wrong column count raises ValueError
    bad_lines = ["a\tb\n", "1\t2\t3\n"]
    try:
        parse_tsv(bad_lines)
        assert False
    ...
    # Test 7: bad cast raises ValueError
    try:
    ...
    # Test 9: select missing column raises KeyError
    try:
    ...
    # Test 10: summarize on non-numeric raises
    try:
        summarize(rows, "name")
        assert False
    except ValueError:
