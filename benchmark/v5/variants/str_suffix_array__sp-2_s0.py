...
def fn_build_suffix_array(s: str) -> List[int]:
    ...
    while gap < n and rank[sa[n - 1]] < n - 1:
        ...
        for i in range(1, n):
            ...
            if fn_sort_key(sa[i]) != fn_sort_key(sa[i - 1]):
                new_rank[sa[i]] += 1
        rank = new_rank
        gap *= 2
    return sa

def fn_build_lcp_array(s: str, sa: List[int]) -> List[int]:
    """
...
def fn_search_pattern(s: str, sa: List[int], pattern: str) -> List[int]:
...
def test_suffix_array():
    sa = fn_build_suffix_array('banana')
    assert sa == [5, 3, 1, 0, 4, 2], f'got {sa}'
    lcp = fn_build_lcp_array('banana', sa)
    assert lcp == [0, 1, 3, 0, 0, 2], f'got {lcp}'
    sa1 = fn_build_suffix_array('a')
    assert sa1 == [0]
    assert fn_build_suffix_array('') == []
    assert fn_build_lcp_array('', []) == []
    sa2 = fn_build_suffix_array('aaaa')
    assert sa2 == [3, 2, 1, 0]
    s = 'abcabcabc'
    sa3 = fn_build_suffix_array(s)
    positions = fn_search_pattern(s, sa3, 'abc')
    assert sorted(positions) == [0, 3, 6], f'got {positions}'
    assert fn_search_pattern(s, sa3, 'xyz') == []
    assert sorted(fn_search_pattern('banana', sa, 'a')) == [1, 3, 5]
    import random
    rng = random.Random(17)
    t = ''.join((rng.choice('abcde') for _ in range(50)))
    sa_fast = fn_build_suffix_array(t)
    sa_brute = sorted(range(len(t)), key=lambda i: t[i:])
    assert sa_fast == sa_brute, f'fast={sa_fast[:5]}, brute={sa_brute[:5]}'
    s2 = 'mississippi'
    sa4 = fn_build_suffix_array(s2)
    lcp2 = fn_build_lcp_array(s2, sa4)
...
if __name__ == '__main__':
    test_suffix_array()
    s = 'banana'
    sa = fn_build_suffix_array(s)
    lcp = fn_build_lcp_array(s, sa)
    print(f'SA({s!r}):  {sa}')
    print(f'LCP({s!r}): {lcp}')
    print("Positions of 'an':", fn_search_pattern(s, sa, 'an'))