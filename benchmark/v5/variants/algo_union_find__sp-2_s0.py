...
class UnionFind:
    ...
    def fn_union(self, x: int, y: int) -> bool:
        """Merge sets. Returns True if merge happened, False if already same."""
        self._validate(x)
        self._validate(y)
        (rx, ry) = (self.fn_find(x), self.fn_find(y))
        if rx == ry:
            return False
    ...
    def fn_connected(self, x: int, y: int) -> bool:
        """True iff x and y are in the same component."""
        return self.fn_find(x) == self.fn_find(y)

    def fn_num_components(self) -> int:
        """Number of disjoint components."""
        return self._components

def test_union_find():
    uf = UnionFind(5)
    assert uf.fn_num_components() == 5
    assert uf.fn_union(0, 1) is True
    assert uf.fn_num_components() == 4
    assert uf.fn_union(0, 1) is False
    assert uf.fn_num_components() == 4
    uf.fn_union(1, 2)
    uf.fn_union(3, 4)
    assert uf.fn_connected(0, 2) is True
    assert uf.fn_connected(0, 3) is False
    uf.fn_union(2, 3)
    assert uf.fn_connected(0, 4) is True
    assert uf.fn_num_components() == 1
    r = uf.fn_find(4)
    assert uf.fn_find(0) == r
    uf0 = UnionFind(0)
    assert uf0.fn_num_components() == 0
    uf1 = UnionFind(1)
    assert uf1.fn_find(0) == 0
    assert uf1.fn_num_components() == 1
    uf2 = UnionFind(3)
    try:
        uf2.fn_find(5)
        assert False
    ...
    for i in range(n - 1):
        ufn.fn_union(i, i + 1)
    assert ufn.fn_num_components() == 1
    assert ufn.fn_connected(0, n - 1) is True
    uf3 = UnionFind(6)
    for pair in [(0, 1), (1, 2), (2, 3), (3, 4), (4, 5)]:
        uf3.fn_union(*pair)
    root_before = uf3.fn_find(5)
    root_after = uf3.fn_find(5)
...
if __name__ == '__main__':
    ...
    for (u, v) in edges:
        uf.fn_union(u, v)
    print('Components:', uf.fn_num_components())
    print('0-2 connected:', uf.fn_connected(0, 2))
    print('0-3 connected:', uf.fn_connected(0, 3))