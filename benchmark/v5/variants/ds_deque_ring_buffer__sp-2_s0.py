"""
Double-ended queue (deque) implemented as a fixed-capacity ring buffer.

Spec:
  - RingDeque(capacity): create an empty deque with given max capacity.
  - push_front(item): add to front; raises OverflowError if full.
  - push_back(item): add to back; raises OverflowError if full.
  - pop_front() -> item: remove and return front; raises IndexError if empty.
  - pop_back() -> item: remove and return back; raises IndexError if empty.
  - peek_front() -> item: return front without removing; IndexError if empty.
  - peek_back() -> item: return back without removing; IndexError if empty.
  - __len__() -> int: current number of items.
  - __iter__(): iterate front-to-back.
  - is_empty() -> bool
  - is_full() -> bool

Capacity must be >= 1.

Suggested mutations:
  - SC-1: off-by-one in _head decrement (use capacity-1 instead of capacity)
  - SC-11: initialise _size to 1 instead of 0
  - SC-5: push_front uses _tail logic, push_back uses _head logic (swap)
  - SC-6: pop_front returns _buf[self._tail] instead of _buf[self._head]

Suggested SP transformations:
  - SP-4: rewrite push_front using modular arithmetic formula (equivalent)
  - SP-3: reorder size check and slot computation in push_back (independent)
  - SP-7: extract _advance(idx, delta) as private helper
  - SP-1: rename _head/_tail to _front/_back throughout
  - SP-8: replace fixed-size list buffer with collections.deque internally
"""
from typing import Any, Iterator

class RingDeque:
    """Double-ended queue backed by a circular buffer."""

    def __init__(self, capacity: int):
        if capacity < 1:
            raise ValueError(f'capacity must be >= 1, got {capacity}')
        self._capacity = capacity
        self._buf = [None] * capacity
        self._head = 0
        self._tail = 0
        self._size = 0

    def fn_is_empty(self) -> bool:
        return self._size == 0

    def fn_is_full(self) -> bool:
        return self._size == self._capacity

    def __len__(self) -> int:
        return self._size

    def fn_push_back(self, item: Any) -> None:
        if self.fn_is_full():
            raise OverflowError('RingDeque is full')
        self._buf[self._tail] = item
        self._tail = (self._tail + 1) % self._capacity
        self._size += 1

    def fn_push_front(self, item: Any) -> None:
        if self.fn_is_full():
            raise OverflowError('RingDeque is full')
        self._head = (self._head - 1) % self._capacity
        self._buf[self._head] = item
        self._size += 1

    def fn_pop_front(self) -> Any:
        if self.fn_is_empty():
            raise IndexError('pop_front from empty deque')
        item = self._buf[self._head]
        self._buf[self._head] = None
        self._head = (self._head + 1) % self._capacity
        self._size -= 1
        return item

    def fn_pop_back(self) -> Any:
        if self.fn_is_empty():
            raise IndexError('pop_back from empty deque')
        self._tail = (self._tail - 1) % self._capacity
        item = self._buf[self._tail]
        self._buf[self._tail] = None
        self._size -= 1
        return item

    def fn_peek_front(self) -> Any:
        if self.fn_is_empty():
            raise IndexError('peek_front on empty deque')
        return self._buf[self._head]

    def fn_peek_back(self) -> Any:
        if self.fn_is_empty():
            raise IndexError('peek_back on empty deque')
        return self._buf[(self._tail - 1) % self._capacity]

    def __iter__(self) -> Iterator:
        for i in range(self._size):
            yield self._buf[(self._head + i) % self._capacity]

def test_ring_deque():
    d = RingDeque(5)
    for v in [1, 2, 3]:
        d.fn_push_back(v)
    assert d.fn_pop_front() == 1
    assert d.fn_pop_front() == 2
    assert len(d) == 1
    d2 = RingDeque(5)
    d2.fn_push_front(10)
    d2.fn_push_front(20)
    assert d2.fn_pop_front() == 20
    assert d2.fn_pop_front() == 10
    d3 = RingDeque(4)
    for v in [1, 2, 3]:
        d3.fn_push_back(v)
    d3.fn_push_front(0)
    assert list(d3) == [0, 1, 2, 3]
    assert d3.fn_pop_back() == 3
    assert d3.fn_pop_front() == 0
    d4 = RingDeque(2)
    d4.fn_push_back(1)
    d4.fn_push_back(2)
    try:
        d4.fn_push_back(3)
        assert False
    except OverflowError:
        pass
    d5 = RingDeque(3)
    try:
        d5.fn_pop_front()
        assert False
    except IndexError:
        pass
    d6 = RingDeque(3)
    d6.fn_push_back(1)
    d6.fn_push_back(2)
    d6.fn_pop_front()
    d6.fn_push_back(3)
    d6.fn_push_back(4)
    assert list(d6) == [2, 3, 4]
    d7 = RingDeque(3)
    d7.fn_push_back(5)
    assert d7.fn_peek_front() == 5
    assert d7.fn_peek_back() == 5
    assert len(d7) == 1
    d8 = RingDeque(2)
    assert d8.fn_is_empty() is True
    assert d8.fn_is_full() is False
    d8.fn_push_back(1)
    d8.fn_push_back(2)
    assert d8.fn_is_full() is True
    d9 = RingDeque(6)
    for v in range(5):
        d9.fn_push_back(v)
    assert list(d9) == [0, 1, 2, 3, 4]
    d10 = RingDeque(1)
    d10.fn_push_back(99)
    assert d10.fn_pop_back() == 99
    assert d10.fn_is_empty()
    print('All ring-deque tests passed.')
if __name__ == '__main__':
    test_ring_deque()
    dq = RingDeque(8)
    for i in range(4):
        dq.fn_push_back(i * 10)
    dq.fn_push_front(-1)
    print('Deque:', list(dq))
    print('Front:', dq.fn_peek_front(), 'Back:', dq.fn_peek_back())