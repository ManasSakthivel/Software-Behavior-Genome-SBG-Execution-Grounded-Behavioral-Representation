...
def fn_word_ngrams(text: str, n: int) -> Counter:
...
def fn_sliding_window_max(nums: List[int], k: int) -> List[int]:
    """
...
def fn_top_k_frequent(items: Iterable, k: int) -> List:
...
def fn_group_by_key(records: List[Any], key_fn: Callable[[Any], Any]) -> Dict[Any, List[Any]]:
...
def fn_running_median(nums: List[float]) -> List[float]:
    """
...
def test_collections_usage():
    counts = fn_word_ngrams('the cat sat on the mat the cat', 2)
    assert counts['the', 'cat'] == 2
    assert counts['cat', 'sat'] == 1
    ug = fn_word_ngrams('a b a c a', 1)
    assert ug['a',] == 3
    assert fn_sliding_window_max([1, 3, -1, -3, 5, 3, 6, 7], 3) == [3, 3, 5, 5, 6, 7]
    assert fn_sliding_window_max([1, 2, 3], 1) == [1, 2, 3]
    assert fn_sliding_window_max([3, 2, 1], 3) == [3]
    assert fn_sliding_window_max([], 3) == []
    assert fn_sliding_window_max([5], 1) == [5]
    result = fn_top_k_frequent([1, 1, 1, 2, 2, 3], 2)
    assert result[0] == 1
    assert result[1] == 2
    result6 = fn_top_k_frequent([3, 3, 1, 1, 2, 2], 3)
    assert result6 == [1, 2, 3]
    records = [('a', 1), ('b', 2), ('a', 3), ('c', 4), ('b', 5)]
    groups = fn_group_by_key(records, lambda r: r[0])
    assert list(groups.keys()) == ['a', 'b', 'c']
    assert groups['a'] == [('a', 1), ('a', 3)]
    assert fn_running_median([3]) == [3.0]
    med = fn_running_median([3, 1, 2])
    assert med == [3.0, 2.0, 2.0]
    med2 = fn_running_median([1, 2])
    assert abs(med2[-1] - 1.5) < 1e-09
    med3 = fn_running_median([1, 2, 3, 4, 5])
    assert med3 == [1.0, 1.5, 2.0, 2.5, 3.0]
    assert fn_running_median([]) == []
    print('All collections-usage tests passed.')
if __name__ == '__main__':
    test_collections_usage()
    nums = [1, 3, -1, -3, 5, 3, 6, 7]
    print('Sliding window max k=3:', fn_sliding_window_max(nums, 3))
    print('Running median:', fn_running_median([5, 2, 8, 1, 9]))
    print('Top-2 frequent from [1,1,2,2,3]:', fn_top_k_frequent([1, 1, 2, 2, 3], 2))