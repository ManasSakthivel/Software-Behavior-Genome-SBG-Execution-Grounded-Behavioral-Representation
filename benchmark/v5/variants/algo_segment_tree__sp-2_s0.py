...
class SegmentTree:
    ...
    def fn_query(self, l: int, r: int) -> int:
        self._check_range(l, r)
        return self._query(1, 0, self._n - 1, l, r)

    def fn_update(self, l: int, r: int, delta: int) -> None:
        self._check_range(l, r)
        self._update(1, 0, self._n - 1, l, r, delta)

    def fn_point_query(self, i: int) -> int:
        if i < 0 or i >= self._n:
            raise IndexError(f'Index {i} out of bounds for n={self._n}')
        return self.fn_query(i, i)

    def fn_to_list(self) -> List[int]:
        return [self.fn_point_query(i) for i in range(self._n)]

def test_segment_tree():
    st = SegmentTree([1, 3, 5, 7, 9, 11])
    assert st.fn_query(0, 5) == 36
    assert st.fn_query(1, 3) == 15
    assert st.fn_query(2, 2) == 5
    assert st.fn_point_query(4) == 9
    st.fn_update(1, 3, 2)
    assert st.fn_query(1, 3) == 21
    assert st.fn_query(0, 5) == 42
    assert st.fn_point_query(0) == 1
    assert st.fn_point_query(4) == 9
    st2 = SegmentTree([0, 0, 0, 0])
    st2.fn_update(0, 3, 5)
    assert st2.fn_query(0, 3) == 20
    assert st2.fn_to_list() == [5, 5, 5, 5]
    st3 = SegmentTree([1, 2, 3, 4, 5])
    st3.fn_update(0, 4, 1)
    st3.fn_update(2, 4, 3)
    assert st3.fn_to_list() == [2, 3, 7, 8, 9]
    assert st3.fn_query(0, 4) == 29
    st_empty = SegmentTree([])
    st1 = SegmentTree([42])
    assert st1.fn_query(0, 0) == 42
    st1.fn_update(0, 0, 8)
    assert st1.fn_query(0, 0) == 50
...
if __name__ == '__main__':
    ...
    arr = [1, 3, 5, 7, 9, 11]
    st = SegmentTree(arr)
    print('Initial:', st.fn_to_list())
    st.fn_update(1, 4, 10)
    print('After update(1,4,+10):', st.fn_to_list())
    print('query(0,5):', st.fn_query(0, 5))