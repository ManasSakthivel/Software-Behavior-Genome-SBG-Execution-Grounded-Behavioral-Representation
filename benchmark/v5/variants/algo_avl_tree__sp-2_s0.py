...
class AVLTree:
    ...
    def fn_inorder(self) -> List:
        result: List = []
        _inorder(self._root, result)
        return result

    def fn_height(self) -> int:
...
def test_avl():
    ...
    for k in [5, 3, 7, 1, 4, 6, 8]:
        avl.fn_insert(k)
    assert avl.fn_inorder() == [1, 3, 4, 5, 6, 7, 8]
    assert _is_balanced(avl._root)
    assert avl.fn_contains(4) is True
    assert avl.fn_contains(9) is False
    avl.fn_insert(5)
    assert avl.fn_inorder().count(5) == 1
    avl.fn_delete(1)
    assert avl.fn_contains(1) is False
    assert _is_balanced(avl._root)
    avl.fn_delete(5)
    assert avl.fn_contains(5) is False
    assert avl.fn_inorder() == sorted(avl.fn_inorder())
    assert _is_balanced(avl._root)
    try:
        avl.fn_delete(99)
        assert False
    ...
    for k in range(1, 16):
        avl2.fn_insert(k)
    assert avl2.fn_inorder() == list(range(1, 16))
    assert _is_balanced(avl2._root)
    assert avl2.fn_height() <= 5
    avl3 = AVLTree()
    for k in range(15, 0, -1):
        avl3.fn_insert(k)
    assert avl3.fn_inorder() == list(range(1, 16))
    assert _is_balanced(avl3._root)
    empty = AVLTree()
    assert empty.fn_height() == -1
...
if __name__ == '__main__':
    ...
    for k in [10, 20, 30, 40, 50, 25]:
        avl.fn_insert(k)
    print('Inorder:', avl.fn_inorder())
    print('Height:', avl.fn_height())