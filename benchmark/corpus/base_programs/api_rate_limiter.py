...
def test_rate_limiter():
    ...
    assert tb.allow(0)   # tokens: 3 → 2
    assert tb.allow(0)   # tokens: 2 → 1
    assert tb.allow(0)   # tokens: 1 → 0
    assert not tb.allow(0)  # empty

    # Test 2: refill on next tick
    assert tb.allow(1)   # +1 token from tick advance
    assert not tb.allow(1)  # no more

    # Test 3: burst capacity respected
    tb2 = TokenBucket(rate=10, capacity=5)
    allowed = sum(1 for _ in range(10) if tb2.allow(100))
    assert allowed == 5   # capped at capacity

    # --- LeakyBucket ---
    # Test 4: filling and overflow
    lb = LeakyBucket(capacity=3, leak_rate=1)
    assert lb.allow(0)   # queue: 1
    assert lb.allow(0)   # queue: 2
    assert lb.allow(0)   # queue: 3
    assert not lb.allow(0)   # full

    # Test 5: draining
    assert lb.allow(3)   # 3 ticks pass → queue drained to 0, add 1 → queue: 1
    ...
    assert sw.allow(2)
    assert sw.allow(3)
    assert not sw.allow(4)   # 3 in window, full

    # Test 7: old requests expire
    assert sw.allow(7)   # tick 7: ticks 1,2,3 have expired (cutoff=2)

    # Test 8: remaining counts
    sw2 = SlidingWindowCounter(10, 5)
    sw2.allow(1); sw2.allow(2)
    assert sw2.remaining(3) == 3

    # Test 9: all rate limiters reject burst beyond capacity
    for Cls, args in [(TokenBucket, (0.1, 2)),
                      (LeakyBucket, (2, 0.5)),
                      (SlidingWindowCounter, (100, 2))]:
        rl = Cls(*args)
