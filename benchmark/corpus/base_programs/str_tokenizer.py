...
_PATTERNS = [
    ("FLOAT",    r"\d+\.\d*|\.\d+"),        # must come before INT
    ("INT",      r"\d+"),
    ("IDENT",    r"[a-zA-Z_]\w*"),
    ("OP",       r"\*\*|==|!=|<=|>=|[+\-*/%<>=]"),
    ("LPAREN",   r"\("),
...
def test_tokenizer():
    ...
    # Test 3: double-char operators
    toks = tokenize("x == y != z")
    ops = [(t, v) for t, v in toks if t == "OP"]
    assert ("OP", "==") in ops
    assert ("OP", "!=") in ops

    # Test 4: ** (exponentiation, higher priority than two separate stars)
    toks = tokenize("x**2")
    assert ("OP", "**") in toks

    # Test 5: parentheses and commas
    ...
    # Test 8: unknown character raises
    try:
        tokenize("a @ b")
        assert False
    ...
    # Test 9: identifier with underscores
    toks = tokenize("_my_var123")
    assert toks == [("IDENT", "_my_var123")]

    # Test 10: negative literal (minus is OP)
    toks = tokenize("-5")
    assert toks[0] == ("OP", "-")
