...
def poly_roots_bisection(p: Poly, a: float, b: float, tol: float = 1e-8) -> float:
    """
...
def test_polynomial():
    # Polynomials: p = 1 + 2x + x² = (x+1)²,  q = -1 + x
    p = [1, 2, 1]    # 1 + 2x + x²
    q = [-1, 1]      # -1 + x

    # Test 1: poly_eval (Horner's)
    assert poly_eval(p, 2) == 9.0    # 1 + 4 + 4 = 9
    assert poly_eval(p, 0) == 1.0
    assert poly_eval(q, 1) == 0.0    # root at x=1

    # Test 2: poly_add
    r = poly_add(p, q)
    # (1 + 2x + x²) + (-1 + x) = 0 + 3x + x² = [0, 3, 1]
    assert r == [0.0, 3.0, 1.0]

    # Test 3: poly_mul
    m = poly_mul([1, 1], [-1, 1])    # (1+x)(-1+x) = -1 + x² = [-1, 0, 1]
    assert m == [-1.0, 0.0, 1.0]

    # Test 4: poly_derivative
    # derivative of 1 + 2x + x² = 2 + 2x
    d = poly_derivative(p)
    assert d == [2.0, 2.0]

    # Test 5: derivative of constant
    assert poly_derivative([5]) == [0]

    # Test 6: bisection — find root of x² - 2 in [1, 2]  (≈ 1.41421356)
    p_sq = [-2, 0, 1]    # -2 + 0x + x²
    root = poly_roots_bisection(p_sq, 1, 2, tol=1e-9)
    assert abs(root - 2 ** 0.5) < 1e-6, root

    # Test 7: bisection — root of x³ - x in (-2, -0.5)  (root at -1)
    p_cubic = [0, -1, 0, 1]   # -x + x³
    root2 = poly_roots_bisection(p_cubic, -2, -0.5, tol=1e-8)
    assert abs(root2 + 1) < 1e-6, root2

    # Test 8: bisection raises if same sign
    try:
        poly_roots_bisection([1, 0, 1], -1, 1)   # x²+1 > 0 everywhere
        assert False
    except ValueError:
        pass

    # Test 9: poly_normalize removes trailing zeros
    assert poly_normalize([1, 0, 0]) == [1]
    assert poly_normalize([0]) == [0]
    assert poly_normalize([]) == [0]

    # Test 10: poly_mul by zero polynomial
