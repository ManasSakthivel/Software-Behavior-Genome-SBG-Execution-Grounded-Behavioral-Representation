# program_id: fsm_order_lifecycle
...
def test_order_lifecycle():
    ...
    # Test 4: confirm empty order raises
    empty = Order("ORD-003")
    try:
        empty.confirm()
        assert False
    ...
    # Test 6: cannot cancel DELIVERED
    order4 = Order("ORD-005")
    order4.add_item("X", 1, 1.0)
    order4.confirm(); order4.start_processing(); order4.ship("T1"); order4.deliver()
    try:
        order4.cancel()
        assert False
    except InvalidTransitionError:
    ...
    # Test 7: skip state raises
    order5 = Order("ORD-006")
    order5.add_item("Y", 1, 2.0)
    try:
        order5.ship("T2")   # must go PENDING→CONFIRMED→PROCESSING→SHIPPED
        assert False
    except InvalidTransitionError:
        pass
    ...
    o.add_item("A", 3, 10.0)
    o.add_item("B", 2, 5.0)
    assert abs(o.total_value() - 40.0) < 1e-9
...
if __name__ == "__main__":
    ...
    o.confirm(); o.start_processing(); o.ship("TRK-123"); o.deliver()
