...
def test_matrix_ops():
    ...
    # Test 2: non-square multiplication 2×3 × 3×2
    X = [[1, 2, 3], [4, 5, 6]]
    Y = [[7, 8], [9, 10], [11, 12]]
    Z = mat_mul(X, Y)
    ...
    # Test 3: incompatible shapes raise
    try:
        mat_mul([[1, 2]], [[1, 2]])
        assert False
    except ValueError:
    ...
    # Test 5: determinant 2×2
    det2 = determinant([[3, 8], [4, 6]])
    assert abs(det2 - (-14.0)) < 1e-9, det2

    # Test 6: determinant 3×3
    M = [[6, 1, 1], [4, -2, 5], [2, 8, 7]]
    det3 = determinant(M)
    assert abs(det3 - (-306.0)) < 1e-6, det3

    # Test 7: singular matrix determinant = 0
    S = [[1, 2], [2, 4]]
    assert abs(determinant(S)) < 1e-9
