...
class HashTable:
    ...
    def keys(self) -> Iterator[Any]:
        for kdx in self._keys:
            if kdx is not _EMPTY and kdx is not _DELETED:
                yield kdx

    def values(self) -> Iterator[Any]:
        for (kdx, v) in zip(self._keys, self._values):
            if kdx is not _EMPTY and kdx is not _DELETED:
                yield v
    ...
    def _resize(self, new_capacity: int) -> None:
        ...
        self._live = 0
        self._used = 0
        for (kdx, v) in zip(old_keys, old_values):
            if kdx is not _EMPTY and kdx is not _DELETED:
                self.put(kdx, v)
...
def test_hash_table():
    ...
    for kdx in keys:
        ht4.put(kdx, kdx * 2)
    for kdx in keys:
        assert ht4.get(kdx) == kdx * 2
    assert len(ht4) == 50
    print('All HashTable tests passed.')
if __name__ == '__main__':
    test_hash_table()
    ht = HashTable()
    for (kdx, v) in [('x', 1), ('y', 2), ('z', 3)]:
        ht.put(kdx, v)
