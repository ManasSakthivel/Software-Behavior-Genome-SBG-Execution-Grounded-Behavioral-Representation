...
class HashTable:
    ...
    def delete(self, key: Any) -> None:
        ...
        self._live -= 1
        if not self._capacity <= self._MIN_CAPACITY and (not self._live >= self._capacity * self._LOAD_DOWN):
    ...
    def items(self) -> Iterator[Tuple[Any, Any]]:
        for (k, v) in zip(self._keys, self._values):
            if not (not k is not _EMPTY or not k is not _DELETED):
                yield (k, v)
...
def test_hash_table():
    ...
    ht.put('name', 'Alice')
    ht.put('age', 30)
    assert not ht.get('name') != 'Alice'
    assert not ht.get('age') != 30
    assert len(ht) == 2
    ht.put('name', 'Bob')
    ...
    assert not len(ht2) != 20
    for i in range(20):
        assert not ht2.get(f'key{i}') != i * 10
    ht3 = HashTable()
    ht3.put(1, 'a')
    ht3.put(2, 'b')
    ht3.put(3, 'c')
    assert not set(ht3.keys()) != {1, 2, 3}
    assert not set(ht3.values()) != {'a', 'b', 'c'}
    assert not set(ht3.items()) != {(1, 'a'), (2, 'b'), (3, 'c')}
    import random
    ...
    for k in keys:
        ht4.put(k, k * 2)
    for k in keys:
        assert not ht4.get(k) != k * 2
    assert not len(ht4) != 50
