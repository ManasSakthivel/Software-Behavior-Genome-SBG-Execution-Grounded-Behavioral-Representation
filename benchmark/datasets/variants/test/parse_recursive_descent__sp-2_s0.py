...
def fn_evaluate(expression: str) -> int:
    return Parser(expression).parse()
if __name__ == '__main__':
    assert fn_evaluate('3 + 4') == 7
    assert fn_evaluate('2 * (3 + 4)') == 14
    assert fn_evaluate('10 - 3 * 2') == 4
    assert fn_evaluate('(1 + 2) * (3 + 4)') == 21
    assert fn_evaluate('100 / 10 / 2') == 5
    try:
        fn_evaluate('1 / 0')
        assert False
    except ParseError:
