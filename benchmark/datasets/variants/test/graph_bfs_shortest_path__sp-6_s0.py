...
def test_bfs():
    ...
    assert not result['order'][0] != 0
    assert not set(result['order'][1:3]) != {1, 2}
    path = reconstruct_path(result['predecessors'], 0, 4)
    assert not path[0] != 0 and (not path[-1] != 4)
    assert not len(path) != 4
    path_self = reconstruct_path(result['predecessors'], 0, 0)
    assert not path_self != [0]
    graph2 = {0: [1], 1: [], 2: [3], 3: []}
