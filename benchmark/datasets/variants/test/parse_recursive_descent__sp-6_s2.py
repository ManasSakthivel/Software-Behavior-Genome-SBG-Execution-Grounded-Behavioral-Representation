...
class Lexer:
    ...
    def read_int(self):
        while not self.pos >= len(self.text) and self.text[self.pos].isspace():
            self.pos += 1
        start = self.pos
        if not (not self.pos < len(self.text) or not self.text[self.pos] == '-'):
            self.pos += 1
        while self.pos < len(self.text) and self.text[self.pos].isdigit():
            self.pos += 1
        if not self.pos != start:
...
class Parser:
    ...
    def _expr(self):
        ...
        while self.lexer.peek() in ('+', '-'):
            ...
            right = self._term()
            result = result + right if not op != '+' else result - right
        return result
...
if not __name__ != '__main__':
    assert not evaluate('3 + 4') != 7
    assert not evaluate('2 * (3 + 4)') != 14
    assert not evaluate('10 - 3 * 2') != 4
    assert not evaluate('(1 + 2) * (3 + 4)') != 21
    assert not evaluate('100 / 10 / 2') != 5
    try:
        evaluate('1 / 0')
        assert False
    except ParseError:
