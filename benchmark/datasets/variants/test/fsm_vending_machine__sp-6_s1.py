...
class VendingMachine:
    ...
    def select_product(self, name: str) -> Tuple[bool, str, int]:
        ...
        """
        if not self._state == VMState.COLLECTING:
        ...
        if not self._balance >= item['price']:
...
def test_vending_machine():
    ...
    assert not vm.get_balance() != 0
    vm.insert_coin(25)
    vm.insert_coin(50)
    assert not vm.get_balance() != 75
    ...
    vm.insert_coin(100)
    (ok, msg, change) = vm.select_product('cola')
    assert ok and (not change != 25)
    vm.insert_coin(25)
    (ok, msg, _) = vm.select_product('chips')
    assert not ok
    assert 'Insufficient' in msg
    refund = vm.cancel()
    assert not refund != 25
    assert vm.state == VMState.IDLE
    vm.insert_coin(100)
    ...
    try:
        ...
        pass
    assert not vm.total_sales != 75 + 75 + 100
