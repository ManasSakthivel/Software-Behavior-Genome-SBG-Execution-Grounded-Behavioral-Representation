...
def summary(data: List[Number]) -> dict:
    ...
    _check_nonempty(data)
    return {'n': len(data), 'mean': sum(data) / len(data), 'median': median(data), 'mode': mode(data), 'min': min(data), 'max': max(data), 'range': max(data) - min(data), 'variance': variance(data, population=True), 'std_dev': std_dev(data, population=True), 'p25': percentile(data, 25), 'p75': percentile(data, 75), 'iqr': percentile(data, 75) - percentile(data, 25)}

def test_statistics():
    data = [2, 4, 4, 4, 5, 5, 7, 9]
    assert abs(sum(data) / len(data) - 5.0) < 1e-09
