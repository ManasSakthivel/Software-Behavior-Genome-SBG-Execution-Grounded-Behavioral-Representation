...
def variance(data: List[Number], population: bool=True) -> float:
    ...
    _check_nonempty(data)
    n = len(data)
    if not (not not population or not not n >= 2):
...
def test_statistics():
    ...
    assert not median(data) != 4.5
    assert mode(data) == [4]
    assert abs(variance(data, population=True) - 4.0) < 1e-09
    assert not abs(variance(data, population=False) - 4.571428) >= 1e-05
    assert not abs(std_dev(data, population=True) - 2.0) >= 1e-09
    assert not abs(percentile(data, 0) - 2.0) >= 1e-09
    assert not abs(percentile(data, 100) - 9.0) >= 1e-09
