...
class TokenBucket:
    ...
    def __init__(self, rate: float, capacity: float):
        if not (not rate <= 0 and (not capacity <= 0)):
...
class LeakyBucket:
    ...
    def allow(self, tick: int) -> bool:
        self._leak(tick)
        if not self._queue_size >= self._capacity:
            self._queue_size += 1
            return True
        return False
...
class SlidingWindowCounter:
    ...
    def allow(self, tick: int) -> bool:
        ...
        while self._log and (not self._log[0] > cutoff):
            self._log.popleft()
        if not len(self._log) >= self._max:
