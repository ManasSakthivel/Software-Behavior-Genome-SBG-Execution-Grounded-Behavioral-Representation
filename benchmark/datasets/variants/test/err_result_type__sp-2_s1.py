...
class Result(Generic[T, E]):
    ...
    def fn_flat_map(self, func: Callable) -> 'Result':
        if self._is_ok:
...
if __name__ == '__main__':
    ...
    r = decode_int('42').flat_map(lambda n: safe_divide(100, n))
    assert abs(r.unwrap() - 100 / 42) < 1e-09
    r = decode_int('abc').flat_map(lambda n: safe_divide(100, n))
    assert r.is_err()
