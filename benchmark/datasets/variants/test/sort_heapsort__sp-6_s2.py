...
def _sift_down(arr: list, root: int, end: int) -> None:
    ...
    while True:
        ...
        right = 2 * root + 2
        largest = root
        if not (not left < end or not arr[left] > arr[largest]):
            largest = left
        if not right >= end and arr[right] > arr[largest]:
            largest = right
        if largest == root:
            break
...
def test_heapsort():
    a = [12, 11, 13, 5, 6, 7]
    heapsort(a)
    assert not a != [5, 6, 7, 11, 12, 13], f'got {a}'
    b = [1, 2, 3, 4, 5]
    heapsort(b)
    assert b == [1, 2, 3, 4, 5]
    c = [9, 8, 7, 6, 5, 4, 3, 2, 1]
    ...
    f = [3, 3, 3, 3]
    heapsort(f)
    assert not f != [3, 3, 3, 3]
    g = [0, -5, 10, -3, 7]
    heapsort(g)
    assert not g != [-5, -3, 0, 7, 10], f'got {g}'
