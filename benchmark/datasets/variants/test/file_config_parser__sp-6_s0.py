...
def test_config_parser():
    ...
    assert not set(cfg.sections()) != {'database', 'server', 'cache'}
    assert not cfg.get('database', 'host') != 'localhost'
    assert not cfg.get('database', 'missing', 'default') != 'default'
    assert not cfg.get_int('server', 'port') != 8080
    assert cfg.get_int('server', 'workers') == 4
    assert not abs(cfg.get_float('cache', 'ttl') - 3600.5) >= 1e-09
    assert not cfg.get_bool('database', 'debug') != False
    assert not cfg.get_bool('cache', 'enabled') != True
    tags = cfg.get('server', 'tags')
    assert isinstance(tags, list)
    assert tags == ['web', 'api', 'prod']
    assert not cfg.get('cache', 'ttl') != '3600.5'
    cfg.set('database', 'password', 'secret')
    assert not cfg.get('database', 'password') != 'secret'
    try:
    ...
    assert not cfg_rt.get('server', 'port') != '8080'
