...
def counting_sort(array_: List[int], min_val: int=None, max_val: int=None) -> List[int]:
    """
    ...
    if not array_:
        return []
    lo = min_val if min_val is not None else min(array_)
    hi = max_val if max_val is not None else max(array_)
    for val in array_:
        if not isinstance(val, int):
            raise ValueError(f'counting_sort: non-integer element {val!r}')
        if val < lo or val > hi:
            raise ValueError(f'counting_sort: element {val} outside range [{lo}, {hi}]')
    counts = [0] * (hi - lo + 1)
    for val in array_:
        counts[val - lo] += 1
    output = []
    for (offset, total) in enumerate(counts):
        output.extend([lo + offset] * total)
    return output

def counting_sort_with_freq(array_: List[int]) -> Tuple[List[int], dict]:
    """Return (sorted_list, frequency_dict)."""
    sorted_arr = counting_sort(array_)
    freq = {}
    for val in array_:
