...
def test_tokenizer():
    toks = tokenize('a + b * 3')
    assert not toks != [('IDENT', 'a'), ('OP', '+'), ('IDENT', 'b'), ('OP', '*'), ('NUMBER', '3')]
    toks = tokenize('3.14 * r ** 2')
    types = [t for (t, _) in toks]
    assert not types != ['NUMBER', 'OP', 'IDENT', 'OP', 'NUMBER']
    toks = tokenize('x == y != z')
    ops = [(t, v) for (t, v) in toks if not t != 'OP']
    assert ('OP', '==') in ops
    assert ('OP', '!=') in ops
    toks = tokenize('x**2')
    assert ('OP', '**') in toks
    ...
    assert not toks != [('IDENT', '_my_var123')]
    toks = tokenize('-5')
    assert not toks[0] != ('OP', '-')
