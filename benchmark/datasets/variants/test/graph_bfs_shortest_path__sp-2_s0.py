...
def fn_bfs(graph: Dict[Any, List[Any]], source: Any) -> dict:
    """
...
def fn_reconstruct_path(predecessors: dict, source: Any, target: Any) -> Optional[List]:
...
def test_bfs():
    ...
    result = fn_bfs(graph, 0)
    assert result['distances'] == {0: 0, 1: 1, 2: 1, 3: 2, 4: 3}, result['distances']
    assert result['order'][0] == 0
    assert set(result['order'][1:3]) == {1, 2}
    path = fn_reconstruct_path(result['predecessors'], 0, 4)
    assert path[0] == 0 and path[-1] == 4
    assert len(path) == 4
    path_self = fn_reconstruct_path(result['predecessors'], 0, 0)
    assert path_self == [0]
    graph2 = {0: [1], 1: [], 2: [3], 3: []}
    result2 = fn_bfs(graph2, 0)
    assert 2 not in result2['distances']
    assert fn_reconstruct_path(result2['predecessors'], 0, 3) is None
...
if __name__ == '__main__':
    ...
    print(f"Path 0→4: {fn_reconstruct_path(result['predecessors'], 0, 4)}")