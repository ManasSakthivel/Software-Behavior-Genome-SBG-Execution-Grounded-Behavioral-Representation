...
def fn__check_nonempty(data: list) -> None:
    if not data:
        raise StatError('data must be non-empty')

def fn_mean(data: List[Number]) -> float:
    """Return arithmetic mean."""
    fn__check_nonempty(data)
    return sum(data) / len(data)

def fn_median(data: List[Number]) -> float:
...
def fn_mode(data: List[Number]) -> List[Number]:
...
def fn_variance(data: List[Number], population: bool=True) -> float:
...
def fn_std_dev(data: List[Number], population: bool=True) -> float:
    """Return standard deviation (population or sample)."""
    return math.sqrt(fn_variance(data, population=population))

def fn_percentile(data: List[Number], p: float) -> float:
    """
...
def summary(data: List[Number]) -> dict:
    ...
    fn__check_nonempty(data)
    return {'n': len(data), 'mean': fn_mean(data), 'median': fn_median(data), 'mode': fn_mode(data), 'min': min(data), 'max': max(data), 'range': max(data) - min(data), 'variance': fn_variance(data, population=True), 'std_dev': fn_std_dev(data, population=True), 'p25': fn_percentile(data, 25), 'p75': fn_percentile(data, 75), 'iqr': fn_percentile(data, 75) - fn_percentile(data, 25)}

def test_statistics():
    data = [2, 4, 4, 4, 5, 5, 7, 9]
    assert abs(fn_mean(data) - 5.0) < 1e-09
    assert fn_median(data) == 4.5
    assert fn_mode(data) == [4]
    assert abs(fn_variance(data, population=True) - 4.0) < 1e-09
    assert abs(fn_variance(data, population=False) - 4.571428) < 1e-05
    assert abs(fn_std_dev(data, population=True) - 2.0) < 1e-09
    assert abs(fn_percentile(data, 0) - 2.0) < 1e-09
    assert abs(fn_percentile(data, 100) - 9.0) < 1e-09
    assert fn_mode([1, 1, 2, 2, 3]) == [1, 2]
    for fn in [mean, median, mode, lambda d: fn_variance(d), lambda d: fn_std_dev(d)]:
        try:
            fn([])
            assert False
        except StatError:
            pass
    try:
        fn_variance([5], population=False)
        assert False
    except ValueError:
        pass
    try:
        fn_percentile(data, 101)
        assert False
    except ValueError:
        pass
    s = summary(data)
