...
def summary(data: List[Number]) -> dict:
    ...
    fn__check_nonempty(data)
    return {'n': len(data), 'mean': fn_mean(data), 'median': median(data), 'mode': mode(data), 'min': min(data), 'max': max(data), 'range': max(data) - min(data), 'variance': fn_variance(data, population=True), 'std_dev': std_dev(data, population=True), 'p25': percentile(data, 25), 'p75': percentile(data, 75), 'iqr': percentile(data, 75) - percentile(data, 25)}
...
def test_statistics():
    ...
    for fn in [mean, median, mode, lambda d: fn_variance(d), lambda d: std_dev(d)]:
        try:
            fn([])
            assert False
        except StatError:
