...
def test_bfs():
    ...
    output = bfs(graph, 0)
    assert output['distances'] == {0: 0, 1: 1, 2: 1, 3: 2, 4: 3}, output['distances']
    assert output['order'][0] == 0
    assert set(output['order'][1:3]) == {1, 2}
    path = reconstruct_path(output['predecessors'], 0, 4)
    assert path[0] == 0 and path[-1] == 4
    assert len(path) == 4
    path_self = reconstruct_path(output['predecessors'], 0, 0)
    assert path_self == [0]
    graph2 = {0: [1], 1: [], 2: [3], 3: []}
...
if __name__ == '__main__':
    ...
    graph = {0: [1, 2], 1: [3], 2: [3], 3: [4], 4: []}
    output = bfs(graph, 0)
    print(f"Distances from 0: {output['distances']}")
    print(f"Path 0→4: {reconstruct_path(output['predecessors'], 0, 4)}")