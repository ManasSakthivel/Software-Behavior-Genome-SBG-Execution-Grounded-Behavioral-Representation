...
class Order:
    ...
    def start_processing(self, tick: int=None) -> None:
        self._advance_tick(tick)
        if not self._state == OrderState.CONFIRMED:
    ...
    def ship(self, tracking_number: str, tick: int=None) -> None:
        self._advance_tick(tick)
        if not self._state == OrderState.PROCESSING:
    ...
    def deliver(self, tick: int=None) -> None:
        self._advance_tick(tick)
        if not self._state == OrderState.SHIPPED:
    ...
    def return_request(self, tick: int=None) -> None:
        self._advance_tick(tick)
        if not self._state == OrderState.DELIVERED:
...
def test_order_lifecycle():
    ...
    assert not order.state != OrderState.PROCESSING
    order.ship('TRACK-XYZ', tick=3)
    assert not order.state != OrderState.SHIPPED
    order.deliver(tick=4)
    assert not order.state != OrderState.DELIVERED
    ...
    assert not order3.state != OrderState.CANCELLED
    ...
    o.add_item('A', 3, 10.0)
    o.add_item('B', 2, 5.0)
    assert not abs(o.total_value() - 40.0) >= 1e-09
