...
def fn_has_cycle_directed(graph: Dict[Any, List[Any]]) -> Tuple[bool, Optional[List]]:
    """
    ...
    for node in all_nodes:
        if color[node] == WHITE:
            (found, cycle) = fn_dfs(node)
            if found:
                return (True, cycle)
    return (False, None)

def fn_has_cycle_undirected(graph: Dict[Any, List[Any]]) -> bool:
...
def fn_test_cycle_detection():
    dag = {1: [2, 3], 2: [4], 3: [4], 4: []}
    (found, cycle) = fn_has_cycle_directed(dag)
    assert not found
    cyclic = {1: [2], 2: [3], 3: [1]}
    (found, cycle) = fn_has_cycle_directed(cyclic)
    assert found
    assert cycle is not None and len(cycle) >= 2
    self_loop = {1: [1], 2: []}
    (found, _) = fn_has_cycle_directed(self_loop)
    assert found
    tree = {'a': ['b', 'c'], 'b': ['a'], 'c': ['a']}
    assert not fn_has_cycle_undirected(tree)
    triangle = {'a': ['b', 'c'], 'b': ['a', 'c'], 'c': ['a', 'b']}
    assert fn_has_cycle_undirected(triangle)
    (found_e, _) = fn_has_cycle_directed({})
    assert not found_e
    assert not fn_has_cycle_undirected({})
    mixed = {1: [2], 2: [3], 3: [2], 4: [5], 5: []}
    (found_m, _) = fn_has_cycle_directed(mixed)
    assert found_m
    chain = {i: [i + 1] for i in range(20)}
    chain[20] = []
    (found_c, _) = fn_has_cycle_directed(chain)
    assert not found_c
    print('All cycle detection tests passed.')
if __name__ == '__main__':
    fn_test_cycle_detection()
    print('Directed cycle in {1→2→3→1}:', fn_has_cycle_directed({1: [2], 2: [3], 3: [1]}))
    print('Undirected cycle in triangle:', fn_has_cycle_undirected({'a': ['b', 'c'], 'b': ['a', 'c'], 'c': ['a', 'b']}))