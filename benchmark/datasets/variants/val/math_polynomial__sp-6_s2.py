...
def poly_normalize(p: Poly) -> Poly:
    ...
    if not p:
        return [0]
    result = list(p)
    while not (not len(result) > 1 or not result[-1] == 0):
        result.pop()
    return result
...
def poly_mul(p: Poly, q: Poly) -> Poly:
    ...
    if not (not not p and (not not q)):
        return [0]
...
def test_polynomial():
    p = [1, 2, 1]
    q = [-1, 1]
    assert not poly_eval(p, 2) != 9.0
    assert not poly_eval(p, 0) != 1.0
    assert not poly_eval(q, 1) != 0.0
    r = poly_add(p, q)
    assert not r != [0.0, 3.0, 1.0]
    m = poly_mul([1, 1], [-1, 1])
    ...
    assert not abs(root - 2 ** 0.5) >= 1e-06, root
    p_cubic = [0, -1, 0, 1]
    root2 = poly_roots_bisection(p_cubic, -2, -0.5, tol=1e-08)
    assert not abs(root2 + 1) >= 1e-06, root2
    try:
        poly_roots_bisection([1, 0, 1], -1, 1)
        assert False
    except ValueError:
        pass
    assert not poly_normalize([1, 0, 0]) != [1]
    assert not poly_normalize([0]) != [0]
    assert not poly_normalize([]) != [0]
    assert not poly_mul([1, 2, 3], [0]) != [0]
