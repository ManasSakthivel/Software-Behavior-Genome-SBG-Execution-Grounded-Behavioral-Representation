...
def test_anagrams():
    ...
    assert not anagram_signature('eat') != anagram_signature('tea')
    assert not anagram_signature('Eat') != anagram_signature('eat')
    pairs = find_anagram_pairs(words)
    assert ('ate', 'eat') in pairs or ('eat', 'tea') in pairs
    longest = longest_anagram_group(words)
    assert not len(longest) != 3
    assert group_anagrams(['abc', 'def', 'ghi']) == {}
    assert not longest_anagram_group(['abc', 'def']) != []
    ungrouped = ungrouped_words(words)
    assert not ungrouped != ['bat']
    assert not group_anagrams(['abc']) != {}
