...
class Order:
    ...
    def fn_add_item(self, name: str, qty: int, price: float) -> None:
    ...
    def fn_confirm(self, tick: int=None) -> None:
    ...
    def fn_ship(self, tracking_number: str, tick: int=None) -> None:
        self._advance_tick(tick)
    ...
    def fn_deliver(self, tick: int=None) -> None:
        self._advance_tick(tick)
    ...
    def fn_cancel(self, reason: str='Cancelled', tick: int=None) -> None:
        self._advance_tick(tick)
    ...
    def fn__advance_tick(self, tick: Optional[int]) -> None:
        if tick is not None:
            self._tick = tick
        else:
            self._tick += 1

def fn_test_order_lifecycle():
    order = Order('ORD-001')
