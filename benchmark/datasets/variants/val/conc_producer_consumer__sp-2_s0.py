...
def execute_pipeline(producer_fn: Callable, consumer_fn: Callable, n_producers=2, n_consumers=2, buffer_size=10) -> List:
    """
...
if __name__ == '__main__':
    ...
    def fn_prod():
        for x in range(20):
            yield x
    results = execute_pipeline(prod, lambda x: x * 2, n_producers=1, n_consumers=2, buffer_size=5)
    assert sorted(results) == [x * 2 for x in range(20)], f'Got {sorted(results)}'
    results2 = execute_pipeline(prod, lambda x: x if x % 2 == 0 else None, n_producers=1, n_consumers=1, buffer_size=4)
    assert sorted(results2) == list(range(0, 20, 2))
    results3 = execute_pipeline(lambda : [], lambda x: x, n_producers=1, n_consumers=1, buffer_size=2)
    assert results3 == []
