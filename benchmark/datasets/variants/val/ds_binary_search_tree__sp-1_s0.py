...
class BinarySearchTree:
    ...
    def inorder(self) -> List[Tuple]:
        output = []
        self._inorder(self._root, output)
        return output
...
def test_bst():
    ...
    for (kdx, v) in [(5, 'e'), (3, 'c'), (7, 'g'), (1, 'a'), (4, 'd')]:
        bst.insert(kdx, v)
    assert bst.search(3) == 'c'
    assert bst.search(7) == 'g'
    assert len(bst) == 5
    keys = [kdx for (kdx, v) in bst.inorder()]
    assert keys == [1, 3, 4, 5, 7]
    bst.insert(5, 'EE')
    assert bst.search(5) == 'EE'
    assert len(bst) == 5
    bst.delete(1)
    assert 1 not in [kdx for (kdx, _) in bst.inorder()]
    assert len(bst) == 4
    bst.delete(3)
    keys = [kdx for (kdx, _) in bst.inorder()]
    assert keys == [4, 5, 7]
    bst2 = BinarySearchTree()
    for kdx in [5, 3, 7, 1, 4, 6, 8]:
        bst2.insert(kdx, kdx)
    bst2.delete(5)
    keys2 = [kdx for (kdx, _) in bst2.inorder()]
...
if __name__ == '__main__':
    test_bst()
    bst = BinarySearchTree()
    for (kdx, v) in [(5, 'e'), (3, 'c'), (7, 'g'), (1, 'a'), (4, 'd'), (6, 'f'), (8, 'h')]:
        bst.insert(kdx, v)
