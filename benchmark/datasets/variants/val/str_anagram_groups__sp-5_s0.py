...
def ungrouped_words(words: List[str]) -> List[str]:
    ...
    return list((w for w in words if w not in grouped))

def test_anagrams():
    words = ['eat', 'tea', 'tan', 'ate', 'nat', 'bat']
    groups = group_anagrams(words)
    assert {'eat', 'tea', 'ate'} in list((set(v) for v in groups.values()))
    assert {'tan', 'nat'} in list((set(v) for v in groups.values()))
