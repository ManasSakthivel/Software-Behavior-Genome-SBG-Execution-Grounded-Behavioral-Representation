...
class StreamWordCounter:
    ...
    def fn_feed(self, line: str) -> None:
    ...
    def fn_top_n(self, n: int) -> List[tuple]:
...
def handle_stream(lines: Iterable[str], chunk_size: Optional[int]=None) -> 'StreamWordCounter':
...
def test_stream_word_counter():
    counter = handle_stream(SAMPLE_TEXT)
    ...
    counter_chunked = handle_stream(SAMPLE_TEXT, chunk_size=2)
    assert counter_chunked.word_count == counter.word_count
    assert counter_chunked.get('the') == counter.get('the')
    half1 = handle_stream(SAMPLE_TEXT[:2])
    half2 = handle_stream(SAMPLE_TEXT[2:])
