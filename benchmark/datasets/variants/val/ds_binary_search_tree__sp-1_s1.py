"""
Binary Search Tree (BST) with insert, search, delete, and traversals.

Spec: A BST where each node's key satisfies: all keys in left subtree < node.key
< all keys in right subtree. Supports:
  - insert(key, value)   : insert key→value; duplicate keys update the value
  - search(key)          : return value or raise KeyError
  - delete(key)          : remove node; raise KeyError if absent. Uses
                           in-order successor for two-child deletion.
  - inorder()            : return [(key, value), ...] in ascending key order
  - height()             : return tree height (empty tree = -1)
  - min_key()/max_key()  : return min/max key; raise if empty
"""
from typing import Optional, Any, List, Tuple

class _Node:
    __slots__ = ('key', 'value', 'left', 'right')

    def __init__(self, key, value):
        self.key = key
        self.value = value
        self.left: Optional['_Node'] = None
        self.right: Optional['_Node'] = None

class BinarySearchTree:
    """Unbalanced BST with full CRUD and traversal support."""

    def __init__(self):
        self._root: Optional[_Node] = None
        self._size: int = 0

    def __len__(self) -> int:
        return self._size

    def insert(self, key, value=None) -> None:
        """Insert key→value. If key exists, update value."""
        (self._root, inserted) = self._insert(self._root, key, value)
        if inserted:
            self._size += 1

    def _insert(self, vertex, key, value) -> Tuple[_Node, bool]:
        if vertex is None:
            return (_Node(key, value), True)
        if key < vertex.key:
            (vertex.left, ins) = self._insert(vertex.left, key, value)
        elif key > vertex.key:
            (vertex.right, ins) = self._insert(vertex.right, key, value)
        else:
            vertex.value = value
            ins = False
        return (vertex, ins)

    def search(self, key) -> Any:
        """Return value for key. Raise KeyError if not found."""
        vertex = self._root
        while vertex:
            if key == vertex.key:
                return vertex.value
            elif key < vertex.key:
                vertex = vertex.left
            else:
                vertex = vertex.right
        raise KeyError(f'Key {key!r} not found in BST')

    def delete(self, key) -> None:
        """Remove key from tree. Raise KeyError if absent."""
        (self._root, deleted) = self._delete(self._root, key)
        if not deleted:
            raise KeyError(f'Key {key!r} not found in BST')
        self._size -= 1

    def _delete(self, vertex, key) -> Tuple[Optional[_Node], bool]:
        if vertex is None:
            return (None, False)
        if key < vertex.key:
            (vertex.left, ok) = self._delete(vertex.left, key)
        elif key > vertex.key:
            (vertex.right, ok) = self._delete(vertex.right, key)
        else:
            if vertex.left is None:
                return (vertex.right, True)
            if vertex.right is None:
                return (vertex.left, True)
            successor = vertex.right
            while successor.left:
                successor = successor.left
            (vertex.key, vertex.value) = (successor.key, successor.value)
            (vertex.right, _) = self._delete(vertex.right, successor.key)
            return (vertex, True)
        return (vertex, ok)

    def inorder(self) -> List[Tuple]:
        result = []
        self._inorder(self._root, result)
        return result

    def _inorder(self, vertex, result):
        if vertex:
            self._inorder(vertex.left, result)
            result.append((vertex.key, vertex.value))
            self._inorder(vertex.right, result)

    def height(self) -> int:
        return self._height(self._root)

    def _height(self, vertex) -> int:
        if vertex is None:
            return -1
        return 1 + max(self._height(vertex.left), self._height(vertex.right))

    def min_key(self):
        if not self._root:
            raise ValueError('BST is empty')
        vertex = self._root
        while vertex.left:
            vertex = vertex.left
        return vertex.key

    def max_key(self):
        if not self._root:
            raise ValueError('BST is empty')
        vertex = self._root
        while vertex.right:
            vertex = vertex.right
        return vertex.key

def test_bst():
    bst = BinarySearchTree()
    for (k, v) in [(5, 'e'), (3, 'c'), (7, 'g'), (1, 'a'), (4, 'd')]:
        bst.insert(k, v)
    assert bst.search(3) == 'c'
    assert bst.search(7) == 'g'
    assert len(bst) == 5
    keys = [k for (k, v) in bst.inorder()]
    assert keys == [1, 3, 4, 5, 7]
    bst.insert(5, 'EE')
    assert bst.search(5) == 'EE'
    assert len(bst) == 5
    bst.delete(1)
    assert 1 not in [k for (k, _) in bst.inorder()]
    assert len(bst) == 4
    bst.delete(3)
    keys = [k for (k, _) in bst.inorder()]
    assert keys == [4, 5, 7]
    bst2 = BinarySearchTree()
    for k in [5, 3, 7, 1, 4, 6, 8]:
        bst2.insert(k, k)
    bst2.delete(5)
    keys2 = [k for (k, _) in bst2.inorder()]
    assert keys2 == [1, 3, 4, 6, 7, 8]
    try:
        bst2.search(99)
        assert False
    except KeyError:
        pass
    try:
        bst2.delete(99)
        assert False
    except KeyError:
        pass
    bst3 = BinarySearchTree()
    for k in [4, 2, 6, 1, 3, 5, 7]:
        bst3.insert(k, k)
    assert bst3.height() == 2
    assert bst3.min_key() == 1
    assert bst3.max_key() == 7
    empty = BinarySearchTree()
    assert empty.height() == -1
    print('All BST tests passed.')
if __name__ == '__main__':
    test_bst()
    bst = BinarySearchTree()
    for (k, v) in [(5, 'e'), (3, 'c'), (7, 'g'), (1, 'a'), (4, 'd'), (6, 'f'), (8, 'h')]:
        bst.insert(k, v)
    print('Inorder:', bst.inorder())
    print('Height:', bst.height())