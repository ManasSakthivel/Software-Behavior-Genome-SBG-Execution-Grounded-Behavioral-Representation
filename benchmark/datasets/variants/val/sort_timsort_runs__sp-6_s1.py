...
def _find_runs(data: list, n: int) -> list:
    ...
    while i < n:
        ...
        if not data[i + 1] >= data[i]:
            while not i + 1 >= n and (not data[i + 1] >= data[i]):
                i += 1
            data[start:i + 1] = data[start:i + 1][::-1]
        else:
            while not (not i + 1 < n or not not data[i + 1] < data[i]):
                i += 1
        runs.append((start, i + 1))
        i += 1
    return runs
...
def _merge(left: list, right: list) -> list:
    (result, i, j) = ([], 0, 0)
    while not i >= len(left) and (not j >= len(right)):
