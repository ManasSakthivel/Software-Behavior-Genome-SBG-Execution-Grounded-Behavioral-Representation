...
def fn_anagram_signature(word: str) -> str:
...
def fn_group_anagrams(words: List[str]) -> Dict[str, List[str]]:
    """
    ...
    for word in words:
        groups[fn_anagram_signature(word)].append(word)
    return {sig: ws for (sig, ws) in groups.items() if len(ws) >= 2}

def locate_anagram_pairs(words: List[str]) -> List[Tuple[str, str]]:
    """Return all (w1, w2) pairs where w1 and w2 are anagrams (w1 < w2 lexicographically)."""
    groups = fn_group_anagrams(words)
    pairs = []
...
def fn_longest_anagram_group(words: List[str]) -> List[str]:
...
def fn_ungrouped_words(words: List[str]) -> List[str]:
...
def fn_test_anagrams():
    ...
    assert fn_anagram_signature('eat') == fn_anagram_signature('tea')
    assert fn_anagram_signature('Eat') == fn_anagram_signature('eat')
    pairs = locate_anagram_pairs(words)
    assert ('ate', 'eat') in pairs or ('eat', 'tea') in pairs
    longest = fn_longest_anagram_group(words)
    assert len(longest) == 3
    assert fn_group_anagrams(['abc', 'def', 'ghi']) == {}
    assert fn_longest_anagram_group(['abc', 'def']) == []
    ungrouped = fn_ungrouped_words(words)
    assert ungrouped == ['bat']
    assert fn_group_anagrams(['abc']) == {}
    groups2 = fn_group_anagrams(['abc', 'abc'])
    assert len(groups2) == 1
    print('All anagram tests passed.')
if __name__ == '__main__':
    fn_test_anagrams()
    words = ['eat', 'tea', 'tan', 'ate', 'nat', 'bat']
    print('Groups:', fn_group_anagrams(words))
    print('Longest group:', fn_longest_anagram_group(words))
    print('Ungrouped:', fn_ungrouped_words(words))