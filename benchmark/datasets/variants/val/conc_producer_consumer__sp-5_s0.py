    ...
    def consumer_worker():
        while True:
            ...
            if out is not None:
                with results_lock:
                    results.append(out)
    producers = list((threading.Thread(target=producer_worker) for _ in range(n_producers)))
    consumers = list((threading.Thread(target=consumer_worker) for _ in range(n_consumers)))
    for t in consumers:
        t.start()
    for t in producers:
        t.start()
...
if __name__ == '__main__':
    ...
    assert sorted(results) == list((x * 2 for x in range(20))), f'Got {sorted(results)}'
