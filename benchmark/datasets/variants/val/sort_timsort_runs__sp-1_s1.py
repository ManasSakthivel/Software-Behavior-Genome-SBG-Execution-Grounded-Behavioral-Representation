...
def timsort_runs(arr: list) -> list:
    ...
    payload = list(arr)
    n = len(payload)
    runs = _find_runs(payload, n)
    _extend_runs(payload, runs, n)
    result = payload[runs[0][0]:runs[0][1]]
    for (start, end) in runs[1:]:
        result = _merge(result, payload[start:end])
    return result

def _find_runs(payload: list, n: int) -> list:
    ...
    while i < n:
        ...
        if payload[i + 1] < payload[i]:
            while i + 1 < n and payload[i + 1] < payload[i]:
                i += 1
            payload[start:i + 1] = payload[start:i + 1][::-1]
        else:
            while i + 1 < n and payload[i + 1] >= payload[i]:
                i += 1
        runs.append((start, i + 1))
...
def _extend_runs(payload: list, runs: list, n: int) -> None:
    """Extend short runs to MINRUN using binary insertion sort."""
    for (idx, (start, end)) in enumerate(runs):
        if end - start < MINRUN:
            new_end = min(start + MINRUN, n)
            _binary_insertion_sort(payload, start, new_end)
            runs[idx] = (start, new_end)

def _binary_insertion_sort(payload: list, lo: int, hi: int) -> None:
    for i in range(lo + 1, hi):
        key = payload[i]
        pos = _binary_search_pos(payload, key, lo, i)
        payload[pos + 1:i + 1] = payload[pos:i]
        payload[pos] = key

def _binary_search_pos(payload: list, key, lo: int, hi: int) -> int:
...
def test_timsort_runs():
    ...
    payload = [1, 3, 5, 4, 2, 6, 8, 7]
    assert timsort_runs(payload) == sorted(payload)
    assert timsort_runs([]) == []
