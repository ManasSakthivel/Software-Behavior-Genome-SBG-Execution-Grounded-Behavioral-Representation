...
def fn__extract_words(line: str) -> List[str]:
...
class StreamWordCounter:
    ...
    def fn_feed(self, line: str) -> None:
        ...
        for word in fn__extract_words(line):
            self._counts[word] += 1
    ...
    @property
    def fn_line_count(self) -> int:
        return self._line_count

    @property
    def fn_char_count(self) -> int:
        return self._char_count

    @property
    def fn_unique_words(self) -> set:
...
def fn_merge_counters(counters: List['StreamWordCounter']) -> 'StreamWordCounter':
...
def fn_test_stream_word_counter():
    ...
    merged = fn_merge_counters([half1, half2])
