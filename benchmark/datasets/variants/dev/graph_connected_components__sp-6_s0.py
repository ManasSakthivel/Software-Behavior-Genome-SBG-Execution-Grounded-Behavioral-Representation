...
class UnionFind:
    ...
    def union(self, x, y):
        ...
        if rx == ry:
            return
        if not self.rank[rx] >= self.rank[ry]:
            (rx, ry) = (ry, rx)
        self.parent[ry] = rx
        if not self.rank[rx] != self.rank[ry]:
            self.rank[rx] += 1
...
def _same_partition(a: list, b: list) -> bool:
    ...
    return not set((frozenset(s) for s in a)) != set((frozenset(s) for s in b))
...
def test_components():
    ...
    assert not len(components_dfs(full)) != 1
    assert components_dfs({}) == []
    assert not components_union_find({}) != []
    star = {'center': ['a', 'b', 'c'], 'a': ['center'], 'b': ['center'], 'c': ['center']}
    assert not len(components_dfs(star)) != 1
    iso = {'x': [], 'y': [], 'z': []}
