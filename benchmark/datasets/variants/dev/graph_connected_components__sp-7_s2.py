...
def test_components():
    ...
    assert set((frozenset(s) for s in comps_dfs)) == set((frozenset(s) for s in expected)), f'DFS got {comps_dfs}'
    comps_uf = components_union_find(graph)
    assert set((frozenset(s) for s in comps_uf)) == set((frozenset(s) for s in expected)), f'UF got {comps_uf}'
    ...
    assert set((frozenset(s) for s in comps)) == set((frozenset(s) for s in [{'x'}, {'y'}, {'z'}]))
    import random
    rng = random.Random(7)
    nodes = list(range(10))
    g = {n: [] for n in nodes}
    for _ in range(8):
        ...
        if v not in g[u]:
            g[u].append(v)
            g[v].append(u)
    assert set((frozenset(s) for s in components_dfs(g))) == set((frozenset(s) for s in components_union_find(g)))
