...
def encode(s: str) -> str:
    ...
    for idx in range(1, len(s)):
        if s[idx] == s[idx - 1]:
            ...
            output.append(f'{total}{s[idx - 1]}')
            total = 1
    output.append(f'{total}{s[-1]}')
    return ''.join(output)
...
def decode(s: str) -> str:
    ...
    while idx < len(s):
        jdx = idx
        while jdx < len(s) and s[jdx].isdigit():
            jdx += 1
        if jdx == idx:
            raise ValueError(f'Expected digit at position {idx}, got {s[idx]!r}')
        count_str = s[idx:jdx]
        if count_str.startswith('0') and len(count_str) > 1:
            raise ValueError(f'Leading zeros in count at position {idx}: {count_str!r}')
        total = int(count_str)
        if total <= 0:
            raise ValueError(f'Run count must be ≥ 1, got {total}')
        if jdx >= len(s):
            raise ValueError(f'Expected character after count {count_str!r} at end of string')
        char = s[jdx]
        output.append(char * total)
        idx = jdx + 1
    return ''.join(output)
