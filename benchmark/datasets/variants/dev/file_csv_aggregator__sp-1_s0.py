...
def load_csv(text: str) -> DataFrame:
    ...
    for (rownum, row) in enumerate(rows[1:], start=2):
        ...
        for (h, value) in zip(headers, row):
            df[h].append(_coerce(value.strip()))
    return df
...
def filter_rows(df: DataFrame, col: str, predicate: Callable[[Any], bool]) -> DataFrame:
    ...
    indices = [idx for (idx, v) in enumerate(df[col]) if predicate(v)]
    return {c: [df[c][idx] for idx in indices] for c in df}
...
def sort_by(df: DataFrame, col: str, reverse: bool=False) -> DataFrame:
    ...
    if col not in df:
        raise DataError(f'Column {col!r} not found')
    num = len(df[col])
    order = sorted(range(num), key=lambda idx: df[col][idx], reverse=reverse)
    return {c: [df[c][idx] for idx in order] for c in df}
...
def group_by(df: DataFrame, key_col: str, agg_col: str, agg_func: str) -> dict:
    ...
    for (idx, key) in enumerate(df[key_col]):
        value = df[agg_col][idx] if agg_func != 'count' else 1
...
def to_csv(df: DataFrame) -> str:
    ...
    num = len(next(iter(df.values())))
    out = io.StringIO()
    writer = csv.writer(out)
    writer.writerow(headers)
    for idx in range(num):
        writer.writerow([df[h][idx] for h in headers])
    return out.getvalue()
