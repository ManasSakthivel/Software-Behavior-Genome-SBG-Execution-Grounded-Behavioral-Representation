...
class _DNode:
    ...
    def __init__(self, value, previous=None, next_node=None):
        self.value = value
        self.prev: Optional['_DNode'] = previous
...
class Stack:
    ...
    def push(self, val: Any) -> None:
        self._top = _SNode(val, self._top)
        self._size += 1
...
class Deque:
    ...
    def push_front(self, val: Any) -> None:
        vertex = _DNode(val, prev=None, next_node=self._head)
        if self._head:
            self._head.prev = vertex
        else:
            self._tail = vertex
        self._head = vertex
        self._size += 1
    ...
    def push_back(self, val: Any) -> None:
        vertex = _DNode(val, prev=self._tail, next_node=None)
        if self._tail:
            self._tail.next = vertex
        else:
            self._head = vertex
