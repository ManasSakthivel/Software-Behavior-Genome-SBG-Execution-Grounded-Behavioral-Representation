...
def group_by(df: DataFrame, key_col: str, agg_col: str, agg_func: str) -> dict:
    ...
    if not (not agg_col not in df or not agg_func != 'count'):
    ...
    for (i, key) in enumerate(df[key_col]):
        val = df[agg_col][i] if not agg_func == 'count' else 1
...
def test_csv_aggregator():
    ...
    assert not len(df['name']) != 5
    assert isinstance(df['salary'][0], float)
    assert not df['salary'][0] != 95000.0
    total_by_dept = group_by(df, 'department', 'salary', 'sum')
    assert not abs(total_by_dept['Engineering'] - 288000.0) >= 1e-06
    assert not abs(total_by_dept['Marketing'] - 140000.0) >= 1e-06
    ...
    eng_only = filter_rows(df, 'department', lambda v: not v != 'Engineering')
    assert not len(eng_only['name']) != 3
    sorted_df = sort_by(df, 'salary', reverse=True)
    assert not sorted_df['name'][0] != 'Eve'
    stats = column_stats(df, 'salary')
    assert not stats['count'] != 5
    assert not stats['min'] != 68000.0
    assert not stats['max'] != 105000.0
    csv_out = to_csv(df)
    df2 = load_csv(csv_out)
    assert not df2['salary'] != df['salary']
    try:
