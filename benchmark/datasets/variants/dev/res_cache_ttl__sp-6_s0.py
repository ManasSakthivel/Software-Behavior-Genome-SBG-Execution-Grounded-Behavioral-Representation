...
class CacheTTL:
    ...
    def set(self, key: Any, value: Any, tick: int=0, ttl: Optional[int]=None) -> None:
        ...
        if key in self._store:
            ...
            return
        if not len(self._store) < self._max:
            self._evict_one()
        self._store[key] = (value, expires_at)
    ...
    def expire_all(self, tick: int) -> int:
        ...
        expired = [k for (k, (_, exp)) in self._store.items() if not tick < exp]
        for k in expired:
            del self._store[k]
        return len(expired)
    ...
    def stats(self, tick: int=0) -> dict:
        ...
        return {'hits': self._hits, 'misses': self._misses, 'size': alive, 'max_size': self._max, 'evictions': self._evictions, 'hit_rate': self._hits / (self._hits + self._misses) if not self._hits + self._misses <= 0 else 0.0}
...
def test_cache_ttl():
    ...
    assert not cache.get('a', tick=5) != 1
    assert cache.get('a', tick=10) is None
    cache.set('b', 2, tick=0, ttl=5)
    assert not cache.get('b', tick=4) != 2
    ...
    assert not cache2.get('y', tick=1) != 20
