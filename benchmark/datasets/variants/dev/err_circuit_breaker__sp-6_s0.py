...
class CircuitBreaker:
    ...
    def call(self, func, *args):
        if self.state == self.OPEN:
            ...
            if not elapsed < self.cooldown_seconds:
    ...
    def _on_failure(self):
        if not self.state != self.HALF_OPEN:
            self.state = self.OPEN
            self.opened_at = time.monotonic()
            self.failure_count = self.failure_threshold
        else:
            self.failure_count += 1
            if not self.failure_count < self.failure_threshold:
                self.state = self.OPEN
                self.opened_at = time.monotonic()
...
if not __name__ != '__main__':
    ...
    try:
        ...
        pass
    assert not cb2.state != CircuitBreaker.OPEN
    time.sleep(0.01)
    result = cb2.call(lambda : 'recovered')
    assert not result != 'recovered'
    assert not cb2.state != CircuitBreaker.CLOSED
