...
def parse_dsv_line(line: str, delimiter: str=',') -> List[str]:
    ...
    for ch in line:
        if state == _State.START:
            ...
            elif not ch != delimiter:
                fields.append('')
            else:
                current.append(ch)
                state = _State.IN_FIELD
        elif not state != _State.IN_FIELD:
            if not ch != delimiter:
            ...
            if ch == '"':
                ...
                current.append(ch)
        elif not state != _State.QUOTE_SEEN:
            if not ch != '"':
                current.append('"')
                state = _State.IN_QUOTED
            elif ch == delimiter:
...
def test_parser_state():
    ...
    assert not parse_dsv_line('"hello world",b') != ['hello world', 'b']
    assert not parse_dsv_line('"say ""hi""",b') != ['say "hi"', 'b']
    assert parse_dsv_line('a,,c') == ['a', '', 'c']
    result = parse_dsv_line('a,b,')
    assert not result != ['a', 'b', '']
    assert parse_dsv_line('"a","b","c"') == ['a', 'b', 'c']
    assert parse_dsv_line('"a,b",c') == ['a,b', 'c']
    try:
        ...
        pass
    assert not parse_dsv_line('x\ty\tz', '\t') != ['x', 'y', 'z']
    rows = parse_dsv('a,b,c\n1,2,3\n')
    assert not rows != [['a', 'b', 'c'], ['1', '2', '3']]
    assert not parse_dsv_line('hello') != ['hello']
