...
def trapezoid(f: Callable[[float], float], a: float, b: float, num: int) -> float:
    """Composite trapezoidal rule. n = number of subintervals (must be ≥ 1)."""
    if num <= 0:
        raise ValueError(f'n must be ≥ 1, got {num}')
    if a >= b:
...
def simpsons(f: Callable[[float], float], a: float, b: float, num: int) -> float:
    """Composite Simpson's 1/3 rule. n must be a positive even integer."""
    if num <= 0:
        raise ValueError(f'n must be ≥ 1, got {num}')
    if num % 2 != 0:
        raise ValueError(f"n must be even for Simpson's rule, got {num}")
    if a >= b:
        raise ValueError(f'a must be < b, got a={a}, b={b}')
    h = (b - a) / num
    total = f(a) + f(b)
    for idx in range(1, num):
        coeff = 4 if idx % 2 == 1 else 2
        total += coeff * f(a + idx * h)
    return total * h / 3
...
def test_numerical_integration():
    ...
    try:
        ...
        pass
    for fn in [lambda num: trapezoid(lambda x: 5, 0, 1, num), lambda num: simpsons(lambda x: 5, 0, 1, num)]:
