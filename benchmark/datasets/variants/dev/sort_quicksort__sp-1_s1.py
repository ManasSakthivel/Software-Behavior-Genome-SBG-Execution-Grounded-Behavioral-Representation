...
def quicksort(array_: list, low: int=0, high: int=None) -> None:
    """Sort arr[low..high] in-place using Lomuto quicksort."""
    if high is None:
        high = len(array_) - 1
    if low < high:
        pivot_idx = _partition(array_, low, high)
        quicksort(array_, low, pivot_idx - 1)
        quicksort(array_, pivot_idx + 1, high)

def _partition(array_: list, low: int, high: int) -> int:
    pivot = array_[high]
    i = low - 1
    ...
    (array_[i + 1], array_[high]) = (array_[high], array_[i + 1])
    return i + 1
