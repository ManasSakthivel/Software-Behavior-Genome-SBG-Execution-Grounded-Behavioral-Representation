...
def fn_load_csv(text: str) -> DataFrame:
    ...
    for (rownum, row) in enumerate(rows[1:], start=2):
        ...
        for (h, val) in zip(headers, row):
            df[h].append(fn__coerce(val.strip()))
    return df

def fn_filter_rows(df: DataFrame, col: str, predicate: Callable[[Any], bool]) -> DataFrame:
...
def fn_sort_by(df: DataFrame, col: str, reverse: bool=False) -> DataFrame:
...
def fn_group_by(df: DataFrame, key_col: str, agg_col: str, agg_func: str) -> dict:
    """
...
def fn_column_stats(df: DataFrame, col: str) -> dict:
...
def fn_to_csv(df: DataFrame) -> str:
...
def test_csv_aggregator():
    ...
    total_by_dept = fn_group_by(df, 'department', 'salary', 'sum')
    assert abs(total_by_dept['Engineering'] - 288000.0) < 1e-06
    assert abs(total_by_dept['Marketing'] - 140000.0) < 1e-06
    count_by_dept = fn_group_by(df, 'department', 'name', 'count')
    assert count_by_dept['Engineering'] == 3
    assert count_by_dept['Marketing'] == 2
    mean_salary = fn_group_by(df, 'department', 'salary', 'mean')
    assert abs(mean_salary['Engineering'] - 96000.0) < 1e-06
    eng_only = fn_filter_rows(df, 'department', lambda v: v == 'Engineering')
    assert len(eng_only['name']) == 3
    sorted_df = fn_sort_by(df, 'salary', reverse=True)
    assert sorted_df['name'][0] == 'Eve'
    stats = fn_column_stats(df, 'salary')
    assert stats['count'] == 5
    assert stats['min'] == 68000.0
    assert stats['max'] == 105000.0
    csv_out = fn_to_csv(df)
    df2 = fn_load_csv(csv_out)
    assert df2['salary'] == df['salary']
    try:
        fn_group_by(df, 'nonexistent', 'salary', 'sum')
        assert False
    except DataError:
        pass
...
if __name__ == '__main__':
    test_csv_aggregator()
    df = fn_load_csv(SAMPLE_CSV)
    print('Salary by department (mean):', fn_group_by(df, 'department', 'salary', 'mean'))
    print('Stats:', fn_column_stats(df, 'salary'))