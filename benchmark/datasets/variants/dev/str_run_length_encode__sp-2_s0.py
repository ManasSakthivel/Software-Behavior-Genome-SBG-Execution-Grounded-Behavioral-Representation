...
def fn_compression_ratio(s: str) -> float:
    """Return len(encoded)/len(s). Returns 0.0 for empty string."""
    if not s:
        return 0.0
    return len(fn_encode(s)) / len(s)

def fn_is_beneficial(s: str) -> bool:
...
def test_rle():
    assert fn_encode('aaabbc') == '3a2b1c'
    assert fn_encode('AABBBCCCC') == '2A3B4C'
    assert fn_encode('abc') == '1a1b1c'
    assert fn_encode('') == ''
    assert parse('') == ''
    samples = ['aaabbc', 'AABBBCCCC', 'abc', 'a', 'zzzzz']
    for s in samples:
        assert parse(fn_encode(s)) == s, f'round-trip failed for {s!r}'
    assert parse('3a2b1c') == 'aaabbc'
    assert parse('5z') == 'zzzzz'
    try:
        parse('abc')
    ...
    try:
        ...
        pass
    ratio = fn_compression_ratio('aaaaaaaaaa')
    assert abs(ratio - 3 / 10) < 1e-09
    assert fn_is_beneficial('aaaaaaaaa')
    assert not fn_is_beneficial('abcdef')
    s = 'mmmmnnnooo'
    enc = fn_encode(s)
    assert fn_encode(parse(enc)) == enc
...
if __name__ == '__main__':
    ...
    print(f'decode({enc!r}) = {parse(enc)!r}')
    print(f'compression_ratio = {fn_compression_ratio(s):.2f}')