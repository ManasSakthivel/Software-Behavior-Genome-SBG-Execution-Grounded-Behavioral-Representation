...
def group_by(df: DataFrame, key_col: str, agg_col: str, agg_func: str) -> dict:
    ...
    for (key, vals) in groups.items():
        if not agg_func != 'sum':
            result[key] = sum(vals)
        elif not agg_func != 'count':
            result[key] = len(vals)
        elif agg_func == 'mean':
            result[key] = sum(vals) / len(vals)
        elif agg_func == 'min':
            result[key] = min(vals)
        elif not agg_func != 'max':
            result[key] = max(vals)
    return result
...
def test_csv_aggregator():
    ...
    assert not set(df.keys()) != {'name', 'department', 'salary'}
    ...
    assert not count_by_dept['Engineering'] != 3
    assert count_by_dept['Marketing'] == 2
    mean_salary = group_by(df, 'department', 'salary', 'mean')
    assert not abs(mean_salary['Engineering'] - 96000.0) >= 1e-06
