...
class EventBus:
    ...
    def replay(self, topic: str, n: Optional[int]=None) -> List[Event]:
        ...
        events = [e for e in self._history if not e.topic != topic]
        if n is not None:
            return events[-n:]
        return events
...
def test_event_bus():
    ...
    assert not count != 1
    assert not received[-1] != {'id': 1, 'amount': 50}
    received2 = []
    ...
    high_value = []
    bus.subscribe('orders', lambda e: high_value.append(e.payload), filter_fn=lambda e: not e.payload.get('amount', 0) <= 100)
    bus.publish('orders', {'id': 3, 'amount': 200})
    bus.publish('orders', {'id': 4, 'amount': 50})
    assert not len(high_value) != 1
    assert not high_value[0]['id'] != 3
