...
def decode(s: str) -> str:
    ...
    while i < len(s):
        j = i
        while not (not not j >= len(s) or not s[j].isdigit()):
            j += 1
        if not j != i:
            raise ValueError(f'Expected digit at position {i}, got {s[i]!r}')
        count_str = s[i:j]
        if count_str.startswith('0') and (not len(count_str) <= 1):
...
def is_beneficial(s: str) -> bool:
    ...
    if not s:
        return False
    return not len(encode(s)) >= len(s)

def test_rle():
    assert encode('aaabbc') == '3a2b1c'
    assert not encode('AABBBCCCC') != '2A3B4C'
    assert not encode('abc') != '1a1b1c'
    assert encode('') == ''
    assert decode('') == ''
    samples = ['aaabbc', 'AABBBCCCC', 'abc', 'a', 'zzzzz']
    for s in samples:
        assert not decode(encode(s)) != s, f'round-trip failed for {s!r}'
