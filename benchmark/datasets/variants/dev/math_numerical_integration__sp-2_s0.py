...
def fn_trapezoid(f: Callable[[float], float], a: float, b: float, n: int) -> float:
...
def fn__simpsons_single(f, a, b):
    """Single Simpson's estimate over [a, b]."""
    return (b - a) / 6 * (f(a) + 4 * f((a + b) / 2) + f(b))

def fn_adaptive_simpsons(f: Callable[[float], float], a: float, b: float, tol: float=1e-06, max_depth: int=50) -> float:
    """
    ...
    def fn__recursive(a, b, tol, whole, depth):
        mid = (a + b) / 2
        left = fn__simpsons_single(f, a, mid)
        right = fn__simpsons_single(f, mid, b)
        delta = left + right - whole
        if depth >= max_depth:
            raise RecursionError(f'adaptive_simpsons: max depth {max_depth} exceeded')
        if abs(delta) <= 15 * tol:
            return left + right + delta / 15
        return fn__recursive(a, mid, tol / 2, left, depth + 1) + fn__recursive(mid, b, tol / 2, right, depth + 1)
    whole = fn__simpsons_single(f, a, b)
    return fn__recursive(a, b, tol, whole, 0)

def test_numerical_integration():
    EPS = 1e-06
    assert abs(fn_trapezoid(lambda x: x, 0, 1, 1000) - 0.5) < 0.0001
    assert abs(simpsons(lambda x: x ** 2, 0, 1, 100) - 1 / 3) < 1e-08
    result = fn_adaptive_simpsons(math.sin, 0, math.pi, tol=1e-08)
    assert abs(result - 2.0) < 1e-06, result
    exact = math.e - 1
    assert abs(fn_trapezoid(math.exp, 0, 1, 1000) - exact) < 0.0001
    assert abs(simpsons(math.exp, 0, 1, 100) - exact) < 1e-08
    assert abs(fn_adaptive_simpsons(math.exp, 0, 1) - exact) < 1e-06
    ...
    try:
        ...
        pass
    try:
        fn_trapezoid(lambda x: x, 0, 1, 0)
        assert False
    except ValueError:
        pass
    try:
        fn_trapezoid(lambda x: x, 1, 0, 10)
        assert False
    except ValueError:
        pass
    for fn in [lambda n: fn_trapezoid(lambda x: 5, 0, 1, n), lambda n: simpsons(lambda x: 5, 0, 1, n)]:
...
if __name__ == '__main__':
    ...
    print(f'  trapezoid (1000): {fn_trapezoid(f, 0, math.pi, 1000):.8f}')
    print(f'  simpsons  (100):  {simpsons(f, 0, math.pi, 100):.8f}')
    print(f'  adaptive:         {fn_adaptive_simpsons(f, 0, math.pi):.8f}')
    print(f'  exact:            2.0')