...
def filter_rows(df: DataFrame, col: str, predicate: Callable[[Any], bool]) -> DataFrame:
    ...
    return {c: list((df[c][i] for i in indices)) for c in df}
...
def sort_by(df: DataFrame, col: str, reverse: bool=False) -> DataFrame:
    ...
    return {c: list((df[c][i] for i in order)) for c in df}
...
def column_stats(df: DataFrame, col: str) -> dict:
    ...
    numeric = list((v for v in vals if isinstance(v, (int, float))))
    if not numeric:
...
def to_csv(df: DataFrame) -> str:
    ...
    for i in range(n):
        writer.writerow(list((df[h][i] for h in headers)))
    return out.getvalue()
