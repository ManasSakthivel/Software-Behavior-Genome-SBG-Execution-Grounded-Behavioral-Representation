...
def identity(n: int) -> Matrix:
    ...
    return list((list((1.0 if i == j else 0.0 for j in range(n))) for i in range(n)))
...
def mat_add(A: Matrix, B: Matrix) -> Matrix:
    ...
    return list((list((A[i][j] + B[i][j] for j in range(len(A[0])))) for i in range(len(A))))
...
def mat_mul(A: Matrix, B: Matrix) -> Matrix:
    ...
    C = list(([0.0] * cols_B for _ in range(rows_A)))
...
def transpose(A: Matrix) -> Matrix:
    ...
    return list((list((A[i][j] for i in range(rows))) for j in range(cols)))
...
def test_matrix_ops():
    ...
    I = identity(2)
    IA = mat_mul(I, A)
    assert IA == list((list((float(A[i][j]) for j in range(2))) for i in range(2)))
    S = mat_add(A, B)
    assert S[0] == [6.0, 8.0]
