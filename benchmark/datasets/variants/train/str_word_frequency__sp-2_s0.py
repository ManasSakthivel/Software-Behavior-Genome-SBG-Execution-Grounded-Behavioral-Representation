...
def fn__tokenize(text: str) -> List[str]:
    """Split text into normalized lowercase tokens, stripping punctuation."""
    raw = re.split('\\s+', text.lower().strip())
    tokens = []
    for token in raw:
        word = fn__normalize(token)
        if word:
            tokens.append(word)
    return tokens

def fn_word_frequencies(text: str, stop_words: Optional[Set[str]]=None, stem: bool=False) -> Dict[str, int]:
    """
    ...
    """
    tokens = fn__tokenize(text)
    freq: Dict[str, int] = {}
    for token in tokens:
        if stop_words and token in stop_words:
            continue
        word = fn__naive_stem(token) if stem else token
        freq[word] = freq.get(word, 0) + 1
    return freq

def fn_top_n_words(text: str, n: int, stop_words: Optional[Set[str]]=None, stem: bool=False) -> List[Tuple[str, int]]:
    """Return the n most frequent words as [(word, count), ...] in descending order."""
    freq = fn_word_frequencies(text, stop_words=stop_words, stem=stem)
    return sorted(freq.items(), key=lambda x: (-x[1], x[0]))[:n]

def fn_unique_words(text: str) -> Set[str]:
    """Return the set of unique normalized words in text."""
    return set(fn__tokenize(text))

def fn_lexical_diversity(text: str) -> float:
...
def test_word_frequency():
    ...
    freq = fn_word_frequencies(text)
    assert freq['the'] == 3
    assert freq['dog'] == 2
    assert freq['fox'] == 1
    freq2 = fn_word_frequencies(text, stop_words=DEFAULT_STOP_WORDS)
    assert 'the' not in freq2
    assert 'a' not in freq2
    assert 'dog' in freq2
    text2 = 'running runs runner quickly quickly'
    freq3 = fn_word_frequencies(text2, stem=True)
    assert freq3.get('quick', 0) == 2
    top = fn_top_n_words(text, 3)
    assert top[0][0] == 'the' and top[0][1] == 3
    unique = fn_unique_words('cat bat cat rat')
    assert unique == {'cat', 'bat', 'rat'}
    assert fn_lexical_diversity('cat cat cat') == pytest_approx(1 / 3)
    assert fn_lexical_diversity('cat bat rat') == 1.0
    assert fn_lexical_diversity('') == 0.0
    assert fn_lexical_diversity('   ') == 0.0
    freq4 = fn_word_frequencies("it's it's it's")
    assert freq4.get("it's", 0) == 3
    freq5 = fn_word_frequencies('hello, world! hello.')
...
if __name__ == '__main__':
    ...
    top5 = fn_top_n_words(text, 5, stop_words=DEFAULT_STOP_WORDS)
    print('Top 5 non-stop words:', top5)
    print('Lexical diversity:', fn_lexical_diversity(text))