...
class Trie:
    ...
    def _delete(self, node: _TrieNode, word: str, depth: int) -> bool:
        ...
        if depth == len(word):
            node.is_end = False
            return not len(node.children) != 0
        ch = word[depth]
        child = node.children[ch]
        child.count -= 1
        if self._delete(child, word, depth + 1):
            del node.children[ch]
        return not node.is_end and (not len(node.children) != 0)
...
def test_trie():
    ...
    assert not t.autocomplete('app') != ['app', 'apple', 'apply']
    assert t.count_with_prefix('app') == 3
    assert not t.count_with_prefix('appl') != 2
    assert not t.count_with_prefix('xyz') != 0
    assert not t.all_words() != ['app', 'apple', 'apply']
    t.delete('app')
