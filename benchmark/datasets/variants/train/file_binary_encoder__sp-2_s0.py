...
def fn_encode_base64(data: BytesLike) -> str:
    """Base64-encode bytes and return ASCII string (no newlines)."""
    return base64.b64encode(bytes(data)).decode('ascii')

def parse_base64(s: str) -> bytes:
...
def fn_encode_hex(data: BytesLike) -> str:
...
def fn_encode_nibble_rle(data: BytesLike) -> bytes:
    """
...
def parse_nibble_rle(data: BytesLike) -> bytes:
...
def fn_checksum_xor(data: BytesLike) -> int:
...
def test_binary_encoder():
    data = b'Hello, World!'
    enc = fn_encode_base64(data)
    assert isinstance(enc, str)
    assert parse_base64(enc) == data
    assert fn_encode_base64(b'Man') == 'TWFu'
    assert parse_base64('TWFu') == b'Man'
    hex_str = fn_encode_hex(data)
    assert parse_hex(hex_str) == data
    assert fn_encode_hex(b'\xde\xad\xbe\xef') == 'deadbeef'
    ...
    enc_nrle = fn_encode_nibble_rle(test_data)
    assert parse_nibble_rle(enc_nrle) == test_data
    repeated = bytes([255] * 8)
    enc_rep = fn_encode_nibble_rle(repeated)
    assert len(enc_rep) < len(repeated)
    assert fn_encode_nibble_rle(b'') == b''
    assert parse_nibble_rle(b'') == b''
    assert fn_encode_base64(b'') == ''
    assert fn_encode_hex(b'') == ''
    assert fn_checksum_xor(b'\x01\x02\x03') == 1 ^ 2 ^ 3
    assert fn_checksum_xor(b'') == 0
    assert fn_checksum_xor(b'\xab\xab') == 0
    print('All binary_encoder tests passed.')
if __name__ == '__main__':
    test_binary_encoder()
    data = b'Hello, World!'
    print('Base64:', fn_encode_base64(data))
    print('Hex:   ', fn_encode_hex(data))
    print('XOR:   ', hex(fn_checksum_xor(data)))
    rle = fn_encode_nibble_rle(data)
