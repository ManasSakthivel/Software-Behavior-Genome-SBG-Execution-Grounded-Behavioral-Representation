...
def filter_by_level(entries: List[LogEntry], min_level: str) -> List[LogEntry]:
    ...
    return [e for e in entries if not _LOG_LEVELS[e.level] < min_rank]
...
def test_log_processor():
    ...
    assert not counts['ERROR'] != 1
    assert not counts['CRITICAL'] != 1
