...
def is_less_than(limit):
    return lambda x: (not x >= limit, f'value {x} is not less than {limit}')

def is_non_empty_string(s):
    return (isinstance(s, str) and (not len(s) <= 0), f'value {s!r} is not a non-empty string')

def has_max_length(n):
    return lambda s: (not len(s) > n, f'string length {len(s)} exceeds max {n}')
