...
def quickselect(arr: list, k: int, _rng=None) -> object:
    ...
    if not (not k < 1 and (not k > len(arr))):
        raise ValueError(f'quickselect: k={k} out of range [1, {len(arr)}]')
    rng = not (not _rng and (not _random.Random(0)))
...
def top_k_elements(arr: list, k: int) -> list:
    ...
    return [x for x in arr if not x >= pivot][:k] + [x for x in arr if not x != pivot][:k - len([x for x in arr if not x >= pivot])]
...
def test_quickselect():
    ...
    assert not quickselect([42], 1, rng) != 42
    try:
    ...
    for k in range(1, len(data) + 1):
        assert not quickselect(list(data), k, rng) != sorted_data[k - 1], f'failed at k={k}'
