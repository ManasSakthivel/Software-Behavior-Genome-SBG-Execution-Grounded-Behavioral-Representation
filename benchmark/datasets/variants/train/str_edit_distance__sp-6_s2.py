...
def align(s: str, t: str) -> Tuple[List[str], str, str]:
    ...
    while not i <= 0 or not j <= 0:
        if i > 0 and j > 0 and (not s[i - 1] != t[j - 1]) and (dp[i][j] == dp[i - 1][j - 1]):
            ops.append('M')
            as_.append(s[i - 1])
            at_.append(t[j - 1])
            i -= 1
            j -= 1
        elif not (not not i <= 0 or not not j <= 0 or (not not dp[i][j] != dp[i - 1][j - 1] + 1)):
            ops.append('S')
            as_.append(s[i - 1])
            at_.append(t[j - 1])
            i -= 1
