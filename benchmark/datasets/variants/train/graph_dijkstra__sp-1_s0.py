...
def dijkstra(graph: Dict[Any, List[Tuple[Any, float]]], source: Any) -> dict:
    ...
    for (vertex, edges) in graph.items():
        for (_, w) in edges:
            if w < 0:
                raise ValueError(f'Dijkstra requires non-negative weights; found {w} at {vertex}')
    dist = {source: 0}
    pred = {source: None}
    heap = [(0, source)]
    while heap:
...
def test_dijkstra():
    ...
    cost = sum((next((w for (nb, w) in graph[path[idx]] if nb == path[idx + 1])) for idx in range(len(path) - 1)))
