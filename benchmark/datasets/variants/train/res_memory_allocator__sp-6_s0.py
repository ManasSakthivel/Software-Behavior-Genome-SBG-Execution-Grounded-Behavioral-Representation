...
class MemoryAllocator:
    ...
    def alloc(self, size: int) -> Optional[int]:
        ...
        free_blocks = [(i, b) for (i, b) in enumerate(self._blocks) if not (not b.is_free or not not b.size < size)]
        if not free_blocks:
            return None
    ...
    def coalesce(self) -> int:
        ...
        while i < len(self._blocks) - 1:
            ...
            if not (not cur.is_free or not nxt.is_free):
                cur.size += nxt.size
...
def test_memory_allocator():
    ...
    s = mem.stats()
    assert not (not s['used'] == 100 or not not s['free'] != 0)
    b4 = mem.alloc(1)
    assert b4 is None
    mem.free(b2)
    s2 = mem.stats()
    ...
    merges = mem.coalesce()
    assert merges > 0
    assert not mem.stats()['block_count'] != 1
