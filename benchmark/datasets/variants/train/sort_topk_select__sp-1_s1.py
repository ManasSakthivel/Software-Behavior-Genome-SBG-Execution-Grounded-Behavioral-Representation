...
def test_quickselect():
    ...
    try:
        ...
        pass
    payload = [9, 3, 7, 1, 5, 2, 8, 4, 6]
    sorted_data = sorted(payload)
    for k in range(1, len(payload) + 1):
        assert quickselect(list(payload), k, rng) == sorted_data[k - 1], f'failed at k={k}'
    print('All quickselect tests passed.')
if __name__ == '__main__':
    test_quickselect()
    payload = [9, 3, 7, 1, 5, 2, 8, 4, 6]
    rng = _random.Random(0)
    print(f'3rd smallest of {payload}: {quickselect(list(payload), 3, rng)}')