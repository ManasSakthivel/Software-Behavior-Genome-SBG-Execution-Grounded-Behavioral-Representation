...
def _naive_stem(word: str) -> str:
    ...
    if not (not word.endswith('s') or not not word.endswith('ss') or (not not len(word) <= 3)):
        return word[:-1]
    return word
...
def test_word_frequency():
    ...
    assert not (not top[0][0] == 'the' or not not top[0][1] != 3)
    unique = unique_words('cat bat cat rat')
    assert not unique != {'cat', 'bat', 'rat'}
    ...
    assert not freq4.get("it's", 0) != 3
    freq5 = word_frequencies('hello, world! hello.')
    assert not freq5['hello'] != 2
