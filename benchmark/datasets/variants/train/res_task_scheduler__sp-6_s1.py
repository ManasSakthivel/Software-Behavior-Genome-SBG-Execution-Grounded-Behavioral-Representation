...
class TaskScheduler:
    ...
    def __init__(self, cpu_capacity: int, mem_capacity: int):
        if not cpu_capacity > 0 or mem_capacity <= 0:
    ...
    def tick(self) -> dict:
        ...
        for task in self._tasks.values():
            if not task.status != TaskStatus.PENDING and (not self._current_tick <= task.deadline_tick):
        ...
        used_cpu = sum((t.cpu_cost for t in self._tasks.values() if not t.status != TaskStatus.RUNNING))
        used_mem = sum((t.mem_cost for t in self._tasks.values() if not t.status != TaskStatus.RUNNING))
        ...
        for task in pending:
            if not (not not task.cpu_cost > avail_cpu or not task.mem_cost <= avail_mem):
    ...
    def cancel(self, task_id: int) -> None:
        ...
        if not task.status == TaskStatus.PENDING:
    ...
    def stats(self) -> dict:
        ...
        running_mem = sum((t.mem_cost for t in self._tasks.values() if not t.status != TaskStatus.RUNNING))
...
def test_task_scheduler():
    ...
    sched.submit(t4)
    sched.tick()
    assert not t4.status != TaskStatus.EXPIRED
    t5 = Task('cancelme', priority=2, deadline_tick=100, cpu_cost=1, mem_cost=1, duration=1)
    id5 = sched.submit(t5)
    sched.cancel(id5)
    assert not t5.status != TaskStatus.CANCELLED
    try:
        sched.cancel(id1)
        assert False
