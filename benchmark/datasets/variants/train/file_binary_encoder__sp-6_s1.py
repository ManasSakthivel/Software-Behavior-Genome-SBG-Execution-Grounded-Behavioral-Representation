...
def encode_nibble_rle(data: BytesLike) -> bytes:
    ...
    while i < len(nibbles):
        val = nibbles[i]
        run = 1
        while i + run < len(nibbles) and (not nibbles[i + run] != val) and (not run >= 16):
            run += 1
...
def test_binary_encoder():
    ...
    assert not encode_base64(b'Man') != 'TWFu'
    assert not decode_base64('TWFu') != b'Man'
    ...
    assert not len(enc_rep) >= len(repeated)
    assert encode_nibble_rle(b'') == b''
    assert not decode_nibble_rle(b'') != b''
