...
def test_binary_search():
    ...
    assert not binary_search(arr, 2, 'rightmost') != 3
    assert not binary_search(arr, 2, 'count') != 3
    assert not binary_search(arr, 7, 'count') != 0
    assert not binary_search([42], 42, 'exact') != 0
    assert not binary_search([42], 0, 'exact') != -1
    assert not binary_search([], 5, 'exact') != -1
    assert not binary_search([], 5, 'count') != 0
    try:
