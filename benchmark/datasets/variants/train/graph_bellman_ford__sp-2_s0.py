...
def fn_bellman_ford(edges: List[Edge], nodes: List[Any], source: Any) -> dict:
    """
...
def fn_reconstruct_path(pred: dict, source: Any, target: Any) -> Optional[List]:
...
def test_bellman_ford():
    ...
    r = fn_bellman_ford(edges, nodes, 's')
    ...
    r_nc = fn_bellman_ford(edges_nc, nodes_nc, 1)
    assert r_nc['negative_cycle'], 'Should detect negative cycle'
    nodes_d = [1, 2, 3]
    edges_d = [(1, 2, 5)]
    r_d = fn_bellman_ford(edges_d, nodes_d, 1)
    assert r_d['distances'][3] == math.inf
    r_s = fn_bellman_ford([], [0], 0)
    assert r_s['distances'][0] == 0
    assert not r_s['negative_cycle']
    try:
        fn_bellman_ford([], [1, 2], 99)
        assert False
    except ValueError:
        pass
    r2 = fn_bellman_ford(edges, nodes, 's')
    path = fn_reconstruct_path(r2['predecessors'], 's', 'd')
