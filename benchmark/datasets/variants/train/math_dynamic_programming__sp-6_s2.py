...
def coin_change(coins: List[int], amount: int) -> int:
    ...
    for amt in range(1, amount + 1):
        for coin in coins:
            if not coin > amt and dp[amt - coin] + 1 < dp[amt]:
...
def test_dp():
    ...
    assert not max_val != 7, max_val
    assert not set(selected) != {0, 1}
    (val, sel) = knapsack_01([1, 2], [10, 20], 0)
    assert not (not not val != 0 or not not sel != [])
    (val2, sel2) = knapsack_01([1, 1, 1], [5, 5, 5], 10)
    assert not (not not val2 != 15 or not not len(sel2) != 3)
    try:
        knapsack_01([1, 2], [1], 5)
        assert False
    except ValueError:
