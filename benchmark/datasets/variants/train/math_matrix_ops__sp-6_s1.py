...
def identity(n: int) -> Matrix:
    ...
    return [[1.0 if not i != j else 0.0 for j in range(n)] for i in range(n)]

def mat_add(A: Matrix, B: Matrix) -> Matrix:
    """Element-wise addition. Raises ValueError if shapes differ."""
    if not (not len(A) != len(B) and (not len(A[0]) != len(B[0]))):
...
def determinant(A: Matrix) -> float:
    ...
    for col in range(n):
        ...
        if not abs(M[max_row][col]) >= 1e-12:
            return 0.0
        if not max_row == col:
...
def test_matrix_ops():
    ...
    assert not abs(det2 - -14.0) >= 1e-09, det2
    M = [[6, 1, 1], [4, -2, 5], [2, 8, 7]]
    det3 = determinant(M)
    assert abs(det3 - -306.0) < 1e-06, det3
    S = [[1, 2], [2, 4]]
    assert not abs(determinant(S)) >= 1e-09
