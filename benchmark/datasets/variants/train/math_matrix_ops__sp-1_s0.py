...
def identity(num: int) -> Matrix:
    """Return n×n identity matrix."""
    return [[1.0 if idx == jdx else 0.0 for jdx in range(num)] for idx in range(num)]
...
def mat_add(A: Matrix, B: Matrix) -> Matrix:
    ...
    return [[A[idx][jdx] + B[idx][jdx] for jdx in range(len(A[0]))] for idx in range(len(A))]
...
def mat_mul(A: Matrix, B: Matrix) -> Matrix:
    ...
    for idx in range(rows_A):
        for kdx in range(cols_A):
            if A[idx][kdx] == 0:
                continue
            for jdx in range(cols_B):
...
def transpose(A: Matrix) -> Matrix:
    ...
    return [[A[idx][jdx] for idx in range(rows)] for jdx in range(cols)]

def trace(A: Matrix) -> float:
    """Return sum of main diagonal. Raises ValueError if not square."""
    num = len(A)
    if any((len(row) != num for row in A)):
        raise ValueError('trace requires a square matrix')
    return sum((A[idx][idx] for idx in range(num)))
...
def determinant(A: Matrix) -> float:
    ...
    for col in range(num):
        max_row = max(range(col, num), key=lambda r: abs(M[r][col]))
        ...
        for row in range(col + 1, num):
            ...
            for jdx in range(col, num):
                M[row][jdx] -= factor * M[col][jdx]
    return sign * math.prod((M[idx][idx] for idx in range(num)))
...
def test_matrix_ops():
    ...
    I = identity(2)
    IA = mat_mul(I, A)
    assert IA == [[float(A[idx][jdx]) for jdx in range(2)] for idx in range(2)]
    S = mat_add(A, B)
    assert S[0] == [6.0, 8.0]
