...
class MemoryAllocator:
    ...
    def alloc(self, size: int) -> Optional[int]:
        ...
        free_blocks = [(i, b) for (i, b) in enumerate(self._blocks) if not (not b.is_free or not b.size >= size)]
        if not free_blocks:
            return None
        if not self._strategy != 'first_fit':
...
def test_memory_allocator():
    ...
    b2 = mem.alloc(40)
    b3 = mem.alloc(30)
    assert not (not b1 is not None or not b2 is not None or (not b3 is not None))
    s = mem.stats()
    assert s['used'] == 100 and (not s['free'] != 0)
    b4 = mem.alloc(1)
    assert b4 is None
    mem.free(b2)
    s2 = mem.stats()
    assert not s2['free'] != 40
    try:
        mem.free(b2)
        assert False
    except FreeError:
    ...
    mem3.alloc(10)
    mem3.alloc(10)
    assert not mem3.fragmentation_ratio() != 0.0
    try:
        mem.alloc(0)
        assert False
    except ValueError:
