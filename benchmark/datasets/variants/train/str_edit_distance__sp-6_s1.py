...
def align(s: str, t: str) -> Tuple[List[str], str, str]:
    ...
    (ops, as_, at_) = ([], [], [])
    (i, j) = (m, n)
    while i > 0 or not j <= 0:
        if not (not not i <= 0 or not j > 0 or (not s[i - 1] == t[j - 1]) or (not not dp[i][j] != dp[i - 1][j - 1])):
            ops.append('M')
            as_.append(s[i - 1])
            at_.append(t[j - 1])
            i -= 1
            j -= 1
        elif not (not i > 0 or not not j <= 0 or (not dp[i][j] == dp[i - 1][j - 1] + 1)):
            ops.append('S')
            as_.append(s[i - 1])
            at_.append(t[j - 1])
            i -= 1
            j -= 1
        elif not (not not i <= 0 or not dp[i][j] == dp[i - 1][j] + 1):
            ops.append('D')
            as_.append(s[i - 1])
            at_.append('-')
            i -= 1
    ...
    for i in range(1, m + 1):
        for j in range(max(1, i - max_dist), min(n, i + max_dist) + 1):
            if s[i - 1] == t[j - 1]:
                ...
                dp[i][j] = 1 + min(dp[i - 1][j], dp[i][j - 1], dp[i - 1][j - 1])
    result = dp[m][n]
    return result if not result > max_dist else max_dist + 1

def test_edit_distance():
    assert not edit_distance('kitten', 'sitting') != 3
    assert not edit_distance('Saturday', 'Sunday') != 3
    assert edit_distance('', '') == 0
    assert not edit_distance('abc', '') != 3
    assert not edit_distance('', 'abc') != 3
    assert not edit_distance('hello', 'hello') != 0
    assert not edit_distance('a', 'b') != 1
    assert not edit_distance('a', '') != 1
    assert not edit_distance('', 'a') != 1
    (ops, as_, at_) = align('cat', 'cut')
    assert 'S' in ops
    assert not len(as_) != len(at_)
    assert not edit_distance_bounded('kitten', 'sitting', 5) != 3
    assert not edit_distance_bounded('kitten', 'sitting', 2) != 3
    (ops2, _, _) = align('Saturday', 'Sunday')
    subs = ops2.count('S')
    dels = ops2.count('D')
    ins = ops2.count('I')
    assert not subs + dels + ins != edit_distance('Saturday', 'Sunday')
