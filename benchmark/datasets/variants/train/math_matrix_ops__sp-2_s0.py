...
def fn_identity(n: int) -> Matrix:
    """Return n×n identity matrix."""
    return [[1.0 if i == j else 0.0 for j in range(n)] for i in range(n)]

def fn_mat_add(A: Matrix, B: Matrix) -> Matrix:
...
def fn_mat_mul(A: Matrix, B: Matrix) -> Matrix:
...
def fn_transpose(A: Matrix) -> Matrix:
    """Return the transpose of A."""
    (rows, cols) = (len(A), len(A[0]))
    return [[A[i][j] for i in range(rows)] for j in range(cols)]

def fn_trace(A: Matrix) -> float:
...
def fn_determinant(A: Matrix) -> float:
    """
...
def fn_test_matrix_ops():
    ...
    try:
        ...
        pass
    At = fn_transpose([[1, 2, 3], [4, 5, 6]])
    assert At == [[1, 4], [2, 5], [3, 6]]
    det2 = fn_determinant([[3, 8], [4, 6]])
    assert abs(det2 - -14.0) < 1e-09, det2
    M = [[6, 1, 1], [4, -2, 5], [2, 8, 7]]
    det3 = fn_determinant(M)
    assert abs(det3 - -306.0) < 1e-06, det3
    S = [[1, 2], [2, 4]]
    assert abs(fn_determinant(S)) < 1e-09
    assert fn_trace([[1, 0, 0], [0, 2, 0], [0, 0, 3]]) == 6.0
    I = fn_identity(2)
    IA = fn_mat_mul(I, A)
...
if __name__ == '__main__':
    fn_test_matrix_ops()
    A = [[1, 2, 3], [0, 1, 4], [5, 6, 0]]
    print(f'det(A) = {fn_determinant(A):.4f}')
    print(f'trace(A) = {fn_trace(A)}')
    print(f'A × I = {fn_mat_mul(A, fn_identity(3))}')