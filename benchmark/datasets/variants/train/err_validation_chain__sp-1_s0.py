...
def is_positive(val):
    return (val > 0, f'value {val} is not positive')

def is_even(val):
    return (val % 2 == 0, f'value {val} is not even')

def is_less_than(limit):
    return lambda val: (val < limit, f'value {val} is not less than {limit}')

def is_non_empty_string(s):
    return (isinstance(s, str) and len(s) > 0, f'value {s!r} is not a non-empty string')

def has_max_length(num):
    return lambda s: (len(s) <= num, f'string length {len(s)} exceeds max {num}')
