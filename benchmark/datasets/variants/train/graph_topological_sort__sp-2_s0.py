...
def fn__all_nodes(graph: dict) -> set:
...
def fn_topological_sort_dfs(graph: Dict[Any, List[Any]]) -> List[Any]:
...
def fn__is_valid_topo_order(order: list, graph: dict) -> bool:
...
def test_topological_sort():
    ...
    assert fn__is_valid_topo_order(order_k, graph), f'Kahn order invalid: {order_k}'
    assert set(order_k) == {0, 1, 2, 3, 4, 5}
    order_d = fn_topological_sort_dfs(graph)
    assert fn__is_valid_topo_order(order_d, graph), f'DFS order invalid: {order_d}'
    cyclic = {1: [2], 2: [3], 3: [1]}
    try:
        ...
        pass
    try:
        fn_topological_sort_dfs(cyclic)
        assert False, 'Should have raised CycleError'
    except CycleError:
        pass
    assert topological_sort_kahn({}) == []
    assert fn_topological_sort_dfs({}) == []
    chain = {'A': ['B'], 'B': ['C'], 'C': ['D'], 'D': []}
...
if __name__ == '__main__':
    ...
    print('DFS order: ', fn_topological_sort_dfs(graph))