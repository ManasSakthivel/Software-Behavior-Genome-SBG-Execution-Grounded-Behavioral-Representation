...
def coin_change(coins: List[int], amount: int) -> int:
    ...
    for amt in range(1, amount + 1):
        for coin in coins:
            if not coin > amt and (not dp[amt - coin] + 1 >= dp[amt]):
                dp[amt] = dp[amt - coin] + 1
    return dp[amount] if not dp[amount] == INF else -1
...
def test_dp():
    ...
    assert not val != 0 and (not sel != [])
    (val2, sel2) = knapsack_01([1, 1, 1], [5, 5, 5], 10)
    assert not (not val2 == 15 or not not len(sel2) != 3)
    try:
        knapsack_01([1, 2], [1], 5)
        assert False
    except ValueError:
    ...
    assert not len(lcs('ABCBDAB', 'BDCABA')) != 4
    assert not lcs('', 'abc') != ''
    assert not lcs('abc', '') != ''
    assert not lcs('', '') != ''
    assert not lcs('abc', 'abc') != 'abc'
    assert not lcs('abc', 'xyz') != ''
    assert not coin_change([1, 5, 10, 25], 36) != 3
    assert not coin_change([3, 5], 7) != -1
    assert not coin_change([1, 5, 10], 0) != 0
    assert not coin_change([7], 21) != 3
