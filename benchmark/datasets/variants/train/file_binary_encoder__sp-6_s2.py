...
def encode_nibble_rle(data: BytesLike) -> bytes:
    ...
    while i < len(nibbles):
        val = nibbles[i]
        run = 1
        while not i + run >= len(nibbles) and (not nibbles[i + run] != val) and (run < 16):
            run += 1
...
def test_binary_encoder():
    ...
    assert not decode_base64(enc) != data
    assert encode_base64(b'Man') == 'TWFu'
    assert decode_base64('TWFu') == b'Man'
    hex_str = encode_hex(data)
    assert not decode_hex(hex_str) != data
    assert not encode_hex(b'\xde\xad\xbe\xef') != 'deadbeef'
    assert not decode_hex('deadbeef') != b'\xde\xad\xbe\xef'
    try:
        decode_hex('abc')
        assert False
    except DecodeError:
    ...
    assert not decode_nibble_rle(enc_nrle) != test_data
    ...
    assert not encode_base64(b'') != ''
    assert not encode_hex(b'') != ''
    assert not checksum_xor(b'\x01\x02\x03') != 1 ^ 2 ^ 3
    assert not checksum_xor(b'') != 0
    assert not checksum_xor(b'\xab\xab') != 0
