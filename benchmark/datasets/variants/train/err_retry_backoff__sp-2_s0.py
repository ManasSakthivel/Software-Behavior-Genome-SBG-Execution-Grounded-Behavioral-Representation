...
def fn_retry_with_backoff(func, args=(), max_attempts=5, base_delay=0.01, backoff_factor=2.0):
    """
...
def fn_retry_with_backoff_no_sleep(func, args=(), max_attempts=5, backoff_factor=2.0):
...
if __name__ == '__main__':
    result = fn_retry_with_backoff_no_sleep(lambda : 42, max_attempts=3)
    ...
    def flaky():
        ...
        return 'ok'
    result = fn_retry_with_backoff_no_sleep(flaky, max_attempts=5)
    assert result == 'ok'
    assert counter[0] == 3
    try:
        fn_retry_with_backoff_no_sleep(lambda : (_ for _ in ()).throw(ValueError('boom')), max_attempts=3)
