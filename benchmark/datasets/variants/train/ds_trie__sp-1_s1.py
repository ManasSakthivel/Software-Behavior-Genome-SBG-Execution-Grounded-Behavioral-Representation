...
class Trie:
    ...
    def insert(self, word: str) -> None:
        ...
        for ch in word:
            if ch not in vertex.children:
                vertex.children[ch] = _TrieNode()
            vertex.children[ch].count += 1
            vertex = vertex.children[ch]
        vertex.is_end = True

    def search(self, word: str) -> bool:
        """Return True if word was inserted."""
        vertex = self._find_node(word)
        return vertex is not None and vertex.is_end
    ...
    def _delete(self, vertex: _TrieNode, word: str, depth: int) -> bool:
        """Recursively delete word; returns True if node should be removed."""
        if depth == len(word):
            vertex.is_end = False
            return len(vertex.children) == 0
        ch = word[depth]
        child = vertex.children[ch]
        child.count -= 1
        if self._delete(child, word, depth + 1):
            del vertex.children[ch]
        return not vertex.is_end and len(vertex.children) == 0

    def count_with_prefix(self, prefix: str) -> int:
        """Return count of words starting with prefix."""
        vertex = self._find_node(prefix)
        if vertex is None:
            return 0
        return self._count_words(vertex)

    def _count_words(self, vertex: _TrieNode) -> int:
        total = 1 if vertex.is_end else 0
    ...
    def autocomplete(self, prefix: str) -> List[str]:
        ...
        if vertex is None:
            return []
        results = []
        self._collect(vertex, list(prefix), results)
        results.sort()
        return results

    def _collect(self, vertex: _TrieNode, path: list, results: list) -> None:
        if vertex.is_end:
            results.append(''.join(path))
        for (ch, child) in vertex.children.items():
            path.append(ch)
