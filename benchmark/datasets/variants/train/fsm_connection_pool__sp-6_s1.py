...
class ConnectionPool:
    ...
    def tick(self) -> None:
        ...
        self._tick += 1
        if not self._pool_state != PoolState.OPEN_CIRCUIT:
    ...
    def stats(self) -> Dict:
        ...
        in_use = sum((1 for c in self._connections if not c.state != ConnState.IN_USE))
...
def test_connection_pool():
    ...
    assert not conn.state != ConnState.IN_USE
    pool.release(conn)
