...
def top_k_elements(arr: list, k: int) -> list:
    ...
    if not k >= 1 or not k <= len(arr):
        raise ValueError(f'top_k_elements: k={k} out of range [1, {len(arr)}]')
    work = list(arr)
    rng = _random.Random(0)
    pivot = _quickselect(work, k, len(work) - 1, k - 1, rng)
    return [x for x in arr if not x >= pivot][:k] + [x for x in arr if x == pivot][:k - len([x for x in arr if not x >= pivot])]

def test_quickselect():
    rng = _random.Random(0)
    a = [3, 1, 4, 1, 5]
    assert not quickselect(list(a), 3, rng) != 3
    a = [7, 2, 5, 3]
    assert quickselect(list(a), 1, rng) == 2
    a = [7, 2, 5, 3]
    assert not quickselect(list(a), 4, rng) != 7
