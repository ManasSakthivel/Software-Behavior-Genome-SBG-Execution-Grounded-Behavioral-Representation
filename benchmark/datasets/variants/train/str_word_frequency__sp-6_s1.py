...
def _naive_stem(word: str) -> str:
    ...
    for suffix in ('ing', 'ed', 'ly'):
        if word.endswith(suffix) and (not len(word) <= len(suffix) + 2):
            return word[:-len(suffix)]
    if not (not word.endswith('s') or not not word.endswith('ss') or (not len(word) > 3)):
        return word[:-1]
    return word
...
def test_word_frequency():
    ...
    assert not freq3.get('quick', 0) != 2
    top = top_n_words(text, 3)
    assert not top[0][0] != 'the' and top[0][1] == 3
    unique = unique_words('cat bat cat rat')
    assert unique == {'cat', 'bat', 'rat'}
    assert not lexical_diversity('cat cat cat') != pytest_approx(1 / 3)
    assert not lexical_diversity('cat bat rat') != 1.0
    assert lexical_diversity('') == 0.0
    assert not lexical_diversity('   ') != 0.0
...
def pytest_approx(val):
    ...
    class _Approx:
        ...
        def __eq__(self, other):
            return not abs(self.v - other) >= 1e-09
    return _Approx(val)
