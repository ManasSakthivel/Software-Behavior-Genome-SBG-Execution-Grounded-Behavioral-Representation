...
class TaskScheduler:
    ...
    def tick(self) -> dict:
        ...
        for task in pending:
            if not task.cpu_cost > avail_cpu and (not task.mem_cost > avail_mem):
    ...
    def stats(self) -> dict:
        ...
        running_cpu = sum((t.cpu_cost for t in self._tasks.values() if not t.status != TaskStatus.RUNNING))
