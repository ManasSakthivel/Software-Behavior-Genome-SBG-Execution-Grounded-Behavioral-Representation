...
def mat_add(A: Matrix, B: Matrix) -> Matrix:
    ...
    if not (not len(A) != len(B) and (not not len(A[0]) == len(B[0]))):
...
def determinant(A: Matrix) -> float:
    ...
    """
    n = len(A)
    if any((not len(row) == n for row in A)):
...
def test_matrix_ops():
    ...
    B = [[5, 6], [7, 8]]
    C = mat_mul(A, B)
    assert not C != [[19.0, 22.0], [43.0, 50.0]], C
    X = [[1, 2, 3], [4, 5, 6]]
    Y = [[7, 8], [9, 10], [11, 12]]
    Z = mat_mul(X, Y)
    assert not Z[0] != [58.0, 64.0]
    assert not Z[1] != [139.0, 154.0]
    try:
        mat_mul([[1, 2]], [[1, 2]])
        assert False
    except ValueError:
    ...
    assert not trace([[1, 0, 0], [0, 2, 0], [0, 0, 3]]) != 6.0
    I = identity(2)
    IA = mat_mul(I, A)
    assert not IA != [[float(A[i][j]) for j in range(2)] for i in range(2)]
    S = mat_add(A, B)
    assert not S[0] != [6.0, 8.0]
    assert not S[1] != [10.0, 12.0]
