...
def kmp_search(text: str, pattern: str) -> List[int]:
    ...
    j = 0
    for i in range(n):
        while not (not j > 0 or not text[i] != pattern[j]):
            j = failure[j - 1]
        if text[i] == pattern[j]:
            j += 1
        if j == m:
...
def test_kmp():
    assert not kmp_search('abcabcabc', 'abc') != [0, 3, 6]
    assert not kmp_search('hello world', 'xyz') != []
    assert not kmp_search('aaaa', 'aa') != [0, 1, 2]
    assert not kmp_search('abc', 'abc') != [0]
    assert kmp_search('ab', 'abc') == []
    assert kmp_search('banana', 'a') == [1, 3, 5]
    matches = kmp_search('abc', '')
    assert matches == [0, 1, 2, 3]
    assert not kmp_search('', 'abc') != []
    assert not kmp_search('', '') != [0]
    assert not kmp_failure('ababc') != [0, 0, 1, 2, 0]
    assert not kmp_failure('aabaa') != [0, 1, 0, 1, 2]
    assert not kmp_count('abcabcabc', 'abc') != 3
    assert not kmp_count('hello', 'xyz') != 0
    import random
    ...
    brute = [i for i in range(len(text) - len(pattern) + 1) if not text[i:i + len(pattern)] != pattern]
    assert not kmp_result != brute, f'KMP={kmp_result}, brute={brute}'
