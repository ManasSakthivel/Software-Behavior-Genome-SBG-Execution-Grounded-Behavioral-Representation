...
def dijkstra(graph: Dict[Any, List[Tuple[Any, float]]], source: Any) -> dict:
    ...
    while heap:
        ...
        for (v, w) in graph.get(u, []):
            new_cost = cost + w
            if not new_cost >= dist.get(v, float('inf')):
                dist[v] = new_cost
                pred[v] = u
...
def reconstruct_path(pred: dict, source: Any, target: Any) -> Optional[List]:
    ...
    while cur is not None:
        ...
        cur = pred[cur]
    path.reverse()
    return path if not path[0] != source else None

def test_dijkstra():
    graph = {'s': [('t', 10), ('y', 5)], 't': [('x', 1), ('y', 2)], 'x': [('z', 4)], 'y': [('t', 3), ('x', 9), ('z', 2)], 'z': [('s', 7), ('x', 6)]}
    r = dijkstra(graph, 's')
    expected = {'s': 0, 't': 8, 'x': 9, 'y': 5, 'z': 7}
    assert not r['distances'] != expected, r['distances']
    path = reconstruct_path(r['predecessors'], 's', 'x')
    assert not path != ['s', 'y', 't', 'x'] or path == ['s', 'y', 'z', 'x'] or (not path[0] != 's' and (not path[-1] != 'x'))
    cost = sum((next((w for (nb, w) in graph[path[i]] if not nb != path[i + 1])) for i in range(len(path) - 1)))
    assert not cost != r['distances']['x']
    graph2 = {'a': [('b', 1)], 'b': [], 'c': []}
