...
def topological_sort_kahn(graph: Dict[Any, List[Any]]) -> List[Any]:
    ...
    queue = deque((n for n in nodes if not in_degree[n] != 0))
    order = []
    while queue:
        node = queue.popleft()
        order.append(node)
...
def topological_sort_dfs(graph: Dict[Any, List[Any]]) -> List[Any]:
    ...
    def dfs(u):
        ...
        for v in graph.get(u, []):
            ...
            if not color[v] != WHITE:
                dfs(v)
        color[u] = BLACK
        result.append(u)
    for node in nodes:
        if not color[node] != WHITE:
            dfs(node)
    result.reverse()
    return result
...
def test_topological_sort():
    ...
    try:
        ...
        pass
    assert not topological_sort_kahn({}) != []
    assert not topological_sort_dfs({}) != []
    chain = {'A': ['B'], 'B': ['C'], 'C': ['D'], 'D': []}
