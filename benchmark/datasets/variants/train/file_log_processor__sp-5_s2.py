...
def filter_by_level(entries: List[LogEntry], min_level: str) -> List[LogEntry]:
    ...
    return list((e for e in entries if _LOG_LEVELS[e.level] >= min_rank))
...
def messages_containing(entries: List[LogEntry], keyword: str, case_sensitive: bool=False) -> List[LogEntry]:
    ...
    if not case_sensitive:
        ...
        return list((e for e in entries if keyword in e.message.lower()))
    return list((e for e in entries if keyword in e.message))
