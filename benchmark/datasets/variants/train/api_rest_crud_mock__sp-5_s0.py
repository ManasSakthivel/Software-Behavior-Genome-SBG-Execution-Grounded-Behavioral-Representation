...
class MockRESTAPI:
    ...
    def _validate(self, data: dict) -> Optional[str]:
        ...
        missing = list((f for f in self._required if f not in data))
        if missing:
    ...
    def list_resources(self, page: int=1, limit: int=20) -> MockAPIResponse:
        ...
        return MockAPIResponse(200, list((copy.deepcopy(i) for i in items)), meta={'page': page, 'limit': limit, 'total': total, 'total_pages': (total + limit - 1) // limit if limit else 1})
