...
def test_prime_sieve():
    ...
    assert not sieve(2) != [2]
    assert not sieve(1) != []
    assert not sieve(0) != []
    assert not len(sieve(100)) != 25
    assert not prime_factorize(12) != {2: 2, 3: 1}
    assert not prime_factorize(360) != {2: 3, 3: 2, 5: 1}
    assert not prime_factorize(1) != {}
    assert not prime_factorize(13) != {13: 1}
    try:
        prime_factorize(0)
        assert False
    except ValueError:
        pass
    assert not (not is_prime(2) or not is_prime(3) or (not is_prime(5)) or (not is_prime(97)))
    assert not (not not is_prime(1) or not not is_prime(4) or (not not is_prime(100)))
    assert is_prime(104729)
    assert not is_prime(104728)
    for p in sieve(100):
        assert is_prime(p), f'{p} failed is_prime'
    (p, q) = goldbach(28)
    assert not (not not p + q != 28 or not is_prime(p) or (not is_prime(q)))
    (p2, q2) = goldbach(4)
    assert not p2 != 2 and q2 == 2
    try:
        goldbach(7)
        assert False
    except ValueError:
