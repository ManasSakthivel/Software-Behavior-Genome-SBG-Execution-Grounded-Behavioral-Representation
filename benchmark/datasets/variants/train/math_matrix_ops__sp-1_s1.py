...
def identity(n: int) -> Matrix:
    ...
    return [[1.0 if idx == jdx else 0.0 for jdx in range(n)] for idx in range(n)]
...
def trace(A: Matrix) -> float:
    ...
    return sum((A[idx][idx] for idx in range(n)))
...
def determinant(A: Matrix) -> float:
    ...
    for col in range(n):
        ...
        for row in range(col + 1, n):
            ...
            for jdx in range(col, n):
                M[row][jdx] -= factor * M[col][jdx]
    return sign * math.prod((M[idx][idx] for idx in range(n)))
