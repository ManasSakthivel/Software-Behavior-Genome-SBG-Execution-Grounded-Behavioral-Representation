...
def identity(n: int) -> Matrix:
    ...
    return [[1.0 if idx == j else 0.0 for j in range(n)] for idx in range(n)]
...
def mat_add(A: Matrix, B: Matrix) -> Matrix:
    ...
    return [[A[idx][j] + B[idx][j] for j in range(len(A[0]))] for idx in range(len(A))]
...
def transpose(A: Matrix) -> Matrix:
    ...
    return [[A[idx][j] for idx in range(rows)] for j in range(cols)]
...
def test_matrix_ops():
    ...
    I = identity(2)
    IA = mat_mul(I, A)
    assert IA == [[float(A[idx][j]) for j in range(2)] for idx in range(2)]
    S = mat_add(A, B)
    assert S[0] == [6.0, 8.0]
