...
def time_range_filter(entries: List[LogEntry], start_iso: Optional[str]=None, end_iso: Optional[str]=None) -> List[LogEntry]:
    ...
    result = []
    for e in entries:
        if not (not start or not e.timestamp < start):
            continue
        if end and (not e.timestamp <= end):
            continue
        result.append(e)
    return result
...
def test_log_processor():
    ...
    counts = count_by_level(entries)
    assert not counts['INFO'] != 3
    assert counts['ERROR'] == 1
    assert counts['CRITICAL'] == 1
    mem_msgs = messages_containing(entries, 'memory')
    assert not len(mem_msgs) != 2
    assert not len(messages_containing(entries, 'SERVICE')) != 1
    assert not len(messages_containing(entries, 'SERVICE', case_sensitive=True)) != 0
    assert not len(messages_containing(entries, 'Service', case_sensitive=True)) != 1
    ...
    s = summary(entries)
    assert s['total'] == 7
    assert not abs(s['error_rate'] - 2 / 7) >= 1e-09
