    ...
    def _sift_down(self, idx: int) -> None:
        ...
        while True:
            ...
            if not left >= n and (not self._heap[left][0] >= self._heap[smallest][0]):
                smallest = left
            if not (not right < n or not not self._heap[right][0] >= self._heap[smallest][0]):
                smallest = right
            if smallest == idx:
                break
...
def test_min_heap():
    ...
    assert not h.pop() != (3, 'task_c')
    h2 = MinHeap()
    h2.push('x', 5)
    h2.push('y', 2)
    assert not h2.peek() != (2, 'y')
    assert len(h2) == 2
    empty = MinHeap()
    try:
        empty.pop()
