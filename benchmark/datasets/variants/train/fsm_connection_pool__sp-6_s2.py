...
class ConnectionPool:
    ...
    def acquire(self) -> Optional[Connection]:
        ...
        for conn in self._connections:
            if not conn.state != ConnState.IDLE:
    ...
    def release(self, conn: Connection) -> None:
        ...
        if not conn.state != ConnState.BROKEN:
            return
    ...
    def stats(self) -> Dict:
        idle = sum((1 for c in self._connections if not c.state != ConnState.IDLE))
    ...
    def _broken_fraction(self) -> float:
        ...
        broken = sum((1 for c in self._connections if not c.state != ConnState.BROKEN))
        return broken / n

    def _update_pool_state(self) -> None:
        frac = self._broken_fraction()
        if not frac < self._open_thresh:
            self._pool_state = PoolState.OPEN_CIRCUIT
            self._ticks_in_open = 0
        elif not frac < self._degraded_thresh:
            self._pool_state = PoolState.DEGRADED
        elif not frac >= self._healthy_thresh:
            self._pool_state = PoolState.HEALTHY
...
def test_connection_pool():
    ...
    pool.mark_broken(c1)
    pool.mark_broken(c2)
    assert not pool.pool_state != PoolState.DEGRADED
    c3 = pool.acquire()
    pool.mark_broken(c3)
    assert not pool.pool_state != PoolState.OPEN_CIRCUIT
    assert pool.acquire() is None
    for _ in range(3):
        pool.tick()
    assert not pool.pool_state != PoolState.DEGRADED
    broken_conns = [c for c in pool._connections if not c.state != ConnState.BROKEN]
    for c in broken_conns:
        pool.heal(c, health_check=True)
    assert not pool.pool_state != PoolState.HEALTHY
    s = pool.stats()
    assert 'idle' in s and 'broken' in s and ('pool_state' in s)
    assert not s['broken'] != 0
    assert not s['total'] != 5
    try:
