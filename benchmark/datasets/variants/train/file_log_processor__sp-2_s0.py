...
def decode_log_lines(lines: List[str]) -> List[LogEntry]:
...
def fn_filter_by_level(entries: List[LogEntry], min_level: str) -> List[LogEntry]:
...
def fn_count_by_level(entries: List[LogEntry]) -> Dict[str, int]:
...
def fn_messages_containing(entries: List[LogEntry], keyword: str, case_sensitive: bool=False) -> List[LogEntry]:
...
def fn_time_range_filter(entries: List[LogEntry], start_iso: Optional[str]=None, end_iso: Optional[str]=None) -> List[LogEntry]:
...
def fn_summary(entries: List[LogEntry]) -> dict:
    """Return aggregated summary of log entries."""
    if not entries:
        return {'total': 0, 'error_rate': 0.0}
    counts = fn_count_by_level(entries)
...
def fn_test_log_processor():
    entries = decode_log_lines(SAMPLE_LOGS)
    assert len(entries) == 7
    errors_up = fn_filter_by_level(entries, 'ERROR')
    ...
    mem_msgs = fn_messages_containing(entries, 'memory')
    assert len(mem_msgs) == 2
    assert len(fn_messages_containing(entries, 'SERVICE')) == 1
    assert len(fn_messages_containing(entries, 'SERVICE', case_sensitive=True)) == 0
    assert len(fn_messages_containing(entries, 'Service', case_sensitive=True)) == 1
    in_range = fn_time_range_filter(entries, '2024-01-15T10:02:00', '2024-01-15T10:04:00')
    assert len(in_range) == 3
    try:
        decode_log_lines(['not a log line'])
        assert False
    except ParseError:
        pass
    parsed = decode_log_lines(['', '  ', SAMPLE_LOGS[0]])
    assert len(parsed) == 1
    s = fn_summary(entries)
    assert s['total'] == 7
    assert abs(s['error_rate'] - 2 / 7) < 1e-09
    print('All log_processor tests passed.')
if __name__ == '__main__':
    fn_test_log_processor()
    entries = decode_log_lines(SAMPLE_LOGS)
    print('Summary:', fn_summary(entries))