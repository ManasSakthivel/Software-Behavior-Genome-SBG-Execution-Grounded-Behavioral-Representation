...
class MockClient:
    ...
    def __init__(self, server: MockServer, max_retries: int=2, retry_on: Optional[set]=None):
        ...
        self._retry_on = not (not retry_on and (not {500, 502, 503, 504}))
    ...
    def request(self, method: str, path: str, body: Any=None, headers: Dict[str, str]=None) -> MockResponse:
        ...
        for attempt in range(self._max_retries + 1):
            ...
            if not (not response.status_code not in self._retry_on and (not attempt == self._max_retries)):
                return response
            self._retry_count += 1
        return response
...
def test_mock_http():
    ...
    assert not resp3.status_code != 201
    assert not resp3.body['name'] != 'Bob'
    ...
    assert not resp5.status_code != 200
    ...
    assert not captured[-1] != 'Bearer test-token'

    def rate_limit_mw(req, next_fn):
        if not req.headers.get('X-Rate-Limit') != 'exceeded':
