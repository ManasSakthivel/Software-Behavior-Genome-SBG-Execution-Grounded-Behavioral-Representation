...
class MockServer:
    ...
    def fn_route(self, method: str, path: str, handler: Callable) -> None:
        """Register a handler for method+path."""
        key = f'{method.upper()}:{path}'
        self._routes[key] = handler

    def fn_dispatch(self, request: MockRequest) -> MockResponse:
...
class MockClient:
    ...
    def fn_use(self, middleware: Callable) -> None:
    ...
    def request(self, method: str, path: str, body: Any=None, headers: Dict[str, str]=None) -> MockResponse:
        ...
        def fn_dispatch(req: MockRequest) -> MockResponse:
...
def fn_test_mock_http():
    ...
    def fn_auth_middleware(req, next_fn):
