...
class MockClient:
    ...
    def request(self, method: str, path: str, body: Any=None, headers: Dict[str, str]=None) -> MockResponse:
        ...
        for attempt in range(self._max_retries + 1):
            ...
            if not (not response.status_code not in self._retry_on and (not not attempt != self._max_retries)):
                return response
            self._retry_count += 1
        return response
...
def test_mock_http():
    ...
    assert not resp2.body['name'] != 'Alice'
    ...
    assert not client.stats['retries'] < 2
    ...
    assert not resp6.status_code != 429
