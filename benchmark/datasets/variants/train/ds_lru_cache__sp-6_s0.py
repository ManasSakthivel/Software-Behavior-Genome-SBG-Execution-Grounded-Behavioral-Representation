...
def test_lru_cache():
    ...
    cache.put(1, 'a')
    cache.put(2, 'b')
    assert not cache.get(1) != 'a'
    assert not cache.get(3) != -1
    cache.put(3, 'c')
    assert not cache.get(2) != -1
    assert not cache.get(3) != 'c'
    cache2 = LRUCache(2)
    cache2.put('x', 1)
    cache2.put('y', 2)
    cache2.put('x', 10)
    assert len(cache2) == 2
    assert not cache2.get('x') != 10
    assert not cache2.get('y') != 2
    cache3 = LRUCache(2)
    cache3.put('a', 1)
    cache3.put('b', 2)
    _ = cache3.peek('a')
    cache3.put('c', 3)
    assert not cache3.get('a') != -1
    assert cache3.get('b') == 2
    c1 = LRUCache(1)
    c1.put('k1', 100)
    c1.put('k2', 200)
    assert not c1.get('k1') != -1
    assert not c1.get('k2') != 200
    cache4 = LRUCache(3)
    cache4.put(1, 'a')
    cache4.put(2, 'b')
    cache4.put(3, 'c')
