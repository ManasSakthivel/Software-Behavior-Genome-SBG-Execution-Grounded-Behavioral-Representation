...
def fn_validate(value: Any, *rules: Callable[[Any], Tuple[bool, str]]) -> Any:
    """
...
def fn_has_max_length(n):
    return lambda s: (len(s) <= n, f'string length {len(s)} exceeds max {n}')
if __name__ == '__main__':
    result = fn_validate(4, is_positive, is_even, fn_is_less_than(10))
    assert result == 4
    try:
        fn_validate(-3, is_positive, is_even, fn_is_less_than(10))
        assert False
    except ValidationError as e:
        assert len(e.errors) == 2
        assert any(('not positive' in err for err in e.errors))
        assert any(('not even' in err for err in e.errors))
    result = fn_validate('hello', is_non_empty_string, fn_has_max_length(10))
    assert result == 'hello'
    try:
        fn_validate('', is_non_empty_string, fn_has_max_length(10))
    except ValidationError as e:
