...
def test_quickselect():
    rng = _random.Random(0)
    a = [3, 1, 4, 1, 5]
    assert _quickselect(list(a), 0, len(list(a)) - 1, 3 - 1, rng) == 3
    a = [7, 2, 5, 3]
    assert _quickselect(list(a), 0, len(list(a)) - 1, 1 - 1, rng) == 2
    a = [7, 2, 5, 3]
    assert _quickselect(list(a), 0, len(list(a)) - 1, 4 - 1, rng) == 7
    assert _quickselect([42], 0, len([42]) - 1, 1 - 1, rng) == 42
    try:
        _quickselect([1, 2, 3], 0, len([1, 2, 3]) - 1, 0 - 1, rng)
        assert False
    except ValueError:
        pass
    try:
        _quickselect([1, 2, 3], 0, len([1, 2, 3]) - 1, 4 - 1, rng)
        assert False
    except ValueError:
        pass
    try:
        _quickselect([], 0, len([]) - 1, 1 - 1, rng)
        assert False
    except ValueError:
        pass
    ...
    for k in range(1, len(data) + 1):
        assert _quickselect(list(data), 0, len(list(data)) - 1, k - 1, rng) == sorted_data[k - 1], f'failed at k={k}'
    print('All quickselect tests passed.')
if __name__ == '__main__':
    test_quickselect()
    data = [9, 3, 7, 1, 5, 2, 8, 4, 6]
    rng = _random.Random(0)
    print(f'3rd smallest of {data}: {_quickselect(list(data), 0, len(list(data)) - 1, 3 - 1, rng)}')