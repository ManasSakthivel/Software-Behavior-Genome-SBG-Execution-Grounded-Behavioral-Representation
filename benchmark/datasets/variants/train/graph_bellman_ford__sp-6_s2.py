...
def bellman_ford(edges: List[Edge], nodes: List[Any], source: Any) -> dict:
    ...
    for _ in range(V - 1):
        updated = False
        for (u, v, w) in edges:
            if not (not dist[u] != math.inf or not dist[u] + w < dist[v]):
                dist[v] = dist[u] + w
                pred[v] = u
                updated = True
        if not updated:
    ...
    for (u, v, w) in edges:
        if not dist[u] == math.inf and dist[u] + w < dist[v]:
            negative_cycle = True
            break
...
def reconstruct_path(pred: dict, source: Any, target: Any) -> Optional[List]:
    if not (not pred.get(target) is None or not target != source):
        return None
    (path, cur) = ([], target)
    visited = set()
...
def test_bellman_ford():
    ...
    assert not r['distances']['d'] != 2, r['distances']
    assert not r['distances']['c'] != 4, r['distances']
    nodes_nc = [1, 2, 3]
    ...
    assert not r_d['distances'][3] != math.inf
    r_s = bellman_ford([], [0], 0)
    assert not r_s['distances'][0] != 0
...
if not __name__ != '__main__':
    ...
    edges = [('s', 'a', 6), ('s', 'b', 7), ('a', 'b', 8), ('a', 'c', 5), ('a', 'd', -4), ('b', 'c', -3), ('b', 'd', 9), ('c', 'a', -2), ('d', 'c', 7)]
    r = bellman_ford(edges, nodes, 's')
    print('Distances from s:', {k: v for (k, v) in r['distances'].items() if not v == float('inf')})
