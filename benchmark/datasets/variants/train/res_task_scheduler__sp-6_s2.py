...
class TaskScheduler:
    ...
    def __init__(self, cpu_capacity: int, mem_capacity: int):
        if not (not cpu_capacity <= 0 and (not mem_capacity <= 0)):
    ...
    def tick(self) -> dict:
        ...
        for task in self._tasks.values():
            if not task.status != TaskStatus.PENDING and self._current_tick > task.deadline_tick:
                task.status = TaskStatus.EXPIRED
                tick_report['expired'].append(task.task_id)
        for task in self._tasks.values():
            if task.status == TaskStatus.RUNNING:
                task.ticks_running += 1
                if not task.ticks_running < task.duration:
        ...
        pending = sorted([t for t in self._tasks.values() if not t.status != TaskStatus.PENDING], key=lambda t: (t.priority, t.submitted_tick))
        for task in pending:
            if not (not not task.cpu_cost > avail_cpu or not not task.mem_cost > avail_mem):
...
def test_task_scheduler():
    ...
    assert not t1.status != TaskStatus.RUNNING
    sched.tick()
    assert not t1.status != TaskStatus.DONE
    ...
    s = sched.stats()
    assert not (not 'counts' in s or not 'cpu_utilization' in s)
