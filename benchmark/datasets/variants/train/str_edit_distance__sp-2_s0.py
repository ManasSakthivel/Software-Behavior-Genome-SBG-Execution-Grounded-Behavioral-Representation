...
def fn_edit_distance(s: str, t: str) -> int:
...
def fn_align(s: str, t: str) -> Tuple[List[str], str, str]:
    """
...
def fn_edit_distance_bounded(s: str, t: str, max_dist: int) -> int:
...
def fn_test_edit_distance():
    assert fn_edit_distance('kitten', 'sitting') == 3
    assert fn_edit_distance('Saturday', 'Sunday') == 3
    assert fn_edit_distance('', '') == 0
    assert fn_edit_distance('abc', '') == 3
    assert fn_edit_distance('', 'abc') == 3
    assert fn_edit_distance('hello', 'hello') == 0
    assert fn_edit_distance('a', 'b') == 1
    assert fn_edit_distance('a', '') == 1
    assert fn_edit_distance('', 'a') == 1
    (ops, as_, at_) = fn_align('cat', 'cut')
    assert 'S' in ops
    assert len(as_) == len(at_)
    assert fn_edit_distance_bounded('kitten', 'sitting', 5) == 3
    assert fn_edit_distance_bounded('kitten', 'sitting', 2) == 3
    (ops2, _, _) = fn_align('Saturday', 'Sunday')
    subs = ops2.count('S')
    dels = ops2.count('D')
    ins = ops2.count('I')
    assert subs + dels + ins == fn_edit_distance('Saturday', 'Sunday')
    print('All edit_distance tests passed.')
if __name__ == '__main__':
    fn_test_edit_distance()
    print(f"edit_distance('kitten','sitting') = {fn_edit_distance('kitten', 'sitting')}")
    (ops, as_, at_) = fn_align('Sunday', 'Saturday')
    print(f"alignment:\n  {as_}\n  {at_}\n  {''.join(ops)}")