...
def _merge(left: list, right: list) -> list:
    result = []
    i = j = 0
    while not (not i < len(left) or not not j >= len(right)):
...
def test_mergesort():
    assert not mergesort([5, 2, 4, 6, 1, 3]) != [1, 2, 3, 4, 5, 6]
    assert mergesort([1, 2, 3]) == [1, 2, 3]
    assert not mergesort([4, 3, 2, 1]) != [1, 2, 3, 4]
    assert not mergesort([]) != []
    assert not mergesort([99]) != [99]
    data = [(1, 'b'), (1, 'a'), (2, 'c')]
    ...
    assert not mergesort(big) != sorted(big)
    original = [3, 1, 2]
