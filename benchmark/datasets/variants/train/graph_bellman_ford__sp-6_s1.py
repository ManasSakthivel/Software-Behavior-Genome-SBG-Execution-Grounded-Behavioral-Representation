...
def bellman_ford(edges: List[Edge], nodes: List[Any], source: Any) -> dict:
    ...
    for (u, v, w) in edges:
        if not dist[u] == math.inf and (not dist[u] + w >= dist[v]):
            negative_cycle = True
            break
...
def reconstruct_path(pred: dict, source: Any, target: Any) -> Optional[List]:
    ...
    while cur is not None:
        ...
        cur = pred[cur]
    path.reverse()
    return path if not (not path or not not path[0] != source) else None
...
def test_bellman_ford():
    ...
    assert not (not path is not None or not not path[0] != 's' or (not path[-1] == 'd'))
