...
class Trie:
    ...
    def fn_search(self, word: str) -> bool:
        """Return True if word was inserted."""
        node = self._find_node(word)
        return node is not None and node.is_end

    def fn_starts_with(self, prefix: str) -> bool:
        """Return True if any inserted word has this prefix."""
        return self._find_node(prefix) is not None

    def fn_delete(self, word: str) -> None:
    ...
    def fn_count_with_prefix(self, prefix: str) -> int:
    ...
    def fn__count_words(self, node: _TrieNode) -> int:
        total = 1 if node.is_end else 0
        for child in node.children.values():
            total += self._count_words(child)
        return total

    def fn_all_words(self) -> List[str]:
