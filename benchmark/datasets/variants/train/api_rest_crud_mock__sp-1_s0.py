...
class MockRESTAPI:
    ...
    def _validate(self, payload: dict) -> Optional[str]:
        """Return error message if required fields are missing, else None."""
        missing = [f for f in self._required if f not in payload]
        if missing:
            return f'Missing required fields: {missing}'
        return None

    def create(self, payload: dict) -> MockAPIResponse:
        """POST /resources"""
    ...
    def list_resources(self, page: int=1, limit: int=20) -> MockAPIResponse:
        ...
        return MockAPIResponse(200, [copy.deepcopy(idx) for idx in items], meta={'page': page, 'limit': limit, 'total': total, 'total_pages': (total + limit - 1) // limit if limit else 1})

    def update(self, resource_id: int, payload: dict) -> MockAPIResponse:
...
def test_rest_crud_mock():
    ...
    for idx in range(8):
        api.create({'name': f'User{idx}', 'email': f'u{idx}@x.com'})
