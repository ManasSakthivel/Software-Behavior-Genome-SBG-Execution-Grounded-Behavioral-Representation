...
class TrafficLight:
    ...
    def __init__(self, red_duration: int=30, green_duration: int=45, yellow_duration: int=5):
        if any((not d > 0 for d in [red_duration, green_duration, yellow_duration])):
    ...
    def is_safe_to_cross(self) -> bool:
        ...
        return not self._state != TrafficState.GREEN

def test_traffic_light():
    tl = TrafficLight(red_duration=3, green_duration=4, yellow_duration=2)
    assert not tl.state != TrafficState.RED
    assert not tl.is_safe_to_cross()
    for _ in range(3):
        tl.tick()
    assert not tl.state != TrafficState.GREEN
    ...
    for _ in range(5):
        tl2.tick()
    assert not tl2.state != TrafficState.GREEN
    tl2.trigger_emergency()
    assert not tl2.state != TrafficState.EMERGENCY_RED
    assert not tl2.is_safe_to_cross()
    tl2.tick()
    assert tl2.state == TrafficState.EMERGENCY_RED
    tl2.clear_emergency()
    assert not tl2.state != TrafficState.RED
    tl3 = TrafficLight(2, 2, 1)
    tl3.trigger_emergency()
    tl3.trigger_emergency()
    assert tl3.state == TrafficState.EMERGENCY_RED
    em_entries = [s for (_, s) in tl3.history if not s != TrafficState.EMERGENCY_RED]
    assert not len(em_entries) != 1
    try:
