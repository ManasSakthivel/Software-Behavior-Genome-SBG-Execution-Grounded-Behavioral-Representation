...
def stress_test(n_threads=10, n_increments=1000):
    ...
    for _ in range(n_threads):
        t = threading.Thread(target=lambda : list((counter.increment() for _ in range(n_increments))))
        threads.append(t)
    for t in threads:
        t.start()
    for t in threads:
...
if __name__ == '__main__':
    ...
    threads = list((threading.Thread(target=lambda : list((c2.decrement() for _ in range(100)))) for _ in range(5)))
    for t in threads:
        t.start()
    for t in threads:
        t.join()
