...
class MinHeap:
    ...
    def __init__(self):
        self._heap: list = []   # list of (priority, key)
        self._positions: dict = {}  # key → index (for decrease_key)
...
def test_min_heap():
    ...
    # Test 3: pop from empty raises IndexError
    empty = MinHeap()
    try:
        empty.pop()
        assert False
    ...
    # Test 4: decrease_key re-orders the heap
    h3 = MinHeap()
    h3.push("a", 10)
    h3.push("b", 5)
    h3.push("c", 8)
    h3.decrease_key("a", 1)  # a should now be minimum
    assert h3.pop() == (1, "a")

    # Test 5: decrease_key to higher priority raises
    h4 = MinHeap()
    h4.push("x", 3)
    try:
        h4.decrease_key("x", 5)
    ...
    # Test 6: decrease_key on missing key raises
    try:
    ...
    # Test 7: heap property maintained on many pushes
    import random
