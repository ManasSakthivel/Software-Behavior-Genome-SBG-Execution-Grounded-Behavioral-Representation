...
class Task:
    ...
    deadline_tick: int
    cpu_cost: int           # CPU units needed per tick
    mem_cost: int           # memory units needed per tick
    duration: int = 1       # ticks required to complete
...
class TaskScheduler:
    ...
    def tick(self) -> dict:
        ...
        # Step 1: expire overdue PENDING tasks
        ...
        # Step 3: recalculate available resources (active RUNNING tasks)
        ...
        # Step 4: schedule PENDING tasks in priority order
        pending = sorted(
            [t for t in self._tasks.values() if t.status == TaskStatus.PENDING],
            key=lambda t: (t.priority, t.submitted_tick)
        )
        for task in pending:
    ...
    def stats(self) -> dict:
        ...
        return {
            ...
            "counts": counts,
            "cpu_utilization": running_cpu / self._cpu_cap,
            "mem_utilization": running_mem / self._mem_cap,
        }
...
def test_task_scheduler():
    ...
    sched.submit(t2); sched.submit(t3)
    report3 = sched.tick()   # tick 3
    assert t3.task_id in report3["started"]
    assert t2.task_id in report3["started"]   # both fit (cpu: 2+2=4 ≤ 10)

    # Test 3: deadline expiry
    t4 = Task("urgent", priority=1, deadline_tick=3, cpu_cost=5, mem_cost=5, duration=1)
    sched.submit(t4)
    sched.tick()   # tick 4 → deadline 3 already passed
    assert t4.status == TaskStatus.EXPIRED

    # Test 4: cancel pending task
    t5 = Task("cancelme", priority=2, deadline_tick=100, cpu_cost=1, mem_cost=1, duration=1)
    id5 = sched.submit(t5)
    sched.cancel(id5)
    assert t5.status == TaskStatus.CANCELLED

    # Test 5: cannot cancel non-pending task
    try:
        sched.cancel(id1)   # already DONE
        assert False
    except SchedulerError:
        pass

    # Test 6: resource contention — task waits if resources unavailable
    sched2 = TaskScheduler(cpu_capacity=5, mem_capacity=10)
    ta = Task("A", 1, 100, cpu_cost=4, mem_cost=2, duration=3)
    tb = Task("B", 1, 100, cpu_cost=3, mem_cost=2, duration=1)
    sched2.submit(ta); sched2.submit(tb)
    r = sched2.tick()
    # ta scheduled (4 cpu used), tb can't fit (only 1 cpu left for 3-cost task)
    assert ta.task_id in r["started"]
    assert tb.task_id not in r["started"]

    # Test 7: stats structure
    s = sched.stats()
