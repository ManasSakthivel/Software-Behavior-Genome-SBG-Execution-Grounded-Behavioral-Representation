...
class HashTable:
    ...
    def __init__(self, initial_capacity: int = 7):
...
def test_hash_table():
    ...
    # Test 3: KeyError on missing key
    try:
        ht.get("missing")
        assert False
    except KeyError:
    ...
    # Test 6: resize triggers correctly under load
    ht2 = HashTable(8)
    for i in range(20):
    ...
    # Test 7: items / keys / values iteration
    ht3 = HashTable()
    ht3.put(1, "a"); ht3.put(2, "b"); ht3.put(3, "c")
    assert set(ht3.keys()) == {1, 2, 3}
    assert set(ht3.values()) == {"a", "b", "c"}
    assert set(ht3.items()) == {(1, "a"), (2, "b"), (3, "c")}

    # Test 8: integer keys with potential hash collision
    import random
