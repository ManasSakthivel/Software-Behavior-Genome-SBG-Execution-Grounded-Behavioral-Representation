...
def bellman_ford(edges: List[Edge], nodes: List[Any], source: Any) -> dict:
    ...
    # Check for negative cycles (one extra pass)
...
def reconstruct_path(pred: dict, source: Any, target: Any) -> Optional[List]:
    ...
    while cur is not None:
        if cur in visited:
            return None  # cycle in predecessor chain
        visited.add(cur)
        path.append(cur)
        cur = pred[cur]
    path.reverse()
...
def test_bellman_ford():
    ...
    assert r["distances"]["c"] == 3, r["distances"]
    ...
    # Test 3: disconnected node remains inf
    nodes_d = [1, 2, 3]
    edges_d = [(1, 2, 5)]
    ...
    # Test 5: source not in nodes raises ValueError
    try:
        bellman_ford([], [1, 2], 99)
        assert False
    except ValueError:
