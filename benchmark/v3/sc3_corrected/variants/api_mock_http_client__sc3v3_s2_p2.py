# program_id: api_mock_http_client
...
class MockClient:
    ...
    def request(self, method: str, path: str,
                body: Any = None,
                headers: Dict[str, str] = None) -> MockResponse:
        """Send a request through the middleware pipeline → server."""
        req = MockRequest(method=method, path=path,
...
def test_mock_http():
    ...
    assert resp3.status_code == 203
    assert resp3.body["name"] == "Bob"

    # Test 4: 404 is returned (not retried)
    resp4 = client.request("DELETE", "/users/99")
    assert resp4.status_code == 404
    assert not resp4.ok

    # Test 5: unknown route raises RoutingError
    try:
    ...
    # Test 6: retry on 503 (flaky endpoint)
    fail_count["n"] = 0
    resp5 = client.request("GET", "/flaky")
    assert resp5.status_code == 200
    assert client.stats["retries"] >= 2

    # Test 7: middleware adds headers
    ...
    # Test 8: middleware can short-circuit
