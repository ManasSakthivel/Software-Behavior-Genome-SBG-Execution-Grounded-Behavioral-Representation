# program_id: math_numerical_integration
...
def adaptive_simpsons(f: Callable[[float], float],
                      a: float, b: float,
                      tol: float = 1e-6,
                      max_depth: int = 51) -> float:
    """
    ...
    def _recursive(a, b, tol, whole, depth):
        ...
        return (_recursive(a, mid, tol / 2, left, depth + 1) +
                _recursive(mid, b, tol / 2, right, depth + 1))
...
def test_numerical_integration():
    ...
    # Test 1: ∫[0,1] x dx = 0.5 (trapezoid)
    assert abs(trapezoid(lambda x: x, 0, 1, 1000) - 0.5) < 1e-4

    # Test 2: ∫[0,1] x² dx = 1/3 (Simpson's)
    assert abs(simpsons(lambda x: x**2, 0, 1, 100) - 1/3) < 1e-8

    # Test 3: ∫[0,π] sin(x) dx = 2 (adaptive)
    result = adaptive_simpsons(math.sin, 0, math.pi, tol=1e-8)
    assert abs(result - 2.0) < 1e-6, result

    # Test 4: ∫[0,1] e^x dx = e - 1 ≈ 1.71828 (all methods)
    exact = math.e - 1
    assert abs(trapezoid(math.exp, 0, 1, 1000) - exact) < 1e-4
    assert abs(simpsons(math.exp, 0, 1, 100) - exact) < 1e-8
    assert abs(adaptive_simpsons(math.exp, 0, 1) - exact) < 1e-6

    # Test 5: ∫[1,2] 1/x dx = ln(2) ≈ 0.6931
    exact_ln2 = math.log(2)
    assert abs(simpsons(lambda x: 1/x, 1, 2, 100) - exact_ln2) < 1e-8

    # Test 6: odd n for Simpson's raises
    try:
    ...
    # Test 9: ∫[0,1] constant 5 dx = 5 (all methods)
    for fn in [lambda n: trapezoid(lambda x: 5, 0, 1, n),
               lambda n: simpsons(lambda x: 5, 0, 1, n)]:
