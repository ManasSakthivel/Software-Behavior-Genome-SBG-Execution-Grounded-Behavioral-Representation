...
class ConfigParser:
    ...
    def get_bool(self, section: str, key: str, default: bool = None) -> bool:
        ...
        if isinstance(val, str):
            if val.lower() in ("true", "yes", "1", "on"):  return True
            if val.lower() in ("false", "no", "0", "off"): return False
...
def test_config_parser():
    ...
    assert cfg.get_int("server", "workers") == 5

    # Test 4: get_float
    assert abs(cfg.get_float("cache", "ttl") - 3600.5) < 1e-9

    # Test 5: get_bool
    assert cfg.get_bool("database", "debug") == False
    assert cfg.get_bool("cache", "enabled") == True

    # Test 6: multi-value list
    tags = cfg.get("server", "tags")
    assert isinstance(tags, list)
    assert tags == ["web", "api", "prod"]

    # Test 7: inline comment stripped
    assert cfg.get("cache", "ttl") == "3600.5"   # comment stripped

    # Test 8: set and retrieve
    cfg.set("database", "password", "secret")
    assert cfg.get("database", "password") == "secret"

    # Test 9: duplicate section raises (default)
    try:
    ...
    # Test 12: dump round-trip (structural)
    dumped = cfg.dump()
