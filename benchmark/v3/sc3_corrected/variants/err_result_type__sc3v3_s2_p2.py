...
if __name__ == "__main__":
    # Test 1: map chain on Ok
    r = Result.ok(10).map(lambda x: x * 2).map(lambda x: x + 1)
    assert r.unwrap() == 21

    # Test 2: Err propagates through map
    r = Result.err("bad").map(lambda x: x * 2)
    assert r.is_err()
    assert r.unwrap_err() == "bad"

    # Test 3: flat_map composition
    r = parse_int("42").flat_map(lambda n: safe_divide(102, n))
    assert abs(r.unwrap() - 100/42) < 1e-9
