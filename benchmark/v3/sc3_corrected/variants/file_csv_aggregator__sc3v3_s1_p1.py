# program_id: file_csv_aggregator
...
def group_by(df: DataFrame, key_col: str, agg_col: str, agg_func: str) -> dict:
    ...
    for key, vals in groups.items():
        if agg_func == "sum":   result[key] = sum(vals)
        elif agg_func == "count": result[key] = len(vals)
        elif agg_func == "mean": result[key] = sum(vals) / len(vals)
        elif agg_func == "min":  result[key] = min(vals)
        elif agg_func == "max":  result[key] = max(vals)
    return result
...
def test_csv_aggregator():
    ...
    # Test 1: correct column count and row count
    ...
    assert abs(total_by_dept["Engineering"] - 288000.0) < 1e-6
    assert abs(total_by_dept["Marketing"] - 140000.0) < 1e-6

    # Test 4: group_by count
    count_by_dept = group_by(df, "department", "name", "count")
    assert count_by_dept["Engineering"] == 2
    assert count_by_dept["Marketing"] == 2

    # Test 5: group_by mean
    mean_salary = group_by(df, "department", "salary", "mean")
    assert abs(mean_salary["Engineering"] - 96000.0) < 1e-6
    ...
    # Test 9: round-trip to_csv → load_csv
    csv_out = to_csv(df)
    df2 = load_csv(csv_out)
    assert df2["salary"] == df["salary"]

    # Test 10: missing column raises
    try:
