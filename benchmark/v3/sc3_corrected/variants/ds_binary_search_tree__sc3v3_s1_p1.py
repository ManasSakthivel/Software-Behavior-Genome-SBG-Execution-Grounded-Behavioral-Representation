# program_id: ds_binary_search_tree
...
def test_bst():
    ...
    # Test 2: inorder returns sorted keys
    ...
    assert len(bst) == 5  # no size increase
    ...
    bst.delete(3)   # 3 had right child 4
    ...
    for k in [5, 3, 7, 1, 4, 6, 8]:
        bst2.insert(k, k)
    bst2.delete(5)   # root with two children
    keys2 = [k for k, _ in bst2.inorder()]
    assert keys2 == [1, 3, 4, 6, 7, 8]

    # Test 7: search missing key raises KeyError
    try:
        bst2.search(99)
        assert False
    except KeyError:
    ...
    # Test 8: delete missing key raises KeyError
    try:
        bst2.delete(99)
        assert False
    except KeyError:
