...
class TrafficLight:
    ...
    def __init__(self,
                 red_duration: int = 30,
                 green_duration: int = 45,
                 yellow_duration: int = 7):
    ...
    def tick(self) -> TrafficState:
        ...
        # Determine duration for current state
        duration_map = {
            TrafficState.RED:    self._red_dur,
            TrafficState.GREEN:  self._grn_dur,
            TrafficState.YELLOW: self._yel_dur,
        }
...
def test_traffic_light():
    ...
    # Test 1: initial state is RED
    assert tl.state == TrafficState.RED
    assert not tl.is_safe_to_cross()

    # Test 2: transitions after red_duration ticks
    for _ in range(3):
        tl.tick()
    assert tl.state == TrafficState.GREEN
    assert tl.is_safe_to_cross()

    # Test 3: GREEN → YELLOW after green_duration
    for _ in range(4):
        tl.tick()
    assert tl.state == TrafficState.YELLOW
    assert not tl.is_safe_to_cross()

    # Test 4: YELLOW → RED after yellow_duration
    for _ in range(2):
        tl.tick()
    assert tl.state == TrafficState.RED

    # Test 5: full cycle produces history
    ...
    tl2.tick()   # stays in EMERGENCY_RED
    assert tl2.state == TrafficState.EMERGENCY_RED

    # Test 7: clear_emergency returns to RED
    tl2.clear_emergency()
    assert tl2.state == TrafficState.RED

    # Test 8: double emergency trigger is idempotent
    ...
    # Test 9: invalid duration raises
    try:
