...
def _tokenize(text: str) -> List[str]:
    ...
    raw = re.split(r"\s+", text.lower().strip())
    tokens = []
    for token in raw:
...
def top_n_words(text: str,
                n: int,
                stop_words: Optional[Set[str]] = None,
                stem: bool = False) -> List[Tuple[str, int]]:
...
def test_word_frequency():
    ...
    # Test 1: basic frequency (case-insensitive, punctuation stripped)
    ...
    # "running" → "runn", "runs" → "run", "quickly" twice → "quick"
    assert freq3.get("quick", 0) == 4

    # Test 4: top_n_words
    top = top_n_words(text, 3)
    assert top[0][0] == "the" and top[0][1] == 3  # "the" appears 3 times

    # Test 5: unique_words
    unique = unique_words("cat bat cat rat")
    assert unique == {"cat", "bat", "rat"}

    # Test 6: lexical_diversity
    ...
    # Test 7: apostrophes inside words preserved
    freq4 = word_frequencies("it's it's it's")
    assert freq4.get("it's", 0) == 3

    # Test 8: punctuation stripping
...
def pytest_approx(val):
    ...
    class _Approx:
        ...
        def __eq__(self, other): return abs(self.v - other) < 1e-9
    return _Approx(val)
