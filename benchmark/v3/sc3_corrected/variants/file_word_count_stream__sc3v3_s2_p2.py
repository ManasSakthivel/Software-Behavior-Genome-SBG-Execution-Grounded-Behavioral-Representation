# program_id: file_word_count_stream
...
def process_stream(lines: Iterable[str],
                   chunk_size: Optional[int] = None) -> "StreamWordCounter":
...
def test_stream_word_counter():
    ...
    assert counter.get("to") == 4   # "to" appears 4 times

    # Test 2: line and char counts
    ...
    # Test 4: top_n
    top3 = counter.top_n(5)
    assert len(top3) == 3
    words_in_top3 = [w for w, _ in top3]
    assert "to" in words_in_top3   # most frequent

    # Test 5: chunked processing produces same result
    ...
    # Test 6: merge_counters (map-reduce)
    ...
    # Test 7: apostrophe handling
    c2 = StreamWordCounter()
    c2.feed("it's a cat's paw")
    assert c2.get("it's") == 1 or c2.get("its") == 0  # apostrophe stripped from ends
    ...
    # Test 9: numbers and punctuation excluded
    c3 = StreamWordCounter()
    c3.feed("hello 123 world! foo-bar")
    # "hello", "world", "foo", "bar" — numbers excluded, hyphen splits
