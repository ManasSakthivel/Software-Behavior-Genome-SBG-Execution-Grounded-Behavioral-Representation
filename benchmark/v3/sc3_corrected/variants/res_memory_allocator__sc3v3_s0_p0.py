# program_id: res_memory_allocator
...
def test_memory_allocator():
    # --- First Fit ---
    mem = MemoryAllocator(101, strategy="first_fit")
    ...
    # Test 2: allocation fails when full
    b4 = mem.alloc(1)
    assert b4 is None
    ...
    # Test 5: coalesce adjacent free blocks
    mem.free(b1)
    mem.free(b3)
    before_blocks = mem.stats()["block_count"]
    merges = mem.coalesce()
    assert merges > 0
    # All freed → single free block
    ...
    id2 = mem2.alloc(20)
    id3 = mem2.alloc(10)
    mem2.free(id1)  # 10 free at start
    mem2.free(id3)  # 10 free after id2

    # Test 6: best_fit should prefer the 10-byte hole for a 10-byte request
    id4 = mem2.alloc(10)
    assert id4 is not None
    # The 20-byte hole should remain if best-fit picked the smaller 10-byte hole

    # Test 7: fragmentation_ratio
    mem3 = MemoryAllocator(100)
    mem3.alloc(10); mem3.alloc(10); mem3.alloc(10)
    # All allocated → no free blocks → ratio = 0
    assert mem3.fragmentation_ratio() == 0.0

    # Test 8: invalid alloc size raises
    try:
        mem.alloc(0)
        assert False
    except ValueError:
