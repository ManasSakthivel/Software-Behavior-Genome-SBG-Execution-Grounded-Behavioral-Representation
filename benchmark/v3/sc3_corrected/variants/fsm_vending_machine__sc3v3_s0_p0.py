# program_id: fsm_vending_machine
...
def test_vending_machine():
    ...
    # Test 2: insert coins and check balance
    vm.insert_coin(25)
    vm.insert_coin(50)
    assert vm.get_balance() == 75
    assert vm.state == VMState.COLLECTING

    # Test 3: successful purchase with exact change
    ...
    # Test 4: purchase with change returned
    vm.insert_coin(100)
    ok, msg, change = vm.select_product("cola")
    ...
    vm.insert_coin(25)
    ok, msg, _ = vm.select_product("chips")   # chips costs 100
    assert not ok
    assert "Insufficient" in msg

    # Test 6: cancel refunds balance
    refund = vm.cancel()
    assert refund == 25
    assert vm.state == VMState.IDLE

    # Test 7: sold-out product
    vm.insert_coin(100)
    vm.select_product("chips")    # buys the last one
    vm.insert_coin(100)
    ok, msg, _ = vm.select_product("chips")
    assert not ok and "sold out" in msg
    vm.cancel()

    # Test 8: invalid coin raises
    try:
        vm.insert_coin(3)
        assert False
    except ValueError:
    ...
    # Test 9: total_sales tracking
    assert vm.total_sales == 75 + 75 + 100   # three cola + one chips

    # Test 10: select without coins
