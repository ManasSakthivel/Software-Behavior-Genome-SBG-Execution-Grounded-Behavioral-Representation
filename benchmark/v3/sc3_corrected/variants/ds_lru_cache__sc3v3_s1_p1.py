...
def test_lru_cache():
    # Test 1: basic get/put, cache hit and miss
    cache = LRUCache(2)
    cache.put(1, "a")
    cache.put(1, "b")
    ...
    # Test 2: eviction of LRU entry
    cache.put(3, "c")   # evicts 2 (LRU after get(1))
    assert cache.get(2) == -1
    assert cache.get(3) == "c"

    # Test 3: update existing key; it becomes MRU
    cache2 = LRUCache(2)
    cache2.put("x", 1)
    cache2.put("y", 2)
    cache2.put("x", 10)   # update "x", should not evict
    assert len(cache2) == 2
    assert cache2.get("x") == 10
    assert cache2.get("y") == 2

    # Test 4: peek does not change order
    cache3 = LRUCache(2)
    cache3.put("a", 1)
    cache3.put("b", 2)
    _ = cache3.peek("a")   # peek "a" but "b" is still MRU
    cache3.put("c", 3)     # should evict "a" (still LRU)
    assert cache3.get("a") == -1
    assert cache3.get("b") == 2

    # Test 5: capacity 1 — every new put evicts old entry
    c1 = LRUCache(1)
    c1.put("k1", 100)
    c1.put("k2", 200)
    assert c1.get("k1") == -1
    ...
    # Test 6: keys_lru_order
    cache4 = LRUCache(3)
    cache4.put(1, "a")
    cache4.put(2, "b")
    cache4.put(3, "c")
    cache4.get(1)   # makes 1 MRU
