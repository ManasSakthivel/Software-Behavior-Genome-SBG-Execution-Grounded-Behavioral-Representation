# program_id: math_dynamic_programming
...
def knapsack_01(weights: List[int],
    ...
    n = len(weights)
    # dp[i][w] = max value using items 0..i-1 with weight budget w
...
def test_dp():
    ...
    assert max_val == 7, max_val   # items 0 (w=2,v=3) + 1 (w=3,v=4)
    assert set(selected) == {0, 1}

    # Test 2: capacity 0 → no items
    ...
    # Test 4: mismatched lengths raise
    try:
        knapsack_01([1, 2], [1], 5)
        assert False
    except ValueError:
    ...
    assert lcs("ABCBDAB", "BDCABA") in ("BCBA", "BDAB", "BCAB")  # any valid LCS of length 4
    ...
    # Test 8: no common characters
    assert lcs("abc", "xyz") == ""

    # --- Coin Change ---
    # Test 9: standard coins
    assert coin_change([1, 5, 10, 25], 36) == 3   # 25+10+1

    # Test 10: impossible
    assert coin_change([3, 5], 7) == -1

    # Test 11: amount 0 → 0 coins
    assert coin_change([1, 5, 10], 0) == 0

    # Test 12: single coin exact
